...
class TestMdBasic(unittest.TestCase):
    ...
    def test_get_kline_serial_update(self):
        ...
        api = TqApi(backtest=TqBacktest(datetime(2019, 7, 1), datetime(2019, 7, 2)), _ins_url=self.ins_url_2019_07_03, _td_url=self.td_url, _md_url=self.md_url)

        quote = api.get_quote("SHFE.rb2001")
        df = api.get_kline_serial("SHFE.rb2001", 1, 10)

        self.assertEqual(quote["datetime"], "2019-06-28 22:59:59.999999")
        self.assertEqual(df["datetime"].iloc[-1], 1.561733999e+18)

        api.wait_update()
        df['ema5'] = df['close'].rolling(5, min_periods=1).mean()
        df.loc[df["ema5"] > 2.5, 'duo'] = 1
        self.assertEqual(quote["datetime"], "2019-07-01 09:00:00.000000")
        self.assertEqual(df["datetime"].iloc[-1], 1.5619428e+18)

        api.wait_update()
        df['ema5'] = df['close'].rolling(5, min_periods=1).mean()
        df.loc[df["ema5"] > 2.5, 'duo'] = 1
        self.assertEqual(quote["datetime"], "2019-07-01 09:00:00.999999")
        self.assertEqual(df["datetime"].iloc[-1], 1.5619428e+18)

        api.wait_update()
        df['ema5'] = df['close'].rolling(5, min_periods=1).mean()
        df.loc[df["ema5"] > 2.5, 'duo'] = 1
        self.assertEqual(quote["datetime"], "2019-07-01 09:00:01.000000")
        self.assertEqual(df["datetime"].iloc[-1], 1.561942801e+18)
