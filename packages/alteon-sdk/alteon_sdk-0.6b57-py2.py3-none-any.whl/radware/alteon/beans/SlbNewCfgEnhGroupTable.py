...
class EnumSlbGroupType(BaseBeanEnum):
    local = 0
    wanlink = 2
...
class EnumSlbGroupSecType(BaseBeanEnum):
    none = 1
    virtual = 2
    layer = 3
    passive = 4
...
class EnumSlbGroupSecDeviceFlag(BaseBeanEnum):
    none = 1
    security = 2
...
class SlbNewCfgEnhGroupTable(DeviceBean):
    def __init__(self, **kwargs):
        ...
        self.Type = EnumSlbGroupType.enum(kwargs.get('Type', None))
        self.Copy = kwargs.get('Copy', None)
        self.IdsChain = EnumSlbGroupIdsChain.enum(kwargs.get('IdsChain', None))
        self.SecType = EnumSlbGroupSecType.enum(kwargs.get('SecType', None))
        self.SecDeviceFlag = EnumSlbGroupSecDeviceFlag.enum(kwargs.get('SecDeviceFlag', None))
