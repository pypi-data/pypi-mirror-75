...
from mneGUI.plot_func_dialog_classes import *
...
class PlotWindow(QWidget):
    ...
    def populate_data_and_widgets(self, data, filename, data_num):
        if data_num == 1:
            ...
            self.picks_data_list[0] = data # set picks data as the data as it is as default, then overwrite later if
            # a subsection of channels are picked
        ...
        if len(self.data_list[0]) == 0:  # if the first data file has not been loaded
            ...
            for i in range(len(self.data_list)):
                try:
                    ...
                    self.select_chs_cbox.setCurrentIndex(0) # reset to "all channels"
                    return
    ...
    def update_channels_for_picks(self):
        ...
        for i in range(len(self.data_list)):
            if len(self.data_list[i]) != 0:
                ...
                try:
                    data = self.data_list[i][0]
                    self.picks_data_list[i] = data.load_data().copy().pick_types(meg=False, eeg=False, stim=False,
                                                                                 eog=False, include=self.picks)
                except ValueError:
                    QMessageBox.warning(self, "Channel Error", "Selected channels not present in both data files")
                    self.select_chs_cbox.setCurrentIndex(0)  # reset to "all channels"
                    return
    ...
    def toggle_enabled_plots(self):
        ...
        elif len(data_item1) != 0 and len(data_item2) != 0 and isinstance(data_item1[0], mne.BaseEpochs) and isinstance(data_item2[0], mne.BaseEpochs):
            self.topo_epoch_evoked_tab.setDisabled(False)
        else:
            self.topo_epoch_evoked_tab.setDisabled(True)
