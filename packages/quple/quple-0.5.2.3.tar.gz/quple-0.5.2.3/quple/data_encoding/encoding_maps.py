...
def second_moment(x: np.ndarray) -> float:
    ...
    coeff = cos_x[0] if len(x) == 1 else (reduce(lambda m, n: m + n, square_x)/len(x))**0.5*2-1
    return coeff   
...
def third_moment(x: np.ndarray) -> float:
    ...
    coeff = cos_x[0] if len(x) == 1 else (abs(reduce(lambda m, n: m + n, cube_x))/len(x))**(1/3.0)*2-1
    return coeff 
...
def exponential_squared_sum(x: np.ndarray) -> float:
    ...
    """
    square_x = [x_i**2 for x_i in x]
    coeff = cos_x[0] if len(x) == 1 else (np.e**(reduce(lambda m, n: m + n, square_x)/len(x))*2)/np.e-1
    return coeff         
...
def exponential_cube_sum(x: np.ndarray) -> float:
    ...
    """
    cube_x = [x_i**3 for x_i in x]
    coeff = cos_x[0] if len(x) == 1 else (np.e**(reduce(lambda m, n: m + n, cube_x)/len(x))*2)/np.e-1
    return coeff     