...
class VQC(tf.keras.Sequential):
    ...
    def compile(self, optimizer='adam', loss='binary_crossentropy', metrics=['binary_accuracy'], *args, **kwargs):
        '''
        self._properties['optimizer'] = optimizer.__name__ if optimizer else ''
        self._properties['loss'] = loss.__name__ if loss else ''
        if isinstance(metrics, (tuple, list)):
            self._properties['metrics'] = [metric.__name__ if metric else '' for metric in metrics]
        else:
            self._properties['metrics'] = metrics.__name if metrics else ''
        '''
    ...
    def encoding_circuit(self, val):
        symbols = quple.get_circuit_symbols(val)
        feature_dimension = len(symbols)
        ...
        self._properties['symbols'] = symbols
        self._properties['feature_dimension'] = feature_dimension
        self._properties['encoding_circuit'] = val.name if isinstance(val, quple.QuantumCircuit) else ''
        self._properties['encoding_map'] = val.encoding_map.__name__ if isinstance(val, quple.EncodingCircuit) else ''
        logger.info('Registered encoding circuit with feature dimension: {}'.format(feature_dimension))
        
    @property
    def symbols(self) -> List[str]:
        return self.properties['symbols']
    
    @property
    def num_parameters(self) -> int:
        return self.properties['num_parameters']
    
    @property
    def feature_dimension(self) -> int:
        return self.properties['feature_dimension']
    ...
    def variational_circuit(self, val):
        ...
        self._properties['num_parameters'] = num_parameters
        self._properties['variational_circuit'] = val.name if isinstance(val, quple.QuantumCircuit) else ''
    ...
    def differentiator(self, val):
        ...
        self._properties['differentiator'] = val.__name__ if val else ''
    ...
    def activation(self, val):
        ...
        self._properties['activation'] = val.__name__ if val else ''
    ...
    def fit(self, x, y,
        ...
        self._properties['batch_size'] = batch_size
        self._properties['epochs'] = epochs
