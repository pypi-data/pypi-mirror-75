...
from atp.api_server_run import ApiRun


def api_workspace(sys_name, data):
    """
    创建API自动化测试执行工作空间并自动/手动执行
    """
    data_dict = {
        'name': data['name'],
    ...
    cwd_dir = Path.cwd()
    work_space = cwd_dir / ('x_atp_api_server_%s' % data_dict['name'])
    ...
    if sys_name == 'Linux':
        ...
        print('###   您需要手动启动执行端服务   ###\ncd x_atp_api_server_%s\npython api_server_run.py\n'
              % data_dict['name'])
...
def create_profile(data, work_space, current_dir):
    ...
    # 拷贝api_server_run执行文件到工作空间
    with open(work_space / 'api_server_run.py', 'w', encoding='utf-8') as file_obj:
        with open(current_dir / 'api_server_run.py', 'r', encoding='utf-8') as api_run:
            file_obj.write(api_run.read())


def daemon_start(data, work_space, stdin='/dev/null', stdout='/dev/null', stderr='atp-log/x-atp-cli.error'):
    """
    启动守护进程
    """
    # fork出一个子进程，父进程退出
