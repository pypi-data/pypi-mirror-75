...
from common_read_write import write_data, read_data
...
class ServerRun:
    ...
    @staticmethod
    def write_test_data_and_run(test_name, test_type):
        """写入测试所需数据并执行测试"""
        ...
        if sys_name == 'Linux':
            # 启动run_case模块
            os.system('nohup python3 ./exec_testcase.py &')
        else:
            os.popen('start python ./exec_testcase.py')
        # 隐式等待/避免执行端假死
