...
from atp.exec_testcase import exec_testcase
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                if length > 0:
                    print('[%s Server Run Info][' + localtime + ']> ' + '发现新的任务信息' % self.TEST_TYPE)
                    self.pull_case()
                    ...
                    try:
                        ...
                        # 执行sweetest，并返回执行结果
                        self.exec_status = exec_testcase(test_name=value_dict['test_name'], test_type=self.TEST_TYPE)
                        # 执行成功
                        if self.exec_status is True:
                        ...
                        # 执行失败上报心跳
                            ...
                            self.post_heartbeat(status=2, localtime=localtime, task_dict=value_dict,
                                                remark=str(self.exec_status))
                            print('[%s Server Run Error]> ' + str(self.exec_status) % self.TEST_TYPE)
                    except Exception as exc:
                        # 上报任务无法完成的心跳
