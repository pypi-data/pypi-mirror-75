...
class LiveClient(WebSocketApp):
    ...
    def check_heartbeat(self):
        while True:
            ...
            if time_diff.total_seconds() > ((self.heartbeat_interval + self.heartbeat_buffer) * self.confirm_heartbeats):  # 3 > 5 + 0.5
    ...
    def reconnect(self):
        ...
        while not t.is_alive():
            time.sleep(1)
        time_of_reconnection = datetime.now()
        self.last_n_heartbeat = self.last_n_heartbeat[1:]
        self.last_n_heartbeat.append(time_of_reconnection)
        return t.is_alive()
# I got the error... Because these 2 commands are fired so close to each other... This message is coming after addsymbols request has gone...
# That's the difference between on_open and first message sent after open...
