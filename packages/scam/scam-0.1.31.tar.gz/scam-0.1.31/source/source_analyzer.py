...
""")
        ...
        self.hm_file = open("../manual.txt", 'r', encoding="utf-8").read()
...
#Main Class
    ...
    #Tools Menu
        ...
        for dirpath, dirs, files in os.walk('./'):  
            for filename in files: 
                manpath = os.path.join(dirpath,filename) 
                if manpath.endswith('manual.txt'): 
                    print(manpath)
                    self.manualfound=True
                if self.manualfound:
                    break
    ...
    #Button Panel
        ...
        for dirpath, dirs, files in os.walk('./'):  
            for filename in files: 
                scampath = os.path.join(dirpath,filename) 
                if scampath.endswith('SCAM.png'): 
                    print(scampath)
                    logonotfound = True
                if logofound:
                    break
        
        logo = tk.PhotoImage(file="../SCAM.png")
