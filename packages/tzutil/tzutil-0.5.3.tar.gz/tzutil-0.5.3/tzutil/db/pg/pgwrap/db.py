...
class SafeNamedTupleCursor(NamedTupleCursor):
    ...
    def _make_nt(self, namedtuple=namedtuple):
        return namedtuple(
            "Record", [d[0] for d in self.description or ()], rename=True
        )
...
class connection(object):
    ...
    def __init__(
        ...
        params = urllib.parse.urlparse(
            url or os.environ.get('DATABASE_URL') or 'postgres://localhost/'
        )
    ...
    def cursor(self, cursor_factory=None):
        return cursor(
            self.pool, cursor_factory or self.default_cursor, self.hstore,
            self.log, self.logf
        )
...
class cursor(object):
    ...
    def join(
        ...
        """
        return self.query(
            self._build_join(tables, where, on, order, columns, limit, offset),
            where
        )
...
if __name__ == '__main__':
    ...
    if sys.argv.count('--interact'):
        ...
        try:
            ...
            for i in range(10):
                id = db.insert(
                    'doctest_t1', {'name': chr(97 + i) * 5}, returning='id'
                )['id']
                _ = db.insert(
                    'doctest_t2',
                    {
