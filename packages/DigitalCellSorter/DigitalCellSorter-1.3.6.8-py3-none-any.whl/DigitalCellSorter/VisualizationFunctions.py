...
class VisualizationFunctions:
    ...
    @tryExcept
    def makeHeatmapGeneExpressionPlot(self, df = None, genes = None, normalize = True, subtract = False, saveExcel = True, nameToAppend = 'heatmap', plotBy = 'cluster', figsize = (8, 4), convertGenes = False, orderGenes=False, orderClusters=False, dpi = 300, extension = 'png', fontsize = 10, labelsFontsize = 10, **kwargs):
        ...
        if df is None:
            ...
            if type(genes) in [list, np.ndarray, tuple]:
                common = pd.Index(genes).intersection(self.df_expr.index)
            elif type(genes) in [dict]:
                common = []
                for key in genes.keys():
                    temp_common = pd.Index(genes[key]).intersection(self.df_expr.index)
            ...
            if type(genes) in [list, np.ndarray, tuple]:
                common = pd.Index(genes).intersection(df.index)
            elif type(genes) in [dict]:
                common = []
                for key in genes.keys():
                    temp_common = pd.Index(genes[key]).intersection(self.df_expr.index)
