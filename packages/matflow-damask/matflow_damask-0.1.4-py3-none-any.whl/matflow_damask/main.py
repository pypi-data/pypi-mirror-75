...
@input_mapper(
    input_file='orientations_coordinate_system.txt',
    task='generate_microstructure_seeds',
    method='random'
)
...
def write_orientations_coordinate_system_from_seeds(path, microstructure_seeds):
    write_orientations_coordinate_system(
        path,
        microstructure_seeds['orientations_coordinate_system']
    )


@input_mapper(
    input_file='model_coordinate_system.txt',
    task='generate_volume_element',
    method='random_voronoi'
)
@input_mapper(
...
def read_model_coordinate_system(path):
    with Path(path).open('r') as handle:
        return json.load(handle)


@output_mapper('microstructure_seeds', 'generate_microstructure_seeds', 'random')
def read_seeds_from_random(seeds_path, orientations_coord_sys_path=None):
    ...
    if orientations_coord_sys_path:
        ...
        out.update({'orientations_coordinate_system': ori_coord_sys})
...
def read_damask_geom(geom_path, orientations_coord_sys_path=None, model_coord_sys_path=None):
    volume_element = read_geom(geom_path)
    if orientations_coord_sys_path:
        ori_coord_sys = read_orientations_coordinate_system(orientations_coord_sys_path)
        volume_element.update({'orientations_coordinate_system': ori_coord_sys})
    if model_coord_sys_path:
        mod_coord_sys = read_model_coordinate_system(model_coord_sys_path)
        volume_element.update({'model_coordinate_system': mod_coord_sys})
