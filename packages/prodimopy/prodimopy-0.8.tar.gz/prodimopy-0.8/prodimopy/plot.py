...
#    ax.plot(model.x[:,0],model.z[:,35],color="black")
  ...
  def plot_radial(self, model, values, ylabel, zidx=0, ax=None,**kwargs):
    '''
  ...
  def plot_abuncont(self, model, species='O', rel2H=True, label=None, zlog=True, 
    ...
    _todo: can be improved with better and smarter default values (e.g. for the colorbar)    
    '''
  ...
  def plot_abuncont_grid(self, model, speciesList=['e-','H2','CO',"H2O"], nrows=2, ncols=2, rel2H=True, label=None, zlog=True, 
    ...
    for species in speciesList:
      values,labelG,zlogG,zlimG,extendG=self._prepareAbunForPlot(model, species, label,rel2H, zlog, zlim,extend)      
  ...
  def plot_reac(self, model,chemistry,rtype, plot_size=10, lograte=True, grid=False, with_abun=False,vmin=-100,  **kwargs):
    '''
    ...
    The routine checks if the reaction type is set, checks if the species exists,
    ...
    Contributors: L. Klarmann, Ch. Rab, G. Chaparro
    ...
    TODO: possibility to pass Axes object, like in all the other plots.
    '''
    print('PLOT: plot_reac ...')

    # Checks whether rtype is set
    try:
      assert((rtype=='d')|(rtype=='f'))
    except:
      print('Error: rtype is not set or it is set to something other than \'d\' (destruction) or \'f\' (formation)')
      return

    # Check if species name exists
...
#     ax2.set_xticklabels(["{:.2f}".format(x) for x in nhver_to_zr(ix, ax.get_xticks(), model)])
  ...
  def plot_dust_opac(self,model,dust=None,ax=None,**kwargs):
    '''
  ...
  def plot_starspec(self, model,ax=None,**kwargs): 
    '''
...
#    ax.yaxis.set_label_position("right")
  ...
  def plot_line_origin(self,model,lineIdents,field, label="value", boxcolors=None, showBox=True,
                       boxlinestyles=None,boxhatches=None,lineLabels=None,showLineLabels=True, zlog=True, 
    ...
    TODO: possibility to pass Axes object like in most other plots.
    '''
    if boxcolors is None:
    ...
    if showLineLabels:
      ...
      for idline in lineIdents:
        ...
        ax.text(0.02, 0.92-ibox/18.0,label,
                horizontalalignment='left',
                verticalalignment='bottom',fontsize=6.5,
                transform = ax.transAxes,color=boxcolors[ibox],
                bbox=dict(boxstyle='square,pad=0.1', fc='white', ec='none'))
        ibox+=1
  ...
  def plot_lineprofile(self,model,wl,ident=None,linetxt=None,lineObs=None,ax=None,**kwargs):
    '''
