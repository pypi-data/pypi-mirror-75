...
class SimpleNetworkEmbedding:
    ...
    def train(self, X: np.ndarray, y: np.ndarray):
        ...
        print("{} iter: Train RMSE = {}, Validation RMSE = {}".format(iteration, c[0], None if len(c) == 1 else c[1]))
        self.model.zero_grad()
        self.criterion.zero_grad()
        ...
        #         if not np.isfinite(self.current_loss[-1]):
        #             return self
        #         # Log stats
        #         if iteration % self.log_loss == 0:
        #             print('iteration [{}/{}], Step [{}/{}], Avg.Loss: {:.4f}'
        #                   .format(iteration, self.iterations, iter + 1,
        #                           len(train_loader), np.mean(self.current_loss[-1000:])))
        #             self.loss_curve.append(np.mean(self.current_loss[-self.log_loss:]))
        #             if len(self.loss_curve) > 1:
        #                 if self.loss_curve[-1] >= self.loss_curve[-2] * 0.99:
        ...
        #                     return self
        #             if (
        #                 len(self.loss_curve) > 1 and self.loss_curve[1] > 1
        #                 or len(self.loss_curve) > 5 and self.loss_curve[5] > 0
        #             ):
        #                 print('Stopping training')
        #                 c = [self._eval(X=X, y=y), ]
        #                 print("{} iter: Train RMSE = {}, Validation RMSE = {}".format(iteration, c[0],
...
class NeuralNet(nn.Module):
    ...
    def initialize_weights(self, X):
        ...
        for i in range(len(self.weights)):
            ...
            self.weights[i].weight.data = self.weights[i].weight.data / std.reshape((-1, 1))
            self.weights[i].bias.data = -mean / std
        torch.nn.init.xavier_normal_(self.outlayer.weight, torch.nn.init.calculate_gain('tanh'))
        self.outlayer.bias.data.fill_(0)
        output = self.outlayer.forward(output)
        mean = output.mean(axis=0)
        std = output.std(axis=0)
        self.outlayer.weight.data = self.outlayer.weight.data / std.reshape((-1, 1))
        self.outlayer.bias.data = -mean / std
