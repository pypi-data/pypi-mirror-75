...
class MDG(Tk):
    ...
    def __build(self):
        ...
        settings_dir_entry = Entry(new_settings_frame, textvariable=self.__local_db_dir)
        settings_dir_label.grid(row=1, column=0, padx=4, pady=5)
        settings_dir_entry.grid(row=1, column=1, padx=4, pady=5)

        #   Apply Settings Directory Finder Button Widget
        find_settings_dir_button = Button(new_settings_frame, text='...', width=8, command=self.__local_dir_gui)
        ...
        settings_fp_entry = Entry(existing_main_frame, textvariable=self.__local_db_path)
