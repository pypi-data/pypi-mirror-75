...
class TBoxBase(object):
    ...
    def local_config_dir(self):
        ...
        if self.__pointer and 'Local_DB_Path' in pointer.keys():
            return dirname(self.__pointer['Local_DB_Path'].decrypt())
        else:
            return None
...
class Toolbox(TBoxBase, SQLQueue, LogHandle):
    ...
    def __init__(self, script_path, logging_dir=None, logging_base_name=None, max_pool_size=None, *args, **kwargs):
        ...
        if logging_dir and os.path.exists(logging_dir) and os.path.isdir(logging_dir):
            ...
            file_dir = self.local_config_dir
