...
class API(PostgreSql):
    def __init__(self, tables, db_host: str = None):
        self._active = False
    ...
    def add_field(self, table_name: str, field_name: str, start_value) -> str:
        ...
        self._cursor.execute(
            "ALTER TABLE %s ADD COLUMN %s%s" % (
                table_name,
                field_name,
                table.get_types()[f'{field_name}'][0]
            ))
        self._cursor.execute(
            "UPDATE %s SET %s = %s" % (
                table_name,
    ...
    def create_db(self, tables: Dict[str, Table] = None) -> str:
        """
        Создание таблиц.
        :param tables: Таблицы которые нужно создать.
        :return: "Successfully"
        """
        ...
        for table_name, table in tables.items():
            ...
            self._cursor.execute(
                "CREATE TABLE %s "
                "(id SERIAL PRIMARY KEY NOT NULL, %s)" % (
                    table_name,
                    ', '.join(fields)
                ))
            self.commit()
    ...
    def _check_active(self):
        if not self._active:
            raise PostgreSqlApiError('Хост базы данных не инициализирован')

    def _get_table(self, table_name):
        if table_name in self._tables_names:
            return self._tables[table_name]()
        else:
            raise PostgreSqlApiError(f'Таблица `{table_name}` не найдена')
