...
class scrape(object):
    ...
    def newspaper_stories(words, search_type='or', search_level=0, urls=None, 
        ...
        if search_level==0:
            print("\nSearch Level 0: Quick search of URLs for:", words)
        else:
            print("\nSearch Level 1: ",
                  "Deep download and search of all qualified articles for:",
                  words)
        print("")   
        ...
        for agency, url in news_urls.items():
            ...
            if display:
                print("{:>6d} Articles available from {:<s} on {:<10s}:".
            ...
            for word in words:
                ...
                for article in paper.articles:
                    ...
                    # search_level 0 only searchs the url for keywords
                    # search_level 0 only downloads articles with a 
                    # keyword in their url.
                    if search_level == 0 and url_lower.find(word) < 0:
                        continue
            ...
            if display:
                print("{:>6d} Selected for download and review. Note that".
                      format(n_to_review))
                print("       download requests can fail, and multiple")
                print("       copies of the same article are common.")
                print("       These are filtered and saved as a single copy.")
            ...
            for article_url in article_collection:
                ...
                while n<2:
                    try:
                        ...
                        stop_sec = stop_sec+1
                if n != 99:
                    print("\nCannot download:", article_url[0:79])
                    n_to_review -= 1
                    continue
                article.nlp()
        ...
        if display:
            ...
            for agency in news_urls:
                ...
                print(ratio)
            print("\nTotal Articles Collected on "+today+":", 
                      df_articles.shape[0],'from', 
                      df_articles['agency'].nunique(), "News Agencies.")
        return df_articles
