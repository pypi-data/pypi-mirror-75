...
class SosBackupsCronScan(object): # pylint: disable-msg=useless-object-inheritance
    def __init__(self, name, paths, options = None, notifiers = None):
        ...
        if 'plugins' in self.options:
            ...
            if isinstance(plugins, six.string_types):
                ...
                for plug_name, plug_opts in six.iteritems(plugins):
                    if not plug_opts:
                        continue
    ...
    def _add_stats(self, key, stats):
        for xtype, values in six.iteritems(self.stats[key]):
            if xtype not in stats:
                continue
            for x in values.iterkeys():
                if x in stats[xtype]:
    ...
    def clean_events(self, wait = False):
        ...
        for event in self.events:
            ...
            if not event.plugs_flag.is_set():
                continue
            for plugin, state in six.iteritems(event.state):
                if state.get('errors'):
    ...
    def run(self, xpath):
        ...
        for plug_opts in six.itervalues(plugins):
...
class SosBackupsCronSyncThread(threading.Thread):
    ...
    def run(self):
        while not _KILLED:
            for tab, tasks in six.iteritems(self.tabs):
                ...
                for params in tasks:
                    ...
                    name    = "%s:%s" % (tab, hashlib.sha1(six.ensure_binary(':'.join(paths))).hexdigest())
