...
from toolkit_w.internal.vars_global import ProblemType, TargetMetric
from toolkit_w.internal.vars_global import InterpretabilityLevel as Interpt
from api_clients.hal_client import HalClient
from api_clients.data_clients import DatasetClient
from api_clients.reports_client import ReportsClient
from api_clients.prediction_client import PredictionClient
from api_clients.ensembles_client import EnsemblesClient
from api_clients.um_clients import UMClient
...
#############################
# conf_server = {
#     "USERM_HOST": "user-management.prod.firefly.ai",
#     "USERM_PORT": 5007,
#     "DATASET_HOST": "data-service.prod.firefly.ai",
#     "DATASET_PORT": 5001,
#     "PREDICTION_HOST": "prediction-manager.prod.firefly.ai",
#     "PREDICTION_PORT": 5017,
#     "HAL_HOST": "task-manager.prod.firefly.ai",
#     "HAL_PORT": 5013,
#     "REPORT_HOST": "reports.prod.firefly.ai",
#     "REPORT_PORT": 5015,
#     "ENSEMBLES_HOST": "ensembles.prod.firefly.ai",
#     "ENSEMBLES_PORT": 80,
#     "TEMPLATES_HOST": "templates.prod.firefly.ai",
#     "TEMPLATES_PORT": 5011
# }
...
class MyWhatify:
    ...
    def __init__(self):
        # self.auth = authenticate email, password
        # self.halClient = HalClient(host=conf_server['HAL_HOST'], port=conf_server['HAL_PORT'], use_https=True)
        # self.predClient = PredictionClient(host=conf_server['PREDICTION_HOST'], port=conf_server['PREDICTION_PORT'],
        #                               use_https=True)
        # self.umClient = UMClient(host=conf_server['USERM_HOST'], port=conf_server['USERM_PORT']),  # use_https=True)
        # self.dataset_client = DatasetClient(host=conf_server['DATASET_HOST'], port=conf_server['DATASET_PORT'],
        #                                use_https=True)
        # self.report_client = ReportsClient(host=conf_server['REPORT_HOST'], port=conf_server['REPORT_PORT'], use_https=True)
        # self.ensembles_client = EnsemblesClient(host=conf_server['ENSEMBLES_HOST'], port=conf_server['ENSEMBLES_PORT'])
        pass
    ...
    def login(self, email, password):
        with allure.step('login with user: ' + str(email)):
            ...
            print("{} - Login successful".format(str(email)))

    # def signup(self, user, password):
    #     with allure.step('login with user: ' + user):
    #         logging.info('login with user: ' + user)  # + ' and password: ' + password)
    #         firefly.signup(username=user, password=password)
    ...
    def preparing_data_set(self, source_id, data_set_name, problem_type=ProblemType.CLASSIFICATION, target='class', wait=True):
        with allure.step('Preparing Dataset name: ' + str(data_set_name) + ' source_id= ' + str(source_id) +
            ...
            try:
                dataset_id = my_toolkit.resources.Datasource.prepare_data(
                    ...
                    sample_id=['car_id'],
                    retype_columns={'car_id': my_toolkit.vars_global.FeatureType.CATEGORICAL},
        ...
        with allure.step('On data_name: ' + data_name + ' ID: ' + str(dataset_id) +
            ...
            try:
                task = my_toolkit.Dataset.train(
                    ...
                    pipeline=pipeline,
                    target_metric=target_metric_eval,
    ...
    def get_tasks_list(self, len=10):
        with allure.step('getting the tasks list with len: ' + str(len)):
            list_tasks = my_toolkit.Task.list()
            tasks = pd.DataFrame(list_tasks['hits'])

            tasks.columns
        return tasks[['creation_date', 'dataset_id', 'name', 'notes', 'problem_type',
               'state', 'target_metric']].head(len)
