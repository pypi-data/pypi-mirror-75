...
class VisaSession(object):
	...
	def __init__(self, resource_name: str, settings: InstrumentSettings, direct_session=None):
		...
		if self.reusing_session:
			# Reuse the session
			assert isinstance(direct_session, visa.Resource), f"direct_session must be a VISA resource object. Actual type: '{type(visa.Resource)}', value: '{visa.Resource}'"
			...
			self.manufacturer = self._rm.visalib.get_attribute(self._rm.session, visa.constants.VI_ATTR_RSRC_MANF_NAME)[0]
			try:
				self._session = self._rm.open_resource(resource_name)
			except visa.VisaIOError as e:
				if e.error_code != visa.constants.StatusCode.error_resource_not_found:
					raise e
				message = e.description
				message += f"\nLibrary: {self._rm.visalib}\nManufacturer: {self.manufacturer}\nResource Name: '{resource_name}'"
				raise InstrumentErrors.ResourceError(resource_name, message)

			self.manufacturer = self._session.resource_manufacturer_name
			self.resource_name = resource_name

		self._interface_type = SessionKind.unsupported
		if self._session.interface_type == visa.constants.InterfaceType.gpib or self._session.interface_type == visa.constants.InterfaceType.gpib:
			self._interface_type = SessionKind.gpib

		elif self._session.interface_type == visa.constants.InterfaceType.rsnrp:
			self._interface_type = SessionKind.rs_nrp

		elif self._session.interface_type == visa.constants.InterfaceType.asrl:
			self._interface_type = SessionKind.serial

		elif self._session.interface_type == visa.constants.InterfaceType.usb:
			# Check whether it is not the NRP-Z
			intf_type = self._session.get_visa_attribute(visa.constants.VI_ATTR_INTF_TYPE)
			if intf_type == visa.constants.InterfaceType.rsnrp:
				self._interface_type = SessionKind.rs_nrp

		elif self._session.interface_type == visa.constants.InterfaceType.tcpip:
		...
		self._assure_write_with_lf = settings.assure_write_with_lf
		...
		if self._interface_type == SessionKind.serial:
			...
			self._assure_write_with_lf = True
	...
	def _narrow_down_io_tout_error(self, context: str) -> None:
		...
		if stb & StatusByte.error_queue_not_empty:
			InstrumentErrors.assert_no_instrument_status_errors(self.resource_name, self.query_all_syst_errors(), context + " and ...")
	...
	def clear_before_read(self) -> None:
		...
		while stb & condition:
			...
			stb = self._query_stb()
			if stb == previous_stb:
				raise Exception(f"Cannot clear instrument's status subsystem. Status Byte: '{stb}'")
	...
	def write(self, cmd: str) -> None:
		...
		add_lf = False
		if self._assure_write_with_lf and not cmd.endswith('\n'):
			add_lf = True
		if add_lf:
	...
	def _read_unknown_len(self, stream: StreamWriter, allow_chunk_events: bool, prepend_data: AnyStr = None) -> None:
		...
		The read is performed in an incremental chunk steps to optimize memory use:
		...
		with self._session.ignore_warning(visa.constants.StatusCode.success_max_count_read):
			if prepend_data and isinstance(prepend_data, str):
				prepend_data = prepend_data.encode('utf-8')
			chunk_ix = 0
			continue_status = visa.constants.StatusCode.success_max_count_read
			eot = False
			while not eot:
				if chunk_ix == 0:
				...
				chunk, status = self._session.visalib.read(self._session.session, chunk_size)
				if chunk_ix == 0 and prepend_data:
					chunk = prepend_data + chunk
				eot = status != continue_status
	...
	def _query_str_no_events(self, query: str) -> str:
		...
		try:
			...
			self._narrow_down_io_tout_error("Querying '{}' - ".format(query.rstrip('\n')))
		return response
	...
	def _check_msg_available_after_opc_wait(self, stb: StatusByte, query: str, timeout: int, context: str) -> None:
		...
		context = context + " SCPI query '{}'".format(query.rstrip('\n'))
		if stb & StatusByte.error_queue_not_empty:
	...
	def _parse_bin_data_header(self, exc_if_not_bin: bool) -> Tuple[ReadDataType, str, int]:
		...
		if exc_if_not_bin:
			if data_type == ReadDataType.null:
				InstrumentErrors.throw_bin_block_unexp_resp_exception(self.resource_name, '\n')
			# Read 20 more characters to compose a better exception message
			whole_hdr += self.read_up_to_char(b'\n', 20).decode("utf-8")
			if self._session.last_status == visa.constants.StatusCode.success_max_count_read:
				self._flush_junk_data()
	...
	def read_bin_block(self, stream: StreamWriter, exc_if_not_bin: bool) -> None:
		...
		if data_type == ReadDataType.ascii:
			...
			stream.switch_to_string_data()
		if data_type == ReadDataType.bin_unknown_len:
			if not self.vxi_capable:
	...
	def query_bin_block(self, query: str, stream: StreamWriter, exc_if_not_bin: bool = True) -> None:
		...
		self.write(query)
		return self.read_bin_block(stream, exc_if_not_bin)
