...
def compose_cmd_string_from_struct_args(args: Dict[int, ArgStruct], composer: StructComposer, values_obj: object) -> str:
	...
	if opt_null_ix >= 0:
		...
		if len(rest):
			msg = f"Optional Argument '{args[opt_null_ix].name}' has no value, but the further ones do. You can not skip only " \
				f"some optional arguments. Clear the values for the rest of the argument(s):\n{', '.join(rest)}"
			raise Exception(msg)
