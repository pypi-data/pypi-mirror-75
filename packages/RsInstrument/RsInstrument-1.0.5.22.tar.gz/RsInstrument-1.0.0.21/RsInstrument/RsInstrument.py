...
class RsInstrument:
	_driver_version_const = '1.0.0.21'
	_driver_options_const = "SupportedInstrModels = All Rohde & Schwarz Instruments, SupportedIdnPatterns = Rohde(-|&)Schwarz, SimulationIdnString = Rohde&Schwarz*SimulationDevice*100001*" + _driver_version_const

	def __init__(
			self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		"""Initializes new RsCmxNrLteSig session. \n
		For cleaner code, use the RsCmxNrLteSig.from_existing_session(existing_session_obj)
		Parameter options tokens examples:
			...
			- 'DriverSetup=(AssureWriteWithLf = True)' - Makes sure each command/query is terminated with linefeed character. Default: Interface dependent
	...
	def _add_all_global_repcaps(self) -> None:
		...
		self._core.io.add_global_repcap('<Instance>', 1)
