...
class VisaSession(object):
	...
	def __init__(self, resource_name: str, settings: InstrumentSettings, direct_session=None):
		...
		if self.reusing_session:
			# Reuse the session
			assert isinstance(direct_session, visa.Resource), f"Direct_session must be a VISA resource object. Actual type: '{type(direct_session)}', value: '{direct_session}'"
	...
	def _parse_bin_data_header(self, exc_if_not_bin: bool) -> Tuple[ReadDataType, str, int]:
		...
		if exc_if_not_bin:
			if data_type == ReadDataType.null:
				InstrumentErrors.throw_bin_block_unexp_resp_exception(self._resource_name, '\n')
