...
class InstrumentSettings(object):
	...
	def __init__(
		...
		self.assure_write_with_lf = False
	...
	def apply_option_settings(self, settings: dict) -> None:
		...
		if value is not None:
			...
			if value == 'STBPOLLING':
				...
				raise ValueError(
					"Unknown value in InitWithOptions string DriverSetup key 'WaitForOPC'. Value 'value' is not recognized. "
		...
		if value is not None:
			self.assure_write_with_lf = Conv.str_to_bool(value)
