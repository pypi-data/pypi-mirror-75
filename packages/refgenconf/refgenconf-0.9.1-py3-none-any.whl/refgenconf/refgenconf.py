...
class RefGenConf(YacAttMap):
    ...
    def list(self, genome=None, order=None, include_tags=False):
        ...
        refgens = _select_genomes(sorted(list(self[CFG_GENOMES_KEY].keys()), key=order), genome)
        if include_tags:
    ...
    def get_default_tag(self, genome, asset, use_existing=True):
        ...
        try:
            ...
            alt = list(self[CFG_GENOMES_KEY][genome][CFG_ASSETS_KEY][asset][CFG_ASSET_TAGS_KEY].keys())[0] if use_existing\
                else DEFAULT_TAG
    ...
    def list_genomes_by_asset(self, asset=None, order=None):
        ...
        return self._invert_genomes(order) if not asset else \
            sorted([g for g, data in list(self[CFG_GENOMES_KEY].items())
    ...
    def get_local_data_str(self, genome=None, order=None):
        ...
        genomes_str = self.genomes_str(order=order) if genome is None \
            else ", ".join(_select_genomes(sorted(list(self[CFG_GENOMES_KEY].keys()), key=order), genome))
    ...
    def _invert_genomes(self, order=None):
        ...
        """
        genomes = {}
        for g, am in list(self[CFG_GENOMES_KEY].items()):
            for a in list(am[CFG_ASSETS_KEY].keys()):
                genomes.setdefault(a, []).append(g)
        assets = sorted(list(genomes.keys()), key=order)
    ...
    def run_plugins(self, hook):
        ...
        """
        for name, func in list(self.plugins[hook].items()):
...
def _list_remote(url, genome, order=None, as_str=True):
    ...
    refgens = _select_genomes(sorted(list(genomes_data.keys()), key=order), genome,
                              strict=True)
    if not refgens:
    ...
    asset_texts = ["{}/   {}".format(g.rjust(20), ", ".join(a))
                   for g, a in list(filtered_genomes_data.items())]
    return ", ".join(refgens), "\n".join(asset_texts)
...
def _make_asset_tags_product(assets, asset_tag_delim=":", asset_sk_delim="."):
    ...
    """
    tagged_assets = []
    for aname, asset in list(assets.items()):
        for tname, tag in list(asset[CFG_ASSET_TAGS_KEY].items()):
            sk_assets = []
...
def map_paths_by_id(json_dict):
    ...
    return {values["get"]["operationId"]: endpoint for endpoint, values in list(json_dict["paths"].items())}
