...
def qjson(x): return json.loads(json.dumps(x))
...
class DragonGather(TaskScheduler):
    ...
    def __init__(self, queue_list, *args, **kwargs):
        ...
        self.log_path = home_path(kwargs.get('log_path', '~/log'))
    ...
    def msg_network_task(self):
        ...
        try:
            ...
            task_client = loop.create_task(
                coromask(
                    self.check_client_status,
                    args, {},
                    simple_fargs_out)
            )
            task_client.add_done_callback(
                functools.partial(
                    renew,
                    task_client,
                    self.check_client_status,
    ...
    def add_sta_instance_origin(self, ids, loop):
        ...
        db_datos = dict(host='localhost',
                        ...
                        io_loop=loop,
                        log_path=self.log_path/"source_rdb",
                        env='gather_%s' % code)
        name = 'RethinkDB'
    ...
    def add_sta_instance_destiny(self, ids, loop):
        ...
        db_datos_enu = dict(host='localhost',
                            ...
                            dbname='enu_data',
                            io_loop=loop,
                            log_path=self.log_path/"enu_rdb",
                            env='enu_%s' % code)
    ...
    def add_process_instance(self, ids):
        ...
        kwargs['log_path'] = self.log_path/'geo_json_data'
    ...
    async def gather_data(self, ipt, ids, *args, **kwargs):
        ...
        print("====Datawork Gather Iteration===")
        bprint("idi %s, args %s" % (idi, args))
        rprint("idi %s, kwargs %s" % (idi, kwargs))
        await asyncio.sleep(.5)
        loop = asyncio.get_event_loop()
        log = kwargs.get('log')
        v = int(args[0])
        ...
        if not self.rethinkdb_origin.get(ids):
            ...
            try:
                ...
                log.exception(
                    "PD_TO1 IDEX %s: Tiempo fuera para conectar db source %s" % rc)
        ...
        if not self.rethinkdb_enu_origin.get(ids):
            ...
            try:
                ...
                idex = my_random_string(uin)
                log.exception(
                    '''PD_TO2 IDEX %s: Tiempo fuera para conectar
                    db destiny %s''' % (idex, e))
        ...
        if body or origin or destiny:
            ...
            try:
                ...
                future = rc.get_data_filter(table_name,
                                            ...
                                            filter_opt,
                                            key)
                cursor = await shield(wait_for(future, timeout=30))
            except asyncio.TimeoutError as e:
                idex = my_random_string(uin)
                log.exception(
                    '''PD_TO3 IDEX %s: Tiempo fuera para conectar db % s,
                    al leer datos desde db, tabla %s''' % (
                        idex, e, table_name))
                ...
                del rc
                rc = None
                log.exception("PD_TO3 IDEX %s: next iteration" %
                              (idex, table_name))
                wargs = [ipt, ids, v, [
            ...
            for c in cursor:
                if check_gsof(c):
                    try:
                        ...
                        result_enu = await rc_enu.save_data(table_name, to_db)
                        # send normal json
                        send = {
                            'channel': 'earlybird',
                            'command': 'add_data',
                        ...
                        log.exception(
                            f"PD_TO3: Tiempo fuera para conectar db {e}, al" +
                            "leer datos desde db")
                        kwargs["origin_exception"] = "PD_TO5_01 + %s" % code
                        self.rethinkdb_origin_enu[ids] = False
                        rc_enu.close()
                        del rc_enu
                        rc_enu = None
    ...
    async def status_gather_task(self, ids, loop, wargs_list, **kwargs):
        ...
        bprint("Check on status task if assigned")
        value = self.status_tasks[ids]
        rprint(f"IDS {ids} -> {value}")
        if self.status_tasks[ids]:
            try:
                wargs = [ids, loop, wargs_list]
                result, kwargs = await self.gather_data(
                    *wargs, **kwargs)
                ...
                raise e
        return [ids, loop, wargs_list], kwargs

    async def gather_process(self, ipt, loop, ipt_wargs_dict, **kwargs):
        result_dict = {}
        ids_list = self.proc_tasks[ipt]
        for ids in ids_list:
            if ids not in ipt_wargs_dict.keys():
                wargs = [None, None, None, None]
                ipt_wargs_dict.update({ids: wargs})
            wargs_dict = [ids, loop, ipt_wargs_dict[ids]]
            result, kwargs = await self.status_gather_task(
                *wargs_dict, **kwargs)
            result_dict.update({ids: result[2]})
        return [ipt, loop, result_dict], kwargs
    ...
    def set_init_args_kwargs(self, ipt):
        ...
        """
        log = FileLog("Gather@Datawork", "CORE_%s" %
