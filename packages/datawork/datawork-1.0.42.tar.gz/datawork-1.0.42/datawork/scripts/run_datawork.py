...
def get_conf_data(conf):
    keys = ['DATAWORK_EST_X_PROC', 'DATAWORK_TSLEEP',
            'DATAWORK_WORKERS', 'DW_STATUS', 'DW_GROUP',
            'SELECT_GROUP', 'COLLECTOR_SOCKET_IP', 'COLLECTOR_SOCKET_PORT',
            'DATAWORK_SOCKET_IP', 'DATAWORK_SOCKET_PORT', 'RDB_SOURCE_HOST',
            'RDB_SOURCE_PORT', 'RDB_ENU_HOST', 'RDB_ENU_PORT', 'MQ_HOST',
            'MQ_NAME', 'MQ_PASS', 'MQ_VHOST', 'MQ_CODE', 'MQ_QUEUE_NAME']
...
def start_datawork(env, conf="collector_example.json"):
    ...
    with concurrent.futures.ProcessPoolExecutor(workers) as executor:
        ...
        server_socket = GNCSocketServer(
            ...
            log_path=str(log_path/"socket_server"))
        client_socket = GNCSocketClient(
            queue_client_n2t,
            queue_client_t2n,
            address=collector_address,
            log_path=str(log_path/"socket_client"))
        ...
        kwargs.update({
            ...
            'rdb_address': tuple(map(data.get, (
                "RDB_SOURCE_HOST", "RDB_SOURCE_PORT"))),
            'rdb_enu_address': tuple(map(data.get, (
                "RDB_ENU_HOST", "RDB_ENU_PORT"))),
            'rdb_dbname': rdb_dbname,
            'chart': 'gnss_graph',
            'gui_set': gui_set,
            'gui_queue': gui_queue,
...
@click.command()
@click.option("--name", default="datawork", show_default=True,
              help="Nombre de la instancia datawork a crear")
@click.option("--env_vars/--no-env_vars", default=True, show_default=True,
              type=bool, help="Para mostrar el nombre de las variables de ambiente")
@click.option("--env/--no-env", default=True, show_default=True,
              type=bool, required=True, help="Si obtener los datos de ambiente o cargarlos de un json o data entregada")
@click.option("--conf", default="JSON FILE",  show_default=True,
