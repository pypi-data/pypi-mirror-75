...
def loadDatasetToPandas(dataset, from_cloud=False, delimiter=',', convertDatetime=True, datetime_format='%Y-%m-%dT%H:%M:%S.000Z', 
    key='../../service_account_keys/humai-sb-7b9db70de787.json'):
    print("OBSOLETE, please use humailib.cloud_tools.GoogleBigQuery.download_table_to_pandas(dataset_table)")
    return


def uploadDataframeToBQ(dataframe, dataset_name, project='humai-sb', key='../../service_account_keys/humai-sb-7b9db70de787.json'):
    print("OBSOLETE, please use humailib.cloud_tools.GoogleBigQuery.upload_and_replace_table(df, dataset_table)")
    return
...
class GoogleCloudStorage:
    ...
    def __init__(self, key='../../service_account_keys/humai-sb-7b9db70de787.json', cache_dir='./cache'):
    ...
    def __get_cache_file(self, filename):
    ...
    def upload_csv(
        ...
        cache_file = self.__get_cache_file(dest_filename)
    ...
    def upload_json(self, json_string, dest_filename):
    ...
    def download_csv(
        ...
        cache_file = Path(self.__get_cache_file(source_filename))
        if not flush_cache and cache_file.is_file():
    ...
    def download_excel(self, source_filename, flush_cache=False):
