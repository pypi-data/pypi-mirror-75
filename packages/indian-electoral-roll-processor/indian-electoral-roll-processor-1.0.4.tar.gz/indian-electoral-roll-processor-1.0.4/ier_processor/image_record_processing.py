...
class HeaderPage(ExtractionSuccess):
    ...
    def __init__(self, **kwargs):
        ...
        self.block = kwargs.get('block', ExtractionFailure("", "Could not extract bloxk"))
...
class VoterRecord(ExtractionSuccess):
    ...
    def __init__(self, **kwargs):
        ...
        self.name = kwargs.get('name', ExtractionFailure("", "Cound not extract name"))
...
class ImageData:
    ...
    def __init__(self, file):
        ...
        if matching:
            ...
            self.page = matching.group(3)
            self.type = matching.group(4)
            self.id = matching.group(5)
...
class ImageProcessor:
    ...
    def process_pdf_header_image(self, image_data):
        ...
        r_main_town_or_village = RegexExtractor(re.compile(f"Main Town or Village{not_alphabets}+(.*)"))
        r_post_office = RegexExtractor(re.compile(f"Post Office{not_alphabets}+(.*)"))
        r_village_council = RegexExtractor(re.compile(f"Village Council{not_alphabets}+(.*)"))
        r_block = RegexExtractor(re.compile(f"Block{not_alphabets}+(.*)"))
        r_police_station = RegexExtractor(re.compile(f"Police Station{not_alphabets}+(.*)"))
        r_sub_division = RegexExtractor(re.compile(f"Sub Division{not_alphabets}+(.*)"))
        r_district = RegexExtractor(re.compile(f"District{not_alphabets}+(.*)"))
        r_pin_code = RegexExtractor(re.compile(f"Pin Code.*?([0-9]+)$"))
    ...
    def process_voter_image(self, image_data):
        name_regex = RegexExtractor(re.compile(r'Name[^A-Z^a-z)]*(.*)'), regex_group_to_extract=1)
        house_number_regex = RegexExtractor(re.compile(r'House Number[^A-Z^a-z^0-9]*(.*)'))
        age_regex = RegexExtractor(re.compile(r"Age: (\d+)"))
        gender_regex = RegexExtractor(re.compile(r"Gender: (MALE|FEMALE)"))


        voter_box_image = self._open_image_in_working_dir(image_data.file)
    ...
    def process_assembly_image(self, image_data):
        assembly_extractor = RegexExtractor(re.compile("Assembly Constituency No and Name : (\\d+-)?(.*)"), 2)
    ...
    def process_section_header(self, image_data):
        section_extractor = RegexExtractor(re.compile("Section No and Name : (\\d+-)?(.*)"), 2)
