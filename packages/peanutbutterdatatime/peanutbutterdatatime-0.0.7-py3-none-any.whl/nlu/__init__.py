...
from nlu.components import tokenizer, stemmer, spell_checker, normalizer, lemmatizer, embeddings
...
from nlu.components.sentence_detectors.deep_sentence_detector.deep_sentence_detector import DeepSentenceDetector
# Embeddings
...
from nlu.components.tokenizers.spark_nlp_tokenizer.spark_nlp_tokenizer import SparkNLPTokenizer
...
def build(component_request):
    ...
    See component_tests.py for usage
    ...
    pipe = NLUPipeline()
    # after this, every component in the pipeline.components list should be a NLU.component type!
    ...
    if type(component_request) == list:
        for component_to_build in component_request:
            if type(component_to_build) == str:
                pass  # string query
            # if type(component_to_build) == type(nlu.SparkNLUComponent): todo bad implemented check that its a subtype of component???
            else:
...
def get_default_component_of_type(missing_component_type):
    ...
    if not '@' in missing_component_type :
        ...
        if 'token' in missing_component_type: return nlu.components.tokenizer.Tokenizer("spark_nlp_tokenizer")
        ...
        if missing_component_type == 'chunk_embeddings': return None
        ...
        if 'chunk_embeddings' in missing_component_type:
            ...
            return None # TODO
        if 'unlabeled_dependency' in missing_component_type or 'dep' in missing_component_type:

            return UnlabledDepParser('unlabeled_dependency_parser') # TODO

        if 'labled_dependency' in missing_component_type or 'dep.untyped' in missing_component_type :
            return LabledDepParser('labeled_dependency_parser') # TODO
...
def parse_component_data_from_name_query(request, detect_lang=False):
    '''
    This method implements the main namespace for all component names
    It returns a list of NLU.component objects.
    ...
    :param detect_lang: Wether to automaltically  detect language
    ...
    return resolve_component_from_parsed_query_data(language, component_type, dataset, component_embeddings, request)
...
def resolve_component_from_parsed_query_data(language, component_type, dataset, component_embeddings, full_request):
    ...
    '''
    component_kind = ''  # either model or pipe
    sparknlp_reference = ''
    ...
    if resolved == False and full_request in NameSpace.default_component_references.keys():
        sparknlp_data = NameSpace.default_component_references[full_request]
    ...
    if component_kind == 'pipe':
        ...
        logger.exception("Could not resolve query=%s for kind=%s and reference=%s ", full_request, component_kind,
                         sparknlp_reference)
        return None
...
def construct_component_from_pipe_identifier(language, sparknlp_reference):
    ...
    for component in pipe.light_model.pipeline_model.stages:
        logger.info("Parsing Component for : %s", component)
        ...
        if parsed == 'sentence': constructed_components.append(nlu.Util(model=component))
        if parsed == 'regex': constructed_components.append(nlu.Util(model=component))
        ...
        if parsed == 'vivekn': constructed_components.append(
            nlu.Classifier(model=component))
        if parsed == 'chunker': constructed_components.append(
            nlu.Classifier(model=component))
        if parsed == 'stopwords': constructed_components.append(
            nlu.Classifier(model=component))

        logger.info("Inferred Component type : %s", parsed)
...
def construct_component_from_identifier(language, component_type, dataset, component_embeddings, full_request,
    ...
    '''
    logger.info('Creating NLU component for type=%s and sparknlp reference=%s', component_type, sparknlp_reference)
    if component_type == 'embed' or 'albert' in component_type or 'bert' in component_type or 'xlnet' in component_type or 'use' in component_type or 'glove' in component_type or 'elmo' in component_type or 'tfhub_use' in sparknlp_reference:
        ...
        return SpellChecker(component_name=component_type, language=language, get_default=False,
                            sparknlp_reference=sparknlp_reference)
    elif component_type == 'dep' or component_type == 'dep.typed':
        return LabledDepParser(component_name='labeled_dependency_parser', language=language, get_default=False,
                               sparknlp_reference=sparknlp_reference)
    elif component_type == 'dep.untyped' or component_type == 'dep.untyped':
        return UnlabledDepParser(component_name='unlabeled_dependency_parser', language=language, get_default=False,
                                 sparknlp_reference=sparknlp_reference)
        ...
        return nlu.normalizer.Normalizer(component_name=component_type, language=language, get_default=False,
                                         sparknlp_reference=sparknlp_reference)
    elif component_type == 'clean':
        return nlu.StopWordsCleaner( language=language, get_default=False,
                                         sparknlp_reference=sparknlp_reference)

    logger.exception('Could not resolve Component for type=%s and sparknl reference=%s',component_type, sparknlp_reference)
    # todo chunks and matchers, selectors?
