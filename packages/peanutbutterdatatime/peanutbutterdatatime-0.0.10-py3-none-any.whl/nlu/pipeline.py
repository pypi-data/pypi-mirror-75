...
class NLUPipeline(BasePipe):
    ...
    def rename_columns_and_extract_map_values(self,ptmp, fields_to_rename, same_output_level):
        ...
        # edge case swap. We must rename .metadata fields before we get the .result fields or there will be errors. So we swap position of them
        cols_to_swap = [field for field in fields_to_rename if '.metadata' in field]
        for swap in cols_to_swap : 
            name = swap.split('.')[0] +'.result'
            fields_to_rename[fields_to_rename.index(swap)], fields_to_rename[fields_to_rename.index(name)] = fields_to_rename[fields_to_rename.index(name)] , fields_to_rename[fields_to_rename.index(swap)] 
            logger.info('Swapped selection order for  %s and %s before renaming ', swap, name)
        if same_output_level == True :
            for i, field in enumerate(fields_to_rename):
                ...
                if 'metadata' in field :  # rename metadata to something more meaningful
                    ...
                    if 'sentiment' in field  or 'class' in field or 'category' in field :
                        new_field = new_field.replace('.metadata', '_confidence').replace('metadata', 'confidence')
                        ptmp = ptmp.withColumn(new_field,map_values('res.' + str(i)))
                        columns_for_select.append(new_field)
                        logger.info('Created Meta Data for : nr=%s , name=%s with new_name=%s and original', i, field,new_field)
                        logger.info('new_field ', new_field)
                        # ptmp = ptmp.withColumn(new_field+'_keys',map_keys('res.' + str(i))) # Map keys seem irrelevant if we extract the feature.results since both containt he same information
                        # columns_for_select.append(new_field+'_keys')
                        continue

                    if 'spell' in field :
                        new_field = new_field.replace('.metadata', '_cost').replace('metadata', 'cost')
                        ptmp = ptmp.withColumn(new_field,map_values('res.' + str(i)))
                        columns_for_select.append(new_field)
                        # ptmp = ptmp.withColumn(new_field+'_keys',map_keys('res.' + str(i)))
                        # columns_for_select.append(new_field+'_keys')
                        logger.info('Created Meta Data for : nr=%s , name=%s with new_name=%s and original', i, field,new_field)
                        continue
            ...
            for i, field in enumerate(fields_to_rename):
                ...
                if 'metadata' in field :  # rename metadata to something more meaningful
                    ...
                    for key in keys_in_metadata: 
                        new_fields.append(new_field.replace('.metadata', '_confidence_'+key).replace('metadata', 'confidence_'+key))

                        # These Pyspark UDF extracts from a lsit of maps all the map values for positive and negative confidence and also spell costs
                    ...
                    #     new_field = new_field.replace('.metadata', '_confidence').replace('metadata', 'confidence')
                    #     # These Pyspark UDF extracts from a lsit of maps all the map values for positive and negative confidence and also spell costs
                    #     def extract_confidence(x): return [float(sentence['confidence']) for sentence in x]
                    #     #extract map values for list of maps 
                    #     confidence_for_list_of_maps = udf(lambda z: extract_confidence(z), ArrayType(FloatType()))
                    #     ptmp = ptmp.withColumn(new_field, confidence_for_list_of_maps(field)) 
                    #     
                    #     columns_for_select.append(new_field)
                    #     logger.info('Created Meta Data for   : nr=%s , name=%s with new_name=%s and original', i, field,new_field)
                    #     logger.info('new_field ', new_field)
                    ...
                    #     new_field = new_field.replace('.metadata', '_cost').replace('metadata', 'cost')
                    #     #extract map values for list of maps 
                    #     def extract_spell_cost(x): return [float(sentence['cost']) for sentence in x]
                    #     spell_Cost_for_list_of_maps = udf(lambda z: extract_spell_cost(z), ArrayType(FloatType()))
                    #     ptmp = ptmp.withColumn(new_field, spell_Cost_for_list_of_maps(field))
                    #     columns_for_select.append(new_field)
                    #     logger.info('Created Meta Data for   : nr=%s , name=%s with new_name=%s and original', i, field,new_field)
                    #     continue
    ...
    def infer_and_set_output_level(self,sdf):
        ...
        if not self.output_level + '.result' in sdf.columns :
            if 'document' + '.result' in sdf.columns: self.output_level='document'
            elif 'sentence' '.result' in sdf.columns: self.output_level='sentence' 
            elif 'chunk' + '.result' in sdf.columns: self.output_level='chunk'
            elif 'token' + '.result' in sdf.columns: self.output_level='token'
        logger.info('Inferred and set output level of pipeline to %s', self.output_level)



    def pythonify_spark_dataframe(self, processed, get_different_level_output=True):
        '''
        ...
        '''
        if not self.user_has_set_output_level  : self.infer_and_set_output_level(processed)
        ...
        for field in processed.schema.fieldNames():
            ...
            if self.resolve_type_to_output_level(f_type, field) == self.output_level:
                if 'embeddings' not in field: same_output_level_fields.append(field + '.result')  # result of embeddigns is just the word/sentence
                ...
                if 'embeddings' not in field: not_at_same_output_level_fields.append(field + '.result')  # result of embeddigns is just the word/sentence
                if self.output_positions:
        ...
        ptmp,final_select_same_output_level =  self.rename_columns_and_extract_map_values(ptmp=ptmp, fields_to_rename=same_output_level_fields, same_output_level=True)
        ...
        return ptmp.select(final_select_same_output_level + final_select_not_at_same_output_level ).toPandas()# + final_select_not_at_same_output_level
    ...
    def predict(self, data):
        ...
        if type(data) is pd.DataFrame:  # casting follows pd->spark->pd
            ...
            if len(data.shape) != 1:
                print("Exception : Input numpy array must be 1 Dimensional for prediction.. Input data shape is",data.shape, ' TODO ACTUALLY THROW EXCPETION here')
                return ''
