...
"""
from pneuro.dataLoad import data_loader
from pneuro.dataPreprocessing import preProcess
from pneuro.dataVisualization import visualization
from pneuro.modelTraining import classificationModels
from pneuro.prediction import predict

app = Flask(__name__)

# All class instantiation
data_getter = data_loader.DataGetter()
preprocessor = preProcess.Preprocessor()
visuals = visualization.DataVisualization()
classification_model = classificationModels.ClassificationModelTuner()
...
def load_data_from_source():
    ...
    data_getter = data_loader.DataGetter()  #Class DataGetter object created
    global dataset    #Declare global variable
    data_getter = data_loader.DataGetter()  # instantiation for genrating preprocessing logs
    dataset = data_getter.get_data(file_type, file)   #call get_data method, it will load the dataset into dataframe
    preprocessor = preProcess.Preprocessor()  # instantiation for genrating preprocessing logs
...
def start_processing():
    ...
    global x, y
    preprocessor = preProcess.Preprocessor()  # instantiation for genrating prediction logs
...
def build_model():
    ...
    if problem_type == 'Classification':
        classification_model = classificationModels.ClassificationModelTuner()  # instantiation for genrating prediction log
...
def predict():
    ...
    data_getter = data_loader.DataGetter() #declare object for DataGetter class
    global prediction_dataset
    prediction = predict.Prediction()  # instantiation for genrating prediction log
