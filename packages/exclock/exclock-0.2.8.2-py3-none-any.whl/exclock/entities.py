...
from ycontract import prev_contract
...
class Sound:
    ...
    @prev_contract(lambda self: Path(self.sound_filename).exists())
    def play(self):
        p = Player(self.sound_filename)
        p.play()

    @classmethod
    @prev_contract(lambda json: "message" in json and "sound_filename" in json)
    ...
    @prev_contract(lambda self, other: isinstance(other, self.__class__))
...
class ClockTimer:
    ...
    @prev_contract(lambda count: count >= 0)
    ...
    @classmethod
    @prev_contract(lambda json: "sounds" in json)
    @prev_contract(lambda json: isinstance(json, dict))
