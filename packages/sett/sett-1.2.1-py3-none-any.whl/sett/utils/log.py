...
def exception_to_message(error_list=(), logger=logging):
    """A function decorator catching the first exception of instance contained
    in or equal to error_list, occurring while executing the decorated
    function, logging an error and exiting python.
    """
    def dec(f):
        @wraps(f)
        def wrapped_f(*args, on_error=lambda: sys.exit(1), **kwargs):
            try:
...
def create_logger(name, logger_level: int = logging.DEBUG) -> logging:
    ...
    # Define a context manager a function decorator for the logger.
    logger.log_task = lambda message: LogTask(message,
                                              ok_logger=logger.info,
                                              err_logger=logger.error)
    logger.exception_to_message = lambda error_list=(): \
        exception_to_message(error_list, logger)
    return logger


def add_rotating_file_handler_to_logger(logger: logging,
                                        log_dir: str,
    ...
    if not log_dir.is_dir():
        try:
            ...
            logger.error(f'unable to create directory {log_dir.as_posix()}.')
...
def add_stream_handler_to_logger(logger: logging,
...
def get_default_log_dir() -> str:
    ...
    """
    default_log_dir_by_os = {'Linux': ('.local/var/log/sett',),
