...
def plot_model(model, filename = 'model.png', show_shapes = True, rankdir='TB', dpi=96, **kwargs):
    ...
    for ly in model.layers:
        ...
        out_shapes[__name__] = f'(?,{",".join([str(lln) for lln in ly.output.shape.as_list()[1:]])})'
    ...
    for node_name in inbounds:
        ...
        for ib,ish in zip(inbounds[node_name],in_shapes[node_name]):
            kww = {'label':ish} if show_shapes and \
                                   (np.any([ish != prev_ish for prev_ish in in_shapes[ib]]) \
                                    or len(in_shapes[ib]) == 0)\
            else {'minlen':1, 'arrowhead':'none'}#, 'style': 'invis'}
        ...
        if node_name in output_nodes:
            ...
            kww = {'label': out_shapes[node_name]} if show_shapes \
                else {'minlen':1,'arrowhead':'none'}#, 'style': 'invis'}
            edge = pydot.Edge(node,out_node,**kww)
            graph.add_edge(edge)
