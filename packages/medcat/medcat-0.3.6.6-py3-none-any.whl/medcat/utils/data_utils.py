...
def load_data(data_path, require_annotations=True):
...
def meta_ann_from_ann(ann, meta_name):
    ...
    if type(meta_anns) == dict:
        return meta_anns.get(meta_name, Name)
    else:
...
def are_docs_same(document, document2, require_double_inner=True, ann_stats=[], meta_names=[]):
    are_same = True
    ...
    for ann2 in document2['annotations']:
        ...
        if ann is None:
            if not require_double_inner:
                ...
                document['annotations'].append(ann2)
            else:
        ...
        for doc in project['documents']:
            for ann in doc['annotations']:
                ...
                if ann['validated']:
                    cnt += 1
                    g_cnt += 1
        print("Number of documents in '{}' is: {}".format(project['name'], cnt))

    print("Total number of documents is: {}".format(g_cnt))
...
def consolidate_double_annotations(data_path, out_path, require_double=True, require_double_inner=True, meta_anns_to_match=[]):
    ...
    require_double (boolean):
        If True everything must be double annotated, meaning there have to be two projects of the same name for each name. Else, it will
            also use projects thta do not have double annotiations.
            ...
            if not require_double:
                ...
                if has_double:
                    for document in documents:
                        ...
                        if document2 is not None and are_docs_same(document, document2, require_double_inner=require_double_inner, ann_stats=ann_stats, meta_names=meta_anns_to_match):
    ...
    ck = cohen_kappa_score(ann_stats[:, 0], ann_stats[:, 1])
    t = 0
    ...
    print("Total vs InAgreement: {} / {}".format(t, len(ann_stats)))
    return ann_stats, out_data
