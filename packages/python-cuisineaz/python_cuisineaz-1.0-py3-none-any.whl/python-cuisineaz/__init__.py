...
def get_ingredient_quantity(self, arg1):
    for j in range(len(arg1)):
        i=0
    while(1):
        if arg1[j][i] in ["1","2","3",'4','5','6','7','8','9','0']:
            i+=1
        else:
            break
...
class CuisineAZ(object):
    ...
    def get(uri):
        ...
        people_data=soup.find("span", {"id": "ContentPlaceHolder_LblRecetteNombre"}).split(" ")[0]

        list_ingredient={}
        ingredients_data = soup.findAll("ul", {"class": "txt-dark-gray"})
        for i in ingredients_data:
            list_ingredient.update({get_ingredient_quantity(i.text): i.text})
