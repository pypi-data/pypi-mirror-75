...
from sklearn.metrics import accuracy_score, confusion_matrix, log_loss, recall_score, roc_auc_score, precision_score, precision_recall_curve
# cllassifiers import
...
defualt_classifers = [{"clf": LinearDiscriminantAnalysis(), "name": "LDA"},
                      {"clf": GaussianNB(), "name": "Naiev Bayes Gaussian"},
                      {"clf": SGDClassifier(), "name": "SGDClassifier"},
                      {"clf": MLPClassifier(), "name": "MLPClassifier"},
                      {"clf": RandomForestClassifier(), "name": "Random Forest"},
                      {"clf": SVC(gamma=0.01, probability=True, C=1, kernel='linear'), "name": "linear SVM"},
                      {"clf": QuadraticDiscriminantAnalysis(), "name": "QDA"},
                      {"clf": AdaBoostClassifier(), "name": "Adaptive Boosting "},
                      {"clf": GradientBoostingClassifier(), "name": "Gradient Boosting "},
                      {"clf": KNeighborsClassifier(), "name": "k-neighbors"},
                      {"clf": DecisionTreeClassifier(), "name": "Decision Tree"},
                      {"clf": LogisticRegression(), "name": "Logistic Regression"},
                      {"clf": KNeighborsClassifier(), "name": "k-neighbors"}]

##
def classifiers_competition(X, y, cv=5, figure_title="Title", random_state=None):
    """
    ...
    :param figure_title:
    :param random_state:
    :return: polting a hirzontal bar of the accurcy scores for each classifer in the list
    ...
    classifiers_list = defualt_classifers
    scoring = ['accuracy']
    ...
    for clf_list in classifiers_list:
        ...
        name = clf_list["name"]
        Accuracy = np.mean(recall['test_accuracy'])
    ...
    results = pd.DataFrame(Acc, index=[_["name"] for _ in classifiers_list], columns=["Accuracy"])
...
def std_metrics_(model, X, y, cv, positive_label=0, sigma=1, groups=None):
    unq = np.unique(y)
    if len(unq) == 2:

        if positive_label == unq[0]:
            N_1 = y[y == unq[0]].shape[0]
            N_2 = y[y == unq[1]].shape[0]
        else:
            N_1 = y[y == unq[1]].shape[0]
            N_2 = y[y == unq[0]].shape[0]
        stds = []
        for scr in ["accuracy", "recall", "precision_weighted", "precision"]:
            scores = cross_val_score(model, X, y, cv=cv, scoring=scr, groups=groups)
            stds.append(2 * scores.std())

        d_accuracy = stds[0] * sigma
        d_recall = stds[1] * sigma
        d_prescion_wighted = stds[2] * sigma
        d_pres = stds[3] * sigma

        N = N_1 + N_2
        n_1, n_2 = N_1 / N, N_2 / N
        d_recall_2 = 0.5 * ((d_accuracy / n_2) ** 2 + ((n_1 / n_2) * d_recall) ** 2) ** 0.5
        d_pres_2 = 0.5 * ((d_prescion_wighted / n_2) ** 2 + ((n_1 / n_2) * d_pres) ** 2) ** 0.5
        errors = {"accuracy_err": d_accuracy, "sensitivity_err": d_recall, "specificity_err": d_recall_2,
                  "ppv_err": d_pres, "ppv_err": d_pres_2}
    else:
        stds = []
        for scr in ["accuracy", "recall_weighted", "precision_weighted"]:
            scores = cross_val_score(model, X, y, cv=cv, scoring=scr, groups=groups)
            stds.append(2 * scores.std())
        d_accuracy = stds[0] * sigma
        d_recall = stds[1] * sigma
        d_prescion = stds[2] * sigma
        errors = {"accuracy_err": d_accuracy, "recall_err": d_recall, "prescion_err": d_prescion}
    return errors
...
def prepare_multi_target_data(df, targets, x_0='1801.264', x_end='898.703', positive_label="S", xy_return="at_least_one"):
    """
...
def std_report(model, X, y, cv):
    accuracy =  cross_val_score(model,X,y,cv)
    recll_1 = cross_val_score(model,X,y,cv,)
...
class DeepClassification:
    ...
    def leave_one_out(self, sensitive_label=1, confidence_interval=False):
        ...
        self.scores_std_ = std_metrics_(self.best_model_, self.X, self.y, cv=loo, positive_label=sensitive_label, sigma=1)
    ...
    def leave_group_out(self, groups, sensitive_label=1, confidence_interval=False):
        ...
        self.y_predict_ = y_hat
        self.y_predict_prob_ = y_hat_prob
        self.scores_std_ = std_metrics_(self.best_model_, self.X, self.y, cv=logo, positive_label=sensitive_label, sigma=1,groups=groups)
    ...
    def train_test_split(self, test_size=0.3, sensitive_label=1, confidence_interval=False):
        ...
        if len(np.unique(self.y)) == 2:
            ...
            self.auc_ = auc(self.fpt_, self.tpr_)
        self.scores_std_ = std_metrics_(self.best_model_, self.X, self.y, cv=5, positive_label=sensitive_label, sigma=1)
    ...
    def train_test_split_mean_median_outliers_drop(self, test_size=0.30, itirations_rate="mean", contam_post=0.05, contam_neg=0.05):
        ...
        thresholds = np.linspace(threshold[-1], threshold[0], mean_tpr.shape[0])
        self.best_threshold_ = thresholds[index_max][0][0]
        self.scores_std_ = std_metrics_(self.best_model_, self.X, self.y, cv=5, positive_label=0, sigma=1)
    ...
    def k_folds(self, n_folds=5, shuffle=False, random_state=None, confidence_interval=False, sensitive_label=1):
        ...
        self.scores_std_ = std_metrics_(self.best_model_, self.X, self.y, cv=n_folds, positive_label=sensitive_label, sigma=1)
        if len(np.unique(self.y)) == 2:
    ...
    def show_roc(self, legend_location=4, fig_size_x=7, fig_size_y=7, roc_save_location=None):
        ...
        if roc_save_location!=None:
            tim = "\\%d_%d_%d__%d_%d_%d" % (time.localtime()[0], time.localtime()[2], time.localtime()[1], time.localtime()[3], time.localtime()[4],
                                          time.localtime()[5])
            plt.savefig(roc_save_location + tim + ".tif", dpi=600, bbox_inches='tight', pil_kwargs={"compression": "tiff_lzw"})
...
def ensemble_targets(actual_targets_list, predicted_targets_list, postive_target):
    ...
    :param actual_targets_list: list of actual targets arrays
    :param predicted_targets_list: list of predicted trgets arrays

    :return: actual, predicted values according to statistical caluclation(pandas data frame) and
             classification report as pandas Data Frame
    """
    actual_ary = actual_targets_list[0]
    predict_ary = predicted_targets_list[0]
    for n_targets in range(1, len(actual_targets_list)):
        actual_ary = np.transpose(np.vstack((actual_ary, actual_targets_list[n_targets])))
        predict_ary = np.transpose(np.vstack((predict_ary, predicted_targets_list[n_targets])))

    y_at_actual = np.zeros(actual_ary.shape[0])
    y_at_predicted = np.zeros(actual_ary.shape[0])
    if postive_target == 0:
        y_at_actual[np.count_nonzero(actual_ary, axis=1) == actual_ary.shape[1]] = 1
        y_at_predicted[np.count_nonzero(predict_ary, axis=1) == actual_ary.shape[1]] = 1
        y_at_predicted[np.sum(predict_ary, axis=1) == (actual_ary.shape[1] * 888)] = 888
        df_ = pd.DataFrame()
        ...
        indexs_list = []
        for ix in range(1, targets_num + 1):
            arr_act = np.zeros(actual_ary.shape[0])
            arr_prd = np.zeros(actual_ary.shape[0])

            arr_act[np.count_nonzero(actual_ary, axis=1) != (targets_num - ix)] = 1
            arr_prd[np.count_nonzero(predict_ary, axis=1) != (targets_num - ix)] = 1

            df_[str(ix) + "/" + str(targets_num) + "_act"] = arr_act.astype(int)
            df_[str(ix) + "/" + str(targets_num) + "_prd"] = arr_prd.astype(int)
            indexs_list.append(str(ix) + "/" + str(targets_num))
        df_act = df_.iloc[:, ::2]
        df_prd = df_.iloc[:, 1::2]

    elif postive_target == 1:
        actual_ary_ = abs(1 - actual_ary)
        predict_ary_ = abs(1 - predict_ary)
        y_at_actual[np.count_nonzero(actual_ary_, axis=1) == actual_ary.shape[1]] = 1
        y_at_predicted[np.count_nonzero(predict_ary_, axis=1) == actual_ary.shape[1]] = 1
        y_at_predicted[np.sum(predict_ary, axis=1) == (actual_ary.shape[1] * 888)] = 888
        df_ = pd.DataFrame()
        df_["at_leas_one_act"] = y_at_actual.astype(int)
        df_["at_leas_one_prd"] = y_at_predicted.astype(int)
        ...
        for ix in range(1, targets_num + 1):
            ...
            arr_act[np.count_nonzero(actual_ary_, axis=1) != (targets_num - ix)] = 1
            arr_prd[np.count_nonzero(predict_ary_, axis=1) != (targets_num - ix)] = 1
        ...
        df_act = df_.iloc[:, ::2]
        df_prd = df_.iloc[:, 1::2]
    ix_888 = df_[df_["at_leas_one_prd"] == 888].index
    contamination = len(ix_888) / actual_ary.shape[0]
    df_ = df_.drop(index=df_[df_["at_leas_one_prd"] == 888].index.values)
    acc = []
    ppv = []
    npv = []
    sen = []
    ...
    return df_, df_scores, contamination
