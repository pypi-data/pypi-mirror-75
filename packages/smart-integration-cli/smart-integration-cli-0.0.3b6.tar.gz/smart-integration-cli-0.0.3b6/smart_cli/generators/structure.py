...
class Structure(object):
    ...
    def rewrite_basic_files(self, default: bool = True):
        ...
        os.chdir(self.project_name)
        lib_path = self.get_lib_path()
        templates_path = lib_path + '/smart_cli/templates' if default else lib_path + \
                                                                           '/smart_cli/templates/docker_files'
        params = {'project_name': self.project_name, 'secret_key': get_random_string(), 'app_name': 'api'}
        if not default:
