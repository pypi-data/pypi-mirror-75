...
class Broker:
    ...
    def start(self):
        ...
        try:
            ...
            self.logger.info("Start broadcast loop转发功能就绪")
    ...
    def client_connected(self, listener_name, reader: ReaderAdapter, writer: WriterAdapter):
        ...
        #初始化和启动循环，用于处理客户端消息（发布，订阅/取消订阅，断开连接）
                ...
                if subscribe_waiter in done:
                    ...
                    for index, subscription in enumerate(subscriptions['topics']):
                        if return_codes[index] != 0x80:
                            ...
                            self.logger.info("发送了该主题的retained信息")
                    subscribe_waiter = asyncio.Task(handler.get_next_pending_subscription(), loop=self._loop)
                    self.logger.debug(repr(self._subscriptions))
                if wait_deliver in done:
                    #if self.logger.isEnabledFor(logging.DEBUG):
                    self.logger.info("转发功能被启用")
                    self.logger.info("%s handling message delivery" % client_session.client_id)
                    app_message = wait_deliver.result()

                    if not app_message.topic:#待转发的topic
    ...
    def _broadcast_loop(self):
        ...
        try:
            while True:
                ...
                print(repr(self._subscriptions))
                self.logger.info("从_broadcast_queue取出")
                if self.logger.isEnabledFor(logging.DEBUG):
    ...
    def _broadcast_message(self, session, topic, data, force_qos=None):
        ...
        self.logger.info("写入_broadcast_queue，_broadcast_loop会来取出")
