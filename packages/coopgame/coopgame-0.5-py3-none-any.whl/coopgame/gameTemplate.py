...
class GameTemplate(MonitoredClass):
    ...
    def __init__(self, fullscreen:bool = False, screen_width:int=1600, screen_height:int=1000, max_fps:int = 60):
        super().__init__()
    ...
    def _handle_events(self):
        ...
        if event.type == pygame.QUIT or (event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE):
            logging.info(f"Exiting game via: {event.type}")
            self.running = False
        elif event.type == pygame.KEYDOWN and event.key == pygame.K_F12:
            self.fullscreen = not self.fullscreen
            self._init_screen(self.fullscreen)
        elif event.type == pygame.KEYDOWN:
            logging.info(f"Key pressed: {pygame.key.name(event.key)} [{event.key}]")
