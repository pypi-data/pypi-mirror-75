...
class spacingUtil(object):

    def __init__(self):
        self.CLASS_1 = 1  # next is space
        self.CLASS_0 = 0  # next is not space

        self.S1 = re.compile(u'''[\s]+''')
    ...
    def build_dictionary(self, train_path, padd):
        char_rdic = []
        visit = {}
        fid = self.open_file(train_path, 'r')
        for line in fid:
            line = line.strip()
            if line == "": continue
            #print(line)
    ...
    def one_hot(self, i, size):
        return [1 if j == i else 0 for j in range(size)]


    def get_xy_data(self, sentence, pos, n_steps, padd):
        slen = len(sentence)
        x_data = []
        y_data = []
        next_pos = -1
        ...
        while i < slen:
            ...
            if next_c == ' ':
                y_data.append(self.CLASS_1)
            else:
                y_data.append(self.CLASS_0)
            count += 1
            i += 1
        ...
        if count == n_steps:
            ...
            diff = n_steps - count
            x_data += [padd] * diff
            y_data += [self.CLASS_0] * diff
            next_pos = -1
        return x_data, y_data, next_pos, count


    def next_batch(self, sentence, pos, char_dic, vocab_size, n_steps, padd):
        '''
        ...
        batch_xs = []
        batch_ys = []
        x_data, y_data, next_pos, count = self.get_xy_data(sentence, pos, n_steps, padd)
        x_data = [char_dic[c] if c in char_dic else padd for c in x_data]
        x_data = [self.one_hot(i, vocab_size) for i in x_data]
    ...
    def test_next_batch(self, train_path, char_dic, vocab_size, n_steps, padd):
        fid = self.open_file(train_path, 'r')
        for line in fid:
            line = line.strip()
            if line == "": continue
            ...
            pos = 0
            while pos != -1:
                batch_xs, batch_ys, next_pos, count = self.next_batch(sentence, pos, char_dic, vocab_size, n_steps, padd)
                print
    ...
    def get_validation_data(self, validation_path, char_dic, vocab_size, n_steps, padd):
        validation_data = []
        fid = self.open_file(validation_path, 'r')
        for line in fid:
            line = line.strip()
            if line == "": continue
            # line = line.decode('utf-8')
    ...
    def to_sentence(self, tag_vector, sentence):
        out = []
        j = 0
    ...
    def build_dictionary_emb(self, embedding_dir):
    ...
    def next_batch_emb(self, sentence, pos, char_dic, id2emb, n_steps, padd):
        '''
        y_data =  1 or 0     => n_steps unfolding => [0,0,1,0,...]
        ^
        |
    ...
    def test_next_batch_emb(self, train_path, char_dic, id2emb, n_steps, padd):
        fid = self.open_file(train_path, 'r')
        for line in fid:
            line = line.strip()
            if line == "": continue
            ...
            pos = 0
            while pos != -1:
                batch_xs, batch_ys, next_pos, count = self.next_batch_emb(sentence, pos, char_dic, id2emb, n_steps, padd)
                print
                'window : ' + sentence[pos:pos + n_steps].encode('utf-8')
                print
                'count : ' + str(count)
    ...
    def get_validation_data_emb(self, validation_path, char_dic, id2emb, n_steps, padd):
        validation_data = []
        fid = self.open_file(validation_path, 'r')
        for line in fid:
            line = line.strip()
