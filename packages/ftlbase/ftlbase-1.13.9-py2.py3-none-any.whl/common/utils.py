...
def export_users():
    ...
    # https://stackoverflow.com/questions/12605265/python-execute-shell-command-and-continue-without-waiting-and-check-if-running-b
    command1 = subprocess.Popen([f'{settings.BASE_DIR}/teste.sh', '1', '8'])
    command1 = subprocess.Popen([f'{settings.BASE_DIR}/teste.sh', '2', '9'])
    command1 = subprocess.Popen([f'{settings.BASE_DIR}/teste.sh', '3', '0'])
    command1 = subprocess.Popen([f'{settings.BASE_DIR}/teste.sh', '4', '1'])
    command1 = subprocess.Popen([f'{settings.BASE_DIR}/teste.sh', '5', '2'])
    command1 = subprocess.Popen([f'{settings.BASE_DIR}/teste.sh', '6', '3'])
    command1 = subprocess.Popen([f'{settings.BASE_DIR}/teste.sh', '7', '4'])
    return file
