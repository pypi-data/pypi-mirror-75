...
from am_get.request_conf import COM, CA, UK, DE, USER_AGENTS
import collections
...
class AmazonRequest():
    ...
    def __init__(self, cookies_com=[],
        ...
        if cookies_com: COM = cookies_com
        if cookies_ca: CA = cookies_ca
        if cookies_uk: UK = cookies_uk
        if cookies_de: DE = cookies_de
    ...
    def amazon_request(self, url):
        ...
        if 'www.amazon.ca' in self.url:
            self.head.update({"cookie": random.choice(CA)})
            proxy_loc = "ca"
            
        if 'www.amazon.co.uk' in self.url:
            self.head.update({"cookie": random.choice(UK)})
            proxy_loc = "uk"
            
        if 'www.amazon.de' in self.url:
            self.head.update({"cookie": random.choice(DE)})
            proxy_loc = "de"
