...
def mainmasterprogfundamental():
    ...
    with conn:
        ...
        jobloadlist =[]
        cursor.execute(seljobload,(jday,))
        jobtbl = cursor.fetchall()
        if jobtbl is None:
            jobtbl = []
        else:
            pass
        ...
        if lt>0:
            for i in range(lt):
                ...
                if symbol in jobloadlist:
                    ...
                    cursor.execute(insjob,(symbol,'mfundamental',jday,'WIP','stock_all'))
                    loadstockfundamentals(symbol, stocktable, fxr, prio, jday, cursor)
        else:
        ...
        bmktblq = """select distinct ba.symbol,ba.prio from benchmark_all ba
                    ...
                    bm.symbol=ba.symbol
                    order by ba.prio"""
        cursor.execute(bmktblq)
        btbl = cursor.fetchall()
        if btbl is None:
            btbl = []
        else:
            pass
        ...
        if blt>0:
            for i in range(blt):
                ...
                if bsymbol in jobloadlist:
                    ...
                    cursor.execute(insjob,(bsymbol,'mfundamental',jday,'WIP','benchmark_all'))
                    loadstockfundamentals(bsymbol, bstocktable, fxr, bprio, jday, cursor)
        else:
            print("no stocks in stock_All. check if someone deleted the stock_All entries")
        ...
        if lt>0:
            for i in range(lt):
                ...
                if symbol in jobloadlist:
                    ...
                    print("Loading:",symbol, stocktable, prio)
                    cursor.execute(insjob,((symbol+'-'+str(prio)),'mprice',jday,'WIP','stock_all'))
                    loadstockprice(symbol, stocktable, jday, prio, cursor)
        else:
            print("no stocks in stock_All. check if someone deleted the stock_All entries")
        bmrk = """select distinct symbol,prio from benchmark_all
        where is_active=True order by prio"""
        cursor.execute(bmrk)
        btbl = cursor.fetchall()
        if btbl is None:
            btbl = []
        ...
        if blt>0:
            for i in range(blt):
                ...
                if bsymbol in jobloadlist:
                    ...
                    print("Loading:",bsymbol, bstocktable, bprio)
                    cursor.execute(insjob,(bsymbol,'mprice',jday,'WIP','benchmark_all'))
                    loadstockprice(bsymbol, bstocktable, jday, bprio, cursor)
        else:
            print("no benchmark in benchmark_All. check if someone deleted the benchmark_All entries")
