...
class maincalculationprog:
    def logreturns():
        ...
        with conn:
            ...
            hq = """select symbol, count(*) as cnt from stock_history
                    ...
                    group by symbol"""
            if len(val)>0:
                cursor.execute(hq, (val,))
                hval = cursor.fetchall()
                # print(hval)
                ...
                print("No stocks present in stock_all for all programs")
                stklst = []
            #startdate = dt.datetime.today()
            ...
            if len(val)>0:
                ...
                print("No benchmarks present in benchmark all in all programs")
                bmklst = []
            ...
            for i in range(len(stklst)):
                ...
                cursor.execute(chk,(stk,))
                chkc = cursor.fetchone()
                chkc = chkc[0]
                if chkc != 0:
                    getlogreturn(stk, 'stock_history', cursor)
                else:
                    getcompletelogreturn(stk, 'stock_history', cursor,'tmp')
                ...
                chk50 = """select count(*) from stock_history
                    ...
                    and price_date > current_date - 31"""
                cursor.execute(chk50,(stk,))
                chk50c = cursor.fetchone()
                chk50c = chk50c[0]
                if chk50c != 0:
                    getlastrollingmean.rollingmean50d(stk,'stock_history', cursor)
                else:
                    getrollingmean.rollingmean50d(stk,'stock_history', cursor,'tmp')
                ...
                chk200 = """select count(*) from stock_history
                    ...
                    and price_date > current_date - 31"""
                cursor.execute(chk200,(stk,))
                chk200c = cursor.fetchone()
                chk200c = chk200c[0]
                if chk200c != 0:
                    getlastrollingmean.rollingmean200d(stk,'stock_history', cursor)
                else:
                    getrollingmean.rollingmean200d(stk,'stock_history', cursor,'tmp')
            ...
            for i in range(len(bmklst)):
                ...
                chkb = """select count(*) from benchmark_history
                    where symbol=%s and price_return is not null
                    and price_date > current_date - 31"""
                cursor.execute(chkb,(bmk,))
                chkbc = cursor.fetchone()
                chkbc = chkbc[0]
                if chkbc != 0:
                    getlogreturn(bmk, 'benchmark_history', cursor)
                else:
                    getcompletelogreturn(bmk, 'benchmark_history', cursor,'tmp')
            ...
            chkretb = """select symbol from dbo.benchmark_history where price_return is null and symbol in %s
                       group by symbol having count(*)>1"""
            if len(stklst)>0:
                cursor.execute(chkrets, (stklst,))
                failstk = cursor.fetchall()
                #print(failstk)
                print(failstk, "\n are the list of ", len(failstk), " records from ", len(stklst),
                      " stock history that have no calculated return")
            else:
                print("No need to check empty returns as stock list for all prog is empty")
            if len(bmklst)>0:
                cursor.execute(chkretb, (bmklst,))
                failbmk = cursor.fetchall()
                print(failbmk, "\n are the list of ",len(failbmk)," records from ",len(bmklst)," benchmark history that have no calculated return")
            else:
                print("No need to calculate empty returns as benchmark list is empty fo all prog")
    ...
    def statistics():
        ...
        with conn:
            ...
            """
            try:
                cursor.execute(statqry)
                statres = cursor.fetchall()
