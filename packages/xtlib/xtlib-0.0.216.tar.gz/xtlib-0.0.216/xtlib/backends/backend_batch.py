...
class AzureBatch(BackendBase):
    ...
    def wrap_cmd_for_batch(self, user_cmd, node_id, store_data_dir, data_action, data_writable, 
        ...
        node_store_path = "jobs/{}/node-$1".format(job_id)

        if docker_cmd:
            # blobfuse preinstalled on DSVM but necessarily on docker image
            self.append_install_blobfuse_cmds(cmds, sudo_available=True)

        self.append_title(cmds, "mount INFO_CONTAINER for node dir")
        self.emit_mount_cmds(cmds, storage_name, storage_key, container=constants.INFO_CONTAINER, store_path=node_store_path, 
            mnt_path="$HOME/mnt/node", is_writable=True, install_blobfuse=False, 
            sudo_available=False, use_username=False, use_allow_other=False, env_var_name="XT_NODE_DIR")
    ...
    def prep_node(self, job_id, node_index, run_data_list, num_nodes, use_low_pri, using_hp,  
        ...
        after_path = "node-{}/after/service_logs".format(node_index)         # run store
   ...
   # BACKEND API
    def log_file_iterator(self, service_node_info, log_name, start_offset=0, end_offset=None, 
        encoding='utf-8', use_best_log=True):
        '''
        returns an iterator for the specified log file.
        '''
        job_id = service_node_info["job_id"]
        ...
        if start_offset == 0:
            ...
            node_index = utils.node_index(node_id)
            job_path = "node-{}/after/service_logs/{}".format(node_index, file_path)
            if self.store.does_job_file_exist(job_id, job_path):
                console.print("==> reading log from job storage...")
                ...
                result = {"new_text": new_text, "simple_status": simple_status, "log_name": log_name, 
                    "service_status": batch_status, "file_path": file_path}

                # here we just yield a single result
                yield result                
        ...
        if not got_text:
            ...
            task_index = service_node_info["task_index"]
            task_id = "task" + str(task_index)
            task = None     # delay creation until needed
            ...
            while True:
                try:
                    gft_opts = batchmodels.FileGetFromTaskOptions(ocp_range='bytes={}-{}'.format(next_offset, end_offset))

                    # read currently available data
                    stream = self.batch_client.file.get_from_task(job_id, task_id, file_path=file_path, 
                    ...
                    next_offset += len(new_bytes)

                    # if we are streaming output, assume we are running
                ...
                if not new_text:
                    # is task done?  update status 
                    # CAUTION: this is a 15-30 second call
                    if task is None:
                        console.print("==> creating azure batch client...")
                    task = self.get_node_task(service_node_info)
                ...
                # we never stop yielding; its up to caller to examine status and stop iteration
                yield result
    ...
    def read_log_file(self, service_node_info, log_name, start_offset=0, end_offset=None, 
        ...
        job_path = "node-{}/after/xt_logs/{}".format(node_index, file_path)
        if self.store.does_job_file_exist(job_id, job_path):
            new_text = self.store.read_job_file(job_id, job_path)
            batch_status = "completed"
            simple_status = "completed"
        ...
        if not found_file:
            ...
            try:
                stream = self.batch_client.file.get_from_task(job_id, task.id, file_path=log_name, 
                    file_get_from_task_options=gft_opts)
    ...
    # API call
    def add_service_log_copy_cmds(self, cmds, dest_dir):
