...
class ImplStorage(ImplBase):
    ...
    def extract_logs(self, name, node_index, dest_dir, workspace, response, browse, log_type):
        ...
        if run_helper.is_simple_run_name(name):
        ...
        if extract:
            ...
            if log_type in ["service", "all"]:
                ...
                job_helper.download_logs_from_storage(self.store, items, job_id, node_index, store_path="after/service_logs/**", dest_dir=log_dir)

            if log_type in ["xt", "all"]:
                # xt logs
                log_dir = dest_dir + "/xt_logs"
                job_helper.download_logs_from_storage(self.store, items, job_id, node_index, store_path="after/xt_logs/**", dest_dir=log_dir)
