...
class Runner():
    ...
    def build_compute_def(self, args):
        ...
        compute_def = dict(self.config.get_compute_def(compute))
    ...
    def build_first_run_for_node(self, node_index, box_name, run_script_path, parent_script_path, using_hp, use_aml_hparam, run_specs, 
            job_id, parent_name, cmds, compute_def, repeat_count, fake_submit, search_style, box_secret, args):
        ...
        run_name, full_run_name, box_name, pool = \
            self.create_run(job_id, actual_parts, box_name=box_name, parent_name=parent_name, node_index=node_index, using_hp=using_hp, 
                repeat=repeat_count, app_info=None, path=path, exper_name=exper_name, compute_def=compute_def, fake_submit=fake_submit, 
    ...
    def write_hparams_to_job(self, job_id, cmds, fake_submit, using_hp, args):
    ...
    def process_run_command(self, args):
        ...
        boxes, num_boxes = self.write_hparams_to_job(job_id, cmds, fake_submit, using_hp, args)
        ...
        for i in range(run_count):
            ...
            run_data = self.build_first_run_for_node(i, boxes[i], target_file, ps_path, using_hp, using_aml_hparam, run_specs, job_id, 
                parent_name, cmds, compute_def, repeat_count, fake_submit, search_style, box_secret, args)
        ...
        # SUBMIT JOB 
        service_job_info, service_info_by_node = self.backend.submit_job(job_id, job_runs, workspace, compute_def, resume_name, 
...
def update_compute_def_from_cmd_options(compute_def, hold=False):
    ...
    compute_props = ["vm_size", "azure_image", "low_pri", "box_class", "docker", "setup", "boxes", 
