...
def GetApp(appName=None, *a, OtherAdminStuff=None, **k):
    ...
    app.config['SCHEDULER_JOBSTORES'] = {
        'default': SQLAlchemyJobStore(url=f'sqlite:///{appName}_jobstore.db')
    }
...
def AddJob(func, args=(), kwargs={}, name=None):
    ...
    return app.apscheduler.add_job(
        func=func,
        args=args,
        kwargs=kwargs,
        name=name,
...
def ScheduleJob(dt, func, args=(), kwargs={}, name=None):
    # dt should be in UTC
    # return flask_jobs.ScheduleJob(dt=dt, func=func, args=args, kwargs=kwargs, name=name)
    return app.apscheduler.add_job(
        next_run_time=dt,
        func=func,
        args=args,
...
def ScheduleIntervalJob(startDT=None, func=None, args=(), kwargs={}, name=None, **k):
    ...
    return app.apscheduler.add_job(
        next_run_time=startDT or datetime.datetime.utcnow(),
        func=func,
        args=args,
        kwargs=kwargs,
        name=name,
...
def RemoveJob(jobID):
    ...
    # job.Delete()
    job = app.apscheduler.get_job(jobID)
    return job.remove()
