...
class HSMNet_model(nn.Module):
    ...
    def forward(self, left, right):
        ...
        conv4, conv3, conv2, conv1 = self.feature_extraction(
            torch.cat([left, right], 0))
        ...
        if self.level > 2:
            cost3 = F.upsample(
                cost5, [
                    left.size()[2], left.size()[3]], mode='bilinear')
        else:
            feat4 = torch.cat((feat5_2x, feat4), dim=1)

            feat4_2x, cost4 = self.decoder4(feat4)  # 32
            if self.level > 1:
                cost3 = F.upsample((cost4).unsqueeze(1),
                                   [self.disp_reg8.disp.shape[1],
                                    left.size()[2],
                                    left.size()[3]],
                ...
                _, cost3 = self.decoder3(feat3)  # 32
                cost3 = F.upsample(
                    cost3, [
                        left.size()[2], left.size()[3]], mode='bilinear')
        if self.level > 2:
        ...
        if self.training:
            cost6 = F.upsample((cost6).unsqueeze(1),
                               [self.disp_reg8.disp.shape[1],
                                left.size()[2],
                                left.size()[3]],
                               mode='trilinear').squeeze(1)
            cost5 = F.upsample((cost5).unsqueeze(1),
                               [self.disp_reg8.disp.shape[1],
                                left.size()[2],
                                left.size()[3]],
                               mode='trilinear').squeeze(1)
...
class projfeat3d(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, stride):
        ...
        self.stride = stride
        self.conv1 = nn.Conv2d(in_planes, out_planes, (1, 1), padding=(
            0, 0), stride=stride[:2], bias=False)
...
class decoderBlock(nn.Module):
    def __init__(
        ...
        self.classify = nn.Sequential(
            sepConv3d(
                channelF, channelF, 3, (1, 1, 1), 1), nn.ReLU(
                    inplace=True), sepConv3d(
                        channelF, 1, 3, (1, 1, 1), 1, bias=True))
        ...
        if pool:
            self.pool_convs = torch.nn.ModuleList(
                [
                    sepConv3d(
                        channelF, channelF, 1, (1, 1, 1), 0), sepConv3d(
                            channelF, channelF, 1, (1, 1, 1), 0), sepConv3d(
                                channelF, channelF, 1, (1, 1, 1), 0), sepConv3d(
                                    channelF, channelF, 1, (1, 1, 1), 0)])
    ...
    def forward(self, fvl):
        ...
        if self.pool:
            fvl_out = fvl
            _, _, d, h, w = fvl.shape
            for i, pool_size in enumerate(np.linspace(
                    1, min(d, h, w) // 2, 4, dtype=int)):
...
class unet(nn.Module):
    def __init__(self):
        ...
        self.convbnrelu1_1 = conv2DBatchNormRelu(
            in_channels=3, k_size=3, n_filters=16, padding=1, stride=2, bias=False)
        ...
        self.pyramid_pooling = pyramidPooling(
            128, None, fusion_mode='sum', model_name='icnet')
        # Iconvs
        self.upconv6 = nn.Sequential(
            ...
            conv2DBatchNormRelu(
                ...
                stride=1,
                bias=False))
        self.iconv5 = conv2DBatchNormRelu(
            in_channels=192,
            k_size=3,
            n_filters=128,
            padding=1,
        ...
        self.upconv5 = nn.Sequential(
            ...
            conv2DBatchNormRelu(
                ...
                stride=1,
                bias=False))
        self.iconv4 = conv2DBatchNormRelu(
            in_channels=192,
            k_size=3,
            n_filters=128,
            padding=1,
        ...
        self.upconv4 = nn.Sequential(
            ...
            conv2DBatchNormRelu(
                ...
                stride=1,
                bias=False))
        self.iconv3 = conv2DBatchNormRelu(
            in_channels=128,
            k_size=3,
            n_filters=64,
            padding=1,
        ...
        self.proj6 = conv2DBatchNormRelu(
            in_channels=128,
            k_size=1,
            n_filters=32,
            padding=0,
            stride=1,
            bias=False)
        self.proj5 = conv2DBatchNormRelu(
            in_channels=128,
            k_size=1,
            n_filters=16,
            padding=0,
            stride=1,
            bias=False)
        self.proj4 = conv2DBatchNormRelu(
            in_channels=128,
            k_size=1,
            n_filters=16,
            padding=0,
            stride=1,
            bias=False)
        self.proj3 = conv2DBatchNormRelu(
            in_channels=64,
            k_size=1,
            n_filters=16,
            padding=0,
