...
from gateways.common.cs18_api_classes import CloudComputeService, AccessLink, CloudAccountCostResponse
...
from gateways.common.cs18_api_responses import (
    ...
    CloudAccountSubnetsResponse, RegionResponse)
...
class Converters:
    ...
    @staticmethod
    def _create_compute_services_response(
            cloud_compute_services: List[dict]
    ) -> List[CloudComputeService]:
        compute_services = [
            CloudComputeService(
    ...
    def create_cloud_account_response(cloud: dict) -> CloudAccountResponse:
        ...
        return CloudAccountResponse(
            ...
            spaces=cloud["spaces"],
            compute_services=Converters._create_compute_services_response(
    ...
    def _create_cloud_account_connectivity_response(conn: dict) -> Optional[CloudAccountConnectivityResponse]:
        ...
        return CloudAccountConnectivityResponse(
            virtual_network=conn.get("virtual_network"),
            subnets=CloudAccountSubnetsResponse(
                gateway_subnet=conn["subnets"].get("gateway_subnet"),
                management_subnet=conn["subnets"].get("management_subnet"),
                application_subnets=conn["subnets"].get("application_subnets")
            ) if "subnets" in conn else None
        )
