...
from gateways.common.cs18_api_requests import (
    ...
    UpdateSpaceRequest,
    ProductionEnvironment, DebuggingServiceValue, SpaceCloudAccountSettingsRequest)
...
class Colony:
    ...
    def add_cloud_account_to_space(self, cloud_account: str, settings: SpaceCloudAccountSettingsRequest = None):
        method_url = self._api_root.spaces.space_cloud_accounts(self.space)
        data = {"name": cloud_account}
        if settings:
            data['internet_facing'] = settings.internet_facing
            data['virtual_network'] = settings.virtual_network
            if settings.subnets:
                data['subnets'] = {'gateway_subnet': settings.subnets.gateway_subnet,
                                   'management_subnet': settings.subnets.management_subnet,
                                   'application_subnets': settings.subnets.application_subnets}
        res = self.session.post(url=method_url, json=data)
        GatewayUtils.handle_response(response=res, return_codes=[200])

    def update_cloud_account_in_space(self, cloud_account: str, settings: SpaceCloudAccountSettingsRequest):
        method_url = self._api_root.spaces.space_cloud_account(space_name=self.space, cloud_account=cloud_account)
        data = {
            'internet_facing': settings.internet_facing,
            'virtual_network': settings.virtual_network
        }
        if settings.subnets:
            data['subnets'] = {'gateway_subnet': settings.subnets.gateway_subnet,
                               'management_subnet': settings.subnets.management_subnet,
