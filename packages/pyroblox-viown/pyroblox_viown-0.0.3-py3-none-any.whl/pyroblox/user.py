...
class User:
    def __init__(self, requests, userid=None):
        ...
        self.authenticated = self.requests.get("https://users.roblox.com/v1/users/authenticated").json()
        self.account = self.authenticated["id"]
        self.filters = assettypes
        self.name = self.requests.get(f"https://api.roblox.com/users/{str(self.id)}").json()["Username"]
        """Name of the user"""
