...
class Cutout2D:
    ...
    def _mask(self):
        bb0x,bb1x = self.bbcorner['bb0x'],self.bbcorner['bb1x']
        bb0y,bb1y = self.bbcorner['bb0y'],self.bbcorner['bb1y']
        xg,yg = self.tbox['xg'],self.tbox['yg']
        hup,hlow = self.tbox['halfdyup'],self.tbox['halfdylow']
        tmp_m = np.full_like(self.image,False,dtype=bool)
        for i,ii in enumerate(xg):
            xx,yy = xg[i],yg[i]
            tmp_m[int(yy-hlow):int(yy+1+hup),int(xx)] = True        
        mask = tmp_m[bb0y:bb1y,bb0x:bb1x].copy()
        return mask
    ##########
    ##########
    ##########
