"""
Unit Tests meant to be run by Travis CI.
"""
...
class TestLimeTabular(unittest.TestCase):
    ...
    def test_lime_stability_indices_good_behaviour(self):
        np.random.seed(1)
        ...
        indices = explainer.check_stability(data_row=self.test[i],
        ...
        csi, vsi = indices
        self.assertTrue((csi >= 0) & (csi <= 100),
                        "CSI Index value is not in the range [0,100]")
        self.assertTrue((vsi >= 0) & (vsi <= 100),
                        "VSI Index value is not in the range [0,100]")

    def test_lime_stability_indices_model_error(self):
        np.random.seed(1)
        ...
        with self.assertRaises(LocalModelError):
            exp = explainer.check_stability(self.test[i],
                                            rf.predict_proba,
                                            num_features=2,
                                            n_calls=10,
                                            model_regressor=lin_regr)
    ...
    def testFeatureNamesAndCategoricalFeats(self):
        ...
        explainer = LimeTabularExplainerOvr(training_data=training_data)
