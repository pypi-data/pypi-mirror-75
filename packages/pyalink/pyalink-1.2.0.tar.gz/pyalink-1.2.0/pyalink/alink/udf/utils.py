...
def _to_dill_base64(v):
    import dill
    return ensure_unicode(base64.b64encode(dill.dumps(v, recurse=True)))
...
def _java_register_function(name, j_func, env_type):
    ...
    if has_pyflink():
        ...
        if env_type == "batch":
            from pyalink.alink import env
            env._mlenv.btenv.registerFunction(name, j_func)
        elif env_type == "stream":
            from pyalink.alink import env
            env._mlenv.stenv.registerFunction(name, j_func)
...
def _to_judf(name, func, result_type, class_object_type="DILL_BASE64"):
    ...
    if class_object_type == "DILL_BASE64":
        class_object = _to_dill_base64(func)
...
def _to_judtf(name, func, result_types, class_object_type="DILL_BASE64"):
    if not callable(func) and not hasattr(func, "eval"):
        raise ValueError("Invalid function: not a callable or an object with attr eval")
