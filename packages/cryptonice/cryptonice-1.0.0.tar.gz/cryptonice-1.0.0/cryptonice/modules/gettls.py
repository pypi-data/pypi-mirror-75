...
def createServerConnections(ip_address, hostname, servers_to_scan, port_to_scan):
    ...
    """
    try:
        version = sslyze.__version__.__version__
    except:
        version = sslyze.__version__

    if version <= "2.1.4":
        try:
            server_tester = sslyze.ServerConnectivityTester(
                hostname=hostname,
                port=port_to_scan,
                tls_wrapped_protocol=sslyze.ssl_settings.TlsWrappedProtocolEnum.PLAIN_TLS
            )
            server_info = server_tester.perform()
            servers_to_scan.append(server_info)
            return "success"
        except sslyze.errors.ServerConnectivityError as e:
            # Could not establish an SSL connection to the server
            raise RuntimeError(f'Could not connect to {e.server_info.hostname}: {e.error_message}')
    else:
        server_location = sslyze.ServerNetworkLocationViaDirectConnection(str(hostname), port_to_scan, ip_address)
        try:
...
def tls_scan(ip_address, str_host, commands_to_run, port_to_scan):
    ...
    for server_scan_result in scanner.get_results():
        ...
        if 'tls_1_3_early_data' in commands_to_run:  # Collect results for early data acceptance
            try:
                tls1_3_early_result = server_scan_result.scan_commands_results[sslyze.ScanCommand.TLS_1_3_EARLY_DATA]
        ...
        if 'robot' in commands_to_run:  # Collect results for robot vulnerability
            try:
                obj_robot_result = server_scan_result.scan_commands_results[sslyze.ScanCommand.ROBOT]
        ...
        if 'http_headers' in commands_to_run:
            http_info = {}
            try:
                http_results = server_scan_result.scan_commands_results[sslyze.ScanCommand.HTTP_HEADERS]
        ...
        if 'session_renegotiation' in commands_to_run:
            try:
                renegotiation_results = server_scan_result.scan_commands_results[sslyze.ScanCommand.SESSION_RENEGOTIATION]
                session_reneg = {}
        ...
        if 'session_resumption' in commands_to_run:
            try:
                session_resumption_results = server_scan_result.scan_commands_results[sslyze.ScanCommand.SESSION_RESUMPTION]
