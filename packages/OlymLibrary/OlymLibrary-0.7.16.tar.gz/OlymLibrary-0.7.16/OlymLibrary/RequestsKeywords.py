...
from OlymLibrary import OlymKeywords
import requests
import robot
...
class RequestsKeywords(object):
    ...
    def to_json(self, content, pretty_print=True):
        ...
        if pretty_print:
            self._json_pretty_print(content)
    ...
    def post_request_new(
        ...
        given `alias`
        """
        if isinstance(data,str) and (isinstance(headers,str) or  headers is None):
            session = self._cache.switch(alias)
            if headers is None:
                if self._is_json(data):
                    ...
                    data=OlymKeywords().FormData_to_Dict(data)
    ...
    def set_session_url(self,alias,url):
        """ change url
        """
        self.builtin.log('set session url: %s' % url, 'DEBUG')
        session = self._cache.switch(alias)
        session.url=url
        return session
    ...
    def Upload_Files(
        ...
        """
            ...
            Content-Disposition: form-data; *name* ="client_logo"; *filename* ="20160606173830.jpg"
            *Content-Type* : image/jpeg

            | ${files} | Set Variable | ${EXECDIR}${/}Resources${/}XXXXXX.xls |
        ...
        with open(files_path,'rb') as f:
            logger.info(type(f))
            file={name: (files_name,f, Content_Type)}
            resp = session.post(self._get_url(session, uri),files=file,data=data)

            if isinstance(data, str):
                data = data.decode('utf-8')
            logger.info('Post Request using : alias=%s, uri=%s, files=%s,'
                        % (alias, uri, files_path))
            session.last_resp = resp
            return resp
...
if __name__ == '__main__':
    a={"aaaa":"11111"}
    b={"aaaa":"11111"}
    c="bb=asda,asdasd=dddd"
    d="strCasenumbers=['25219362']&isCanclePub=true"

    print OlymKeywords().FormData_to_Dict(d)
 #   print eval(d)
