...
#include "MathsCustomFunctions.hpp"
    ...
    boost::shared_ptr<RegularStimulus> Celliribe_model_2006_without_otherwise_sectionFromCellMLCvodeDataClamp::UseCellMLDefaultStimulus()
    {
    ...
    double Celliribe_model_2006_without_otherwise_sectionFromCellMLCvodeDataClamp::GetIntracellularCalciumConcentration()
    {
        return NV_Ith_S(mStateVariables, 1);
    }
    
    Celliribe_model_2006_without_otherwise_sectionFromCellMLCvodeDataClamp::Celliribe_model_2006_without_otherwise_sectionFromCellMLCvodeDataClamp(boost::shared_ptr<AbstractIvpOdeSolver> pOdeSolver /* unused; should be empty */, boost::shared_ptr<AbstractStimulusFunction> pIntracellularStimulus)
                ...
                pIntracellularStimulus)
    {
        ...
        //
        this->mpSystemInfo = OdeSystemInformation<Celliribe_model_2006_without_otherwise_sectionFromCellMLCvodeDataClamp>::Instance();
        Init();
        ...
        NV_Ith_S(this->mParameters, 3) = 1.0; // (var_time_dependent_rectifier_potassium_current__i_Kmax) [nanoA]
        NV_Ith_S(this->mParameters, 4) = 2.5; // (var_fast_sodium_current__g_Na) [microS]
        NV_Ith_S(this->mParameters, 5) = 0; // (var_fast_sodium_current__perc_reduced_inact_for_IpNa) [dimensionless]
        NV_Ith_S(this->mParameters, 6) = 0; // (var_fast_sodium_current__shift_INa_inact) [millivolt]
        NV_Ith_S(this->mParameters, 7) = 0.00050000000000000001; // (var_sodium_calcium_exchanger__i_NaCa_max) [nanoA_per_millimolar4]
    }

    Celliribe_model_2006_without_otherwise_sectionFromCellMLCvodeDataClamp::~Celliribe_model_2006_without_otherwise_sectionFromCellMLCvodeDataClamp()
    {
    }
    
    double Celliribe_model_2006_without_otherwise_sectionFromCellMLCvodeDataClamp::GetIIonic(const std::vector<double>* pStateVariables)
    {
        ...
        const double var_fast_sodium_current__i_Na = pow(var_chaste_interface__fast_sodium_current_m_gate__m, 3) * (-var_reversal_potentials__E_mh + var_chaste_interface__membrane_potential__V) * NV_Ith_S(mParameters, 4) * var_chaste_interface__fast_sodium_current_h_gate__h; // nanoA
    ...
    void Celliribe_model_2006_without_otherwise_sectionFromCellMLCvodeDataClamp::EvaluateYDerivatives(double var_chaste_interface__environment__time_converted, const N_Vector rY, N_Vector rDY)
    {
        // Inputs:
        // Time units: millisecond
        double var_chaste_interface__membrane_potential__V = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : NV_Ith_S(rY, 0));
        ...
        const double var_fast_sodium_current_h_gate__alpha_h = 20.0 * exp(-9.375 + 0.125 * NV_Ith_S(mParameters, 6) - 0.125 * var_chaste_interface__membrane_potential__V); // per_second
        const double var_fast_sodium_current_h_gate__beta_h = 2000.0 / (1.0 + 320.0 * exp(-7.5 + 0.10000000000000001 * NV_Ith_S(mParameters, 6) - 0.10000000000000001 * var_chaste_interface__membrane_potential__V)); // per_second
        const double var_fast_sodium_current_h_gate__h_inf = 0.01 * NV_Ith_S(mParameters, 5) + (1.0 - 0.01 * NV_Ith_S(mParameters, 5)) * var_fast_sodium_current_h_gate__alpha_h / (var_fast_sodium_current_h_gate__alpha_h + var_fast_sodium_current_h_gate__beta_h); // dimensionless
        ...
        else
        {
            ...
            if (mDataClampIsOn)
            {
                var_chaste_interface__membrane_data_clamp_current = (-GetExperimentalVoltageAtTimeT(var_chaste_interface__environment__time_converted) + var_chaste_interface__membrane_potential__V) * NV_Ith_S(mParameters, 2); // uA_per_cm2
            }
    ...
    N_Vector Celliribe_model_2006_without_otherwise_sectionFromCellMLCvodeDataClamp::ComputeDerivedQuantities(double var_chaste_interface__environment__time_converted, const N_Vector & rY)
    {
        // Inputs:
        // Time units: millisecond
        double var_chaste_interface__membrane_potential__V = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : NV_Ith_S(rY, 0));
        ...
        NV_Ith_S(dqs, 5) = var_sodium_calcium_exchanger__i_NaCa_converted;
        NV_Ith_S(dqs, 6) = var_membrane_potential__i_Stim_converted;
        return dqs;
    }

template<>
void OdeSystemInformation<Celliribe_model_2006_without_otherwise_sectionFromCellMLCvodeDataClamp>::Initialise(void)
{
