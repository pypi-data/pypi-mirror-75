...
class QuestionIndexViewTests(TestCase):
    ...
    def test_question_without_choice(self):
        """
        Question without any given choices aren't displayed in the index page.
        """
        question_without_choice = create_question(
            question_text="Question without choice", days=-1)
        response = self.client.get(reverse('polls:index'))
        self.assertContains(response, 'No polls are available.')
        self.assertQuerysetEqual(question_without_choice.choice_set.all(), [])
        self.assertQuerysetEqual(response.context['latest_question_list'], [])
...
class QuesionResultsViewTests(TestCase):
    def test_future_question(self):
        """
