...
class TestStatements(TestCase):
    ...
    def test_get_with_items(self):
        count = 30
        res = self.orkg.statements.get(items=count)
        self.assertTrue(res.succeeded)
        self.assertEqual(len(res.content), count)
    ...
    def test_get_by_subject_and_predicate(self):
        subject = self.orkg.resources.add(label="ORKG Subject 1").content['id']
        predicate = self.orkg.predicates.add(label="ORKG Predicate 1").content['id']
        predicate2 = self.orkg.predicates.add(label="ORKG Predicate 2").content['id']
        object = self.orkg.literals.add(label="ORKG Object 1").content['id']
        object2 = self.orkg.literals.add(label="ORKG Object 2").content['id']
        st1 = self.orkg.statements.add(subject_id=subject, predicate_id=predicate, object_id=object)
        self.orkg.statements.add(subject_id=subject, predicate_id=predicate2, object_id=object2)
        res = self.orkg.statements.get_by_subject_and_predicate(subject_id=subject, predicate_id=predicate)
        self.assertTrue(res.succeeded, "The call to the backend didn't succeed!")
        self.assertEqual(res.content[0]['id'], st1.content['id'], "Returned statement doesn't match the expected one!")
    ...
    def test_get_by_object_and_predicate(self):
        ...
        st1 = self.orkg.statements.add(subject_id=subject, predicate_id=predicate, object_id=object)
        self.orkg.statements.add(subject_id=subject, predicate_id=predicate2, object_id=object2)
        res = self.orkg.statements.get_by_object_and_predicate(object_id=object, predicate_id=predicate)
        self.assertTrue(res.succeeded, "The call to the backend didn't succeed!")
        self.assertEqual(res.content[0]['id'], st1.content['id'], "Returned statement doesn't match the expected one!")
