...
class ResourcesClient(NamespacedClient):
    ...
    def find_contributors(self, resource_id):
        """
        find the contributors of a resource
        :param resource_id: the id of the ORKG resource to check for contributors
        :return: an OrkgResponse object containing a list of ORKG users
        """
        self.client.backend._append_slash = True
        response = self.client.backend.resources(resource_id).contributors.GET()
        return OrkgResponse(response)

    @query_params("id", "label", "classes")
    def find_or_add(self, params=None):
        """
    ...
    def save_dataset(self, file, label, dimensions):
        """
