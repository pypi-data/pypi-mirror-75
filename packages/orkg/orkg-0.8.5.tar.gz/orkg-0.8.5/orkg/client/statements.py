...
class StatementsClient(NamespacedClient):
    ...
    @query_params("page", "items", "sortBy", "desc")
    def get_by_subject_and_predicate(self, subject_id, predicate_id, params=None):
        if len(params) > 0:
            self.client.backend._append_slash = False
            response = self.client.backend.statements.subject(subject_id).predicate(predicate_id).GET(dict_to_url_params(params))
        else:
            self.client.backend._append_slash = True
    ...
    @query_params("page", "items", "sortBy", "desc")
    def get_by_object_and_predicate(self, object_id, predicate_id, params=None):
        if len(params) > 0:
            self.client.backend._append_slash = False
