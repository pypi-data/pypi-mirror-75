...
if sys.version_info >= (3, 8):
    ...
    logger.info(f"Python {sys.version_info} == 3.6.x. Defining get_origin, get_args ")
    # 3.6 `List.__origin__ == List`, `Optional` does not have `__dict__`
    get_origin, get_args = lambda tp: vars(tp).get('__orig_bases__')[0], lambda tp: getattr(tp, '__args__') if hasattr(tp, '__args__') else ()  # noqa: E731
else:
    try:
...
class ArgSuite(Generic[ArgType]):
    ...
    def _gen_cmd_argv(self, args: ArgType, prefix) -> Iterable[str]:
        for name, arg in args._asdict().items():
            if arg is None:
                assert get_origin(args.__class__._field_types[name]) == Union, f"Field {name} should not be None"
            else:
