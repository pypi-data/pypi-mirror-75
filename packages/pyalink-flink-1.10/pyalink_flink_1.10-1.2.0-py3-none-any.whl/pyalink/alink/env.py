...
from .common.utils.packages import get_alink_lib_path, get_pyflink_path, has_pyflink

if has_pyflink():
    __all__ = ["useLocalEnv", "useRemoteEnv", "useCustomEnv", "getMLEnv", "resetEnv"]
else:
    __all__ = ["useLocalEnv", "useRemoteEnv", "getMLEnv", "resetEnv"]
...
def _add_default_configurations(configuration, parallelism: int):
    if has_pyflink():
...
def _make_j_configuration(config, parallelism: int):
    configuration = get_java_gateway().jvm.org.apache.flink.configuration.Configuration()
    _add_default_configurations(configuration, parallelism)
...
def setup_py_ml_env(gateway: JavaGateway, j_benv: object, j_senv: object):
    if has_pyflink():
        ...
        j_btenv = get_java_class("org.apache.flink.table.api.java.BatchTableEnvironment").create(j_benv)
        j_stenv = get_java_class("org.apache.flink.table.api.java.StreamTableEnvironment").create(j_senv)
        j_mlenv = gateway.jvm.com.alibaba.alink.common.MLEnvironment(j_benv, j_btenv, j_senv, j_stenv)
        gateway.jvm.com.alibaba.alink.common.MLEnvironmentFactory.setDefault(j_mlenv)
        mlenv = MLEnv(j_benv, j_btenv, j_senv, j_stenv)
    return mlenv
...
def useLocalEnv(parallelism: int, flinkHome: str = None, config: object = None) -> MLEnv:
    ...
    if in_custom_env():
        ...
        return
    resetEnv()
    g_config["alink_deps_dir"] = get_alink_lib_path()
    g_config["flink_home"] = get_pyflink_path() if flinkHome is None else flinkHome
    if config is not None and "debug_mode" in config:
        g_config["debug_mode"] = config["debug_mode"]
    j_configuration = _make_j_configuration(config, parallelism)
...
def useRemoteEnv(host: str, port: Union[int, str], parallelism: int, flinkHome: str = None,
    ...
    g_config["alink_deps_dir"] = alink_lib_path
...
def getMLEnv() -> MLEnv:
    ...
    if _mlenv is None:
        if has_pyflink():
            ...
            raise Exception(
                "Call useLocalEnv/useRemoteEnv to initialize MLEnv first, or update to a newer version of PyAlink which depends on PyFlink-1.10 to use PyFlink env.")
    return _mlenv
