...
def dFF_generic(data, signal='Gcamp', reference='Isosbestic',
                freq=12, sample_rate=1220, filter_order=2):
    """
    ...
    inputs
      ...
      kwargs:
        ...
        sample_rate: int for sampe rate (samples/second) of the data;
                     default = 1220 samples/second
...
def dFF_runningmean(data, rm_window, signal='Rcamp', freq=12,
                    sample_rate=1220, filter_order=2):
    """
    Calculates dF/F for a given signal and reference, by filtering the signal
    and the reference, scaling the reference to the signal, and calculating
    (filter_signal-scaled_reference)/scaled_reference.
