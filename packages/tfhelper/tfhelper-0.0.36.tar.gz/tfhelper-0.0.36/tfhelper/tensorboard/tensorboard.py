...
class ModelSaverCallback(tf.keras.callbacks.Callback):
    ...
    def __init__(self, best_metric=float('inf'), save_root="./", save_metric='val_loss',
                metric_type="loss",file_name="my_model", enable=True, epoch=0, include_optimizer=False):
        """
...
def get_tf_callbacks(root,
                     ...
                     modelsaver_callback=False, best_loss=float('inf'), save_root=None, best_epoch=0, save_metric='val_loss', save_file_name="my_model", metric_type="loss",
    ...
    if modelsaver_callback:
        if not save_root:
            save_root = log_root_
        callbacks_.append(
            ModelSaverCallback(best_metric=best_loss, save_root=save_root, epoch=best_epoch, save_metric=save_metric, file_name=save_file_name, metric_type=metric_type)
        )
