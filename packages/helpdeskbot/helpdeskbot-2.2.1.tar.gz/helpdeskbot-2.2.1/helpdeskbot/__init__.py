...
class HelpDeskBot:
    ...
    def setup(self):
        ...
        async def lmgtfy(context):
            # TODO it only strips lmgtfy otherwise it will search for "google this" literally
            search_term = context.message.content.strip("".join(list(self.BOT_PREFIX)))
            for alias in ['lmgtfy', 'google', 'search']:
                search_term = search_term.replace(alias, "")
