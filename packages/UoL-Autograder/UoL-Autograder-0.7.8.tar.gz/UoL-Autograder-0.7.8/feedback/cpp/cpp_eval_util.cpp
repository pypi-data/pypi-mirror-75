...
void Results::output(char filename[])
{
    ...
    string result = json_result[0].dump(4);
    o << result;
}

void Results::output(const char* filename){
    std::ofstream o(filename);
    nlohmann::json json_result = {_test_results};
    string result = json_result[0].dump(4);
    o << result;
...
void Results::output(const char* filename, const char* validator_filename, std::string secret){
    std::ofstream o(filename);
    std::ofstream v(validator_filename);
    nlohmann::json json_result = {_test_results};
    string result = json_result[0].dump(4);
    
    string secret_result(result + secret);
    stripUnicode(secret_result);
    v << sha256(secret_result);
