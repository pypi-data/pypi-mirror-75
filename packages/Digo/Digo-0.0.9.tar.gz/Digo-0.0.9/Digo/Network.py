...
def upload_file_to_main(file, api_key, account_id, file_type):
    conn = requests.post(DIGO_WEB + "/UploadFileForModule", params={ 'api_key': api_key, 'account_id': account_id, 'file_type':file_type}, files={"file":file})
...
def uploadModelFile(api_key, account_id,project_id, experiment_name, sas_token, container_name, project_name, experiment_id, model_path):
    ...
    blob_id = upload_file_to_main(file=open(convert_full_path, 'rb'), account_id= account_id, api_key=api_key, file_type=FILETYPE.MODEL)
...
def uploadImageLog(api_key, account_id, sas_token, project_id, experiment_id, container_name, project_name, experiment_name, imageLog):
    ...
    for key in imageLog:
        ...
        result_image_log[key] = upload_file_to_main(file=open(full_path, 'rb'), api_key=api_key, account_id=account_id, file_type=FILETYPE.IMAGE)
