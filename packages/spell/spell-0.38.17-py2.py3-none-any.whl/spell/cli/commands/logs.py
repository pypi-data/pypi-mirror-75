    ...
    with api_client_exception_handler():
        ...
        try:
            log_printer = LogPrinter(
                utf8_enabled=utf8, delay=delay, verbose=verbose or not sys.stdout.isatty(),
            )
            status = ""
            ...
            if stop_status is not None and status != stop_status:
                ...
                click.echo(with_emoji(u"ℹ️ ", msg, utf8))
        except KeyboardInterrupt:
            if stop_status is not None:
                raise click.Abort()
            click.echo()
            if run_warning:
                click.echo(with_emoji(u"✨", "Your run is still running remotely.", utf8))
                click.echo(
                    with_emoji(
                        u"✨", "Use 'spell kill {}' to terminate your run".format(run_id), utf8
                    )
                )
            click.echo(
                with_emoji(u"✨", "Use 'spell logs {}' to view logs again".format(run_id), utf8)
            )
...
class LogPrinter:
    ...
    def __init__(self, utf8_enabled=True, delay=False, verbose=False):
        ...
        self.wait_spinner = self.star_spinner if utf8_enabled else self.ascii_spinner
