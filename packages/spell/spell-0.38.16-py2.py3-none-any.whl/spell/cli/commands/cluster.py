...
@click.option(
    ...
    Because Spell sets up your cluster in a particular manner this option is only likely to work with clusters created
    exactly the way we set ours up. This flag is likely only valuable if you experienced an error the first
    time you tried to run this command, but the kube cluster creation succeeded.""",
)
@click.option(
    "--aws-zones",
    type=str,
...
@click.command(
    name="scale", short_help="Adjust minimum and maximum node counts for a given serving group",
)
@click.pass_context
@cluster_utils.pass_cluster
@click.option(
...
def serving_group_delete(
    ...
    active_servers = [
        server for server in serving_group.model_servers if server.status in ["running", "starting", "updating"]
    ]
    ...
    if len(active_servers) > 0:
        ...
        for model_server in active_servers:
            spell_client.stop_model_server(model_server.server_name, model_server.tag)
