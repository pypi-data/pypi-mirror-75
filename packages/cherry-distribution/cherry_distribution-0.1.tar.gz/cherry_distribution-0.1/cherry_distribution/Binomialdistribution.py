...
class Binomial(Distribution):
    ...
    def __init__(self, prob=.5, size=20):
        ...
        self.n = size
        mean = self.p*self.n
        stdv = math.sqrt(self.n * self.p * (1 - self.p))
        Distribution.__init__(self, mean, stdv)  
    ...
    def replace_stats_with_data(self):
        ...
        #           defined previously.
        success_count = self.data.count(1)
        self.n = len(self.data) 
        self.p = success_count/self.n
    ...
    def plot_bar(self):
        ...
        success_count = self.data.count(1)
        plt.bar(x = ['0', '1'], height = [self.n - success_count, succes_count])
        plt.title('Histogram')
        plt.xlabel('outcome')
        plt.ylabel('count')
    ...
    def plot_bar_pdf(self):
        ...
        """
        # The pdf is the proba to get k success out of n trials k ranges from 0 to n
        ...
        for i in range(self.n + 1):
            # range(self.n) goes from 0 to n-1. We need to go to n
            x.append(i) # the nunber of successes
            y.append(self.pdf(i)) # prob of getting this proba 
    ...
    def __add__(self, other):
        ...
        binomial = Binomial(prob=self.p, size=new_n)
    ...
    def __repr__(self):
        ...
        return "Mean {mean}, standard deviation {stdv}, p {p}, n {size}".format(mean = self.mean, stdv = self.stdev, p = self.p, size = self.size)
