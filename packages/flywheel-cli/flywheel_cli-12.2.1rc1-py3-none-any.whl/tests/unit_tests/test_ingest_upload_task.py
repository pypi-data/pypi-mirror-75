...
def test_create_packfile_tempfile(attr_dict, upload_task_with_walker_mock):
    upload_task_with_walker_mock.worker_config = config.WorkerConfig(max_tempfile=0)

    ctx = attr_dict({"packfile": {"type": "zip"}})

    tmpfile, _ = upload_task_with_walker_mock.create_packfile(
        context=ctx, filename="filename", files=["file1", "file2"], subdir="subdir"
    )
    ...
    tmpfile, _ = upload_task_with_walker_mock.create_packfile(
        context=ctx, filename="filename", files=["file1", "file2"], subdir=None
    )

    assert isinstance(tmpfile, type(tempfile.SpooledTemporaryFile()))


def test_create_packfile_wo_subdir(attr_dict, upload_task_with_walker_mock):
    ctx = attr_dict({"packfile": {"type": "zip"}})
...
def test_create_packfile_w_subdir(attr_dict, upload_task_with_walker_mock):
    ctx = attr_dict({"packfile": {"type": "zip"}})
...
def test_create_packfile_flatten(attr_dict, upload_task_with_walker_mock):
    ctx = attr_dict({"packfile": {"type": "zip", "flatten": True}})
...
def test_run_packfile_safe_filename(mocker, upload_task_with_walker_mock, sdk_mock):
    ...
    sdk_mock.upload.assert_called_once_with(
        ...
        "safe_filename",
        TempFileObject,
        {"name": "safe_filename", "zip_member_count": 2, "type": "zip"},
    )
...
def test_run_single_file_safe_filename(mocker, upload_task_with_walker_mock, sdk_mock):
    ...
    sdk_mock.upload.assert_called_once_with(
        "group", "id_val", "safe_filename", file_mock, None
    )
...
def setup_upload_task_with_walker_mock(
    ...
    if get_item is not None:
        ...
        db_mock.get_item.return_value = T.ItemOut(
            id=uuid4(),
            dir="dir",
            type="file",
            files=["file"],
    ...
    if get_container is not None:
        ...
        db_mock.get_container.return_value = T.ContainerOut(
            id=container_id,
            level=0,
            path="label_value",
            src_context={"label": "label_value"},
