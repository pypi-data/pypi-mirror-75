...
def test_progress(mock_request, client):
    response = {
        "scans": {"scanned": 1, "pending": 2,},
        "items": {"finished": 8, "total": 9,},
        "files": {"scanned": 1, "failed": 4, "canceled": 5, "total": 9,},
        "bytes": {"scanned": 1, "pending": 2, "completed": 6, "skipped": 7,},
    }
    mock_request.return_value = MockResponse(response, 200)
...
def test_tree(mock_request, client):
    ...
    assert len(tree) == 1
    tree = tree[0]
    assert isinstance(tree, T.ContainerOut)
