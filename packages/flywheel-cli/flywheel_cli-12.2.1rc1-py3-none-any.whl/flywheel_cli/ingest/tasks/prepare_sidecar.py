...
class PrepareSidecarTask(PrepareTask):
    ...
    def _create_container(self, container, ancestors):
        ...
        super()._create_container(container, ancestors)
        if container.level.name == "project":
            orig_project_id = self.original_project.dst_context["_id"]
            side_project_id = container.dst_context["_id"]
