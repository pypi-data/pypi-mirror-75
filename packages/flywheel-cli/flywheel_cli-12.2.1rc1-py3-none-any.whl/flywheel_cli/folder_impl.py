...
class FSWrapper(Uploader, ContainerResolver):
    ...
    def path_el(self, container):
        if container.container_type == "group":
            return str_to_filename(container.id)
        return str_to_filename(container.label)

    def resolve_path(self, container_type, path):
        # Resolve folder
        path = os.path.join(*[str_to_filename(part) for part in path])
    ...
    def create_container(self, parent, container):
        ...
        if parent:
            ...
            path = fs.path.join(parent_path, str_to_filename(container.label))
        else:
            path = str_to_filename(container.id)  # Group id
