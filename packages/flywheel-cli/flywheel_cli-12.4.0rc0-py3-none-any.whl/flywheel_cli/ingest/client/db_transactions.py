...
def get_progress(db: Session, ingest_id: UUID) -> T.Progress:
    ...
    scan_tasks_by_status = (
        db.query(M.Task.status, sqla.sql.func.count(M.Task.id).label("count"),)
        .filter(
...
def get_report(db: Session, ingest_id: UUID) -> T.Report:
    ...
    return T.Report(status=ingest.status, elapsed=elapsed, errors=task_errors,)
...
def resolve_subject(db: Session, ingest_id: UUID, map_values: typing.List[str]) -> str:
    ...
    subject = (
        db.query(M.Subject)
        .filter(M.Subject.ingest_id == ingest_id, M.Subject.map_values == map_values,)
        .first()
    )
    if subject is None:
...
def load_subject_csv(
    ...
    for line in subject_csv:
        ...
        db.add(M.Subject(ingest_id=ingest_id, code=code, map_values=map_values,))
    sqla.orm.attributes.flag_modified(ingest, "config")
    db.flush()
...
def one_container_multiple_uid_item_ids(
    ...
    query = (
        ...
        .join(M.UID, uid_container_column == sub_query.columns.container_column,)
...
def _cancel_pending_tasks(db: Session, ingest_id: UUID) -> None:
    """Cancel all pending tasks"""
    pending_tasks = _for_update(
        db.query(M.Task).filter(
            M.Task.ingest_id == ingest_id, M.Task.status == T.TaskStatus.pending,
        )
    )
