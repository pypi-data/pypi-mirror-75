...
#Packaging Notes: Changed the sword image loading to a modified getImage function.
...
class Sword(pygame.sprite.DirtySprite):
    ...
    def __init__(self, x_pos, y_pos, rotation, direction):
        ...
        self.image = self.getImage("Resources/Images/MontoyaSword.png")
        self.rect = Rect(x_pos, y_pos, 67, 9)

    def getImage(self, path):
        canonicalized_path = path.replace('/', os.sep).replace('\\', os.sep)
        image = pygame.image.load(os.path.join(os.path.dirname(os.path.realpath(__file__)), canonicalized_path)).convert_alpha()
        return image
