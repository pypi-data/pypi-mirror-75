...
import time
from duel.tile import Tile

#Packaging Notes: Changed all image loading to use getImage from the original package. Changed update() and initialization image.
...
class Player(pygame.sprite.DirtySprite):
    ...
    def __init__(self, x_pos, y_pos, direction_facing, sword_height, is_ghost, is_locked_on, image_dict, win_direction):
        ...
        self.image = self.getImage(self.getSprite(), self._image_dict)
    ...
    def getImage(self, path, _image_library):
        if path not in _image_library:
            ...
            image = pygame.image.load(
                os.path.join(os.path.dirname(os.path.realpath(__file__)), canonicalized_path)).convert_alpha()
