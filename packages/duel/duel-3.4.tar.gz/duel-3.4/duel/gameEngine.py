...
"""
import duel.mapSelectionList
import sys
from duel.player import Player
from duel.sword import Sword
import pygame
from pygame import Rect
from duel.playerSkinsList import getSkin
import duel.gameFrame
from duel.swordHitBoxes import getSwordLine
from time import time
import duel.mainMenuFrame
from duel.camera import Camera
import math
...
def mainMenu(screen , audio):  # TODO Call Main Menu Frame instead and have it call startGame
    duel.mainMenuFrame.mainMenu(screen, audio)
...
def startGame(screen, map_selection, skin_selection1, skin_selection2 , audio):
    ...
    current_map = duel.mapSelectionList.selectMap(map_selection)  # returns a child of the map class
    ...
    player1 = Player(current_map.getP1InitialPosition()["X"], current_map.getP1InitialPosition()["Y"], 1, 2, False, True, getSkin(skin_selection1), 1)  # Initializes player1 -50, 100
    player2 = Player(current_map.getP2InitialPosition()["X"], current_map.getP2InitialPosition()["Y"], 1, 2, False, False, getSkin(skin_selection2), -1)  # Initializes player2 675, 100

    display, camera = duel.gameFrame.init(current_map)
    ...
    while active_match:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_ESCAPE:
                    duel.mainMenuFrame.pauseMenu(screen, p1_meta_info, p2_meta_info, audio)
        ...
        timer = time()
        #render
        duel.gameFrame.render(display, screen, player1, player2, entities, camera, swords, current_map.assets_dict)
        if player1.getPlayerState("player_won") or player2.getPlayerState("player_won"):
            break

    if player1.getPlayerState("player_won"):
        duel.mainMenuFrame.endGameMenu(screen, audio, 1)
    elif player2.getPlayerState("player_won"):
        duel.mainMenuFrame.endGameMenu(screen, audio, 2)
