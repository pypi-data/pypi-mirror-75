...
class UrllibInstrumentationTest(TestCase):
    ...
    def assert_span(self, status, prefix=False):
        ...
        if prefix:
            assert span_data['props']['REQUEST_BODY'] == ("test" if PY3 else "")
            assert span_data['props']['REQUEST_SIZE_BYTES'] == ("4" if PY3 else "0")
            if PY3:
                assert "X-foo" in span_data['props']['REQUEST_HEADERS']
            else:
                assert span_data['props']['REQUEST_HEADERS']
            assert span_data['props']['RESPONSE_BODY'] == ""
            assert span_data['props']['RESPONSE_SIZE_BYTES'] == "0"
            assert "X-Foo" in span_data['props']['RESPONSE_HEADERS']
