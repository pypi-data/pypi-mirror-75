...
class RequestInstrumentationTest(TestCase):
    ...
    def assert_span(self, method, status, prefix=False):
        ...
        if prefix:
            assert span_data['props']['REQUEST_BODY'] == '{"foo": "bar"}'
            assert span_data['props']['REQUEST_SIZE_BYTES'] == "14"
            assert "X-Foo" in span_data['props']['REQUEST_HEADERS']
            assert span_data['props']['RESPONSE_BODY'] == "test"
            assert span_data['props']['RESPONSE_SIZE_BYTES'] == "4"
            assert "Foo" in span_data['props']['RESPONSE_HEADERS']
