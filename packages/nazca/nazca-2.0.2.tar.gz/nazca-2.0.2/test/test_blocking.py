...
# Make sure tests are repeatable / Minhashing
random.seed(6)
...
class KeyBlockingTest(unittest.TestCase):
    ...
    def test_soundex_blocks(self):
        blocking = SoundexBlocking(ref_attr_index=1, target_attr_index=1, language="english")
        blocking.fit(SOUNDEX_REFSET, SOUNDEX_TARGETSET)
        blocks = list(blocking.iter_id_blocks())
        self.assertEqual(len(blocks), 3)
        self.assertIn((["a1", "a7"], ["b3", "b6"]), blocks)
...
class NGramBlockingTest(unittest.TestCase):
    ...
    def test_ngram_blocks_2(self):
        refset = [["3", "ccdd", "aabb"], ["4", "ccdd", "bbaa"]]
        targetset = [["c", "ccdd", "aabb"], ["d", "ccdd", "bbaa"]]
        true_pairs = [("3", "c"), ("4", "d")]
        blocking = NGramBlocking(ref_attr_index=2, target_attr_index=2, ngram_size=2, depth=1)
...
class SortedNeighborhoodBlockingTest(unittest.TestCase):
    def test_sorted_neighborhood_blocks(self):
        blocking = SortedNeighborhoodBlocking(ref_attr_index=1, target_attr_index=1, window_width=1)
        blocking.fit(SOUNDEX_REFSET, SOUNDEX_TARGETSET)
        blocks = list(blocking.iter_id_blocks())
        true_blocks = [
            (["a6"], ["b7", "b5"]),
...
class MergeBlockingTest(unittest.TestCase):
    def test_merge_blocks(self):
        blocking = MergeBlocking(
            ref_attr_index=1, target_attr_index=None, score_func=lambda x: x[2]
        )
        refset = [
        ...
        targetset = [("Paris (Texas)", 25000), ("Paris (France)", 12000000)]
        true_blocks = [
            (
                ["http://fr.wikipedia.org/wiki/Paris", "http://fr.wikipedia.org/wiki/Saint-Malo"],
    ...
    def test_merge_blocks_targetset(self):
        blocking = MergeBlocking(
            ref_attr_index=None, target_attr_index=2, score_func=lambda x: x[1]
        )
        refset = [("Paris (Texas)", 25000), ("Paris (France)", 12000000)]
        targetset = [
            ("http://fr.wikipedia.org/wiki/Paris_%28Texas%29", 25898, "Paris"),
...
class MinHashingBlockingTest(unittest.TestCase):
    def test_minhashing(self):
        refset = [
            ["V1", "label1", "Un nuage flotta dans le grand ciel bleu."],
            ["V2", "label2", "Pour quelle occasion vous êtes-vous apprêtée ?"],
            ["V3", "label3", "Je les vis ensemble à plusieurs occasions."],
            ["V4", "label4", "Je n'aime pas ce genre de bandes dessinées tristes."],
            ["V5", "label5", "Ensemble et à plusieurs occasions, je les vis."],
        ]
        targetset = [
            ["T1", "labelt1", "Des grands nuages noirs flottent dans le ciel."],
            ["T2", "labelt2", "Je les ai vus ensemble à plusieurs occasions."],
            ["T3", "labelt3", "J'aime les bandes dessinées de genre comiques."],
        ]
...
class PipelineBlockingTest(unittest.TestCase):
    def test_pipeline_blocking(self):
        ...
        blocking_1 = NGramBlocking(ref_attr_index=1, target_attr_index=1, ngram_size=2, depth=1)
        blocking_2 = NGramBlocking(ref_attr_index=2, target_attr_index=2, ngram_size=2, depth=1)
