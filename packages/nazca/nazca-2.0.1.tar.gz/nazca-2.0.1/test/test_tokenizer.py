...
class TokenizerTest(unittest.TestCase):
    ...
    def test_richstringtokenizer_minsize(self):
        text = "Hello everyone, this is   me speaking. And me."
        tokenizer = RichStringTokenizer(text, token_min_size=2, token_max_size=3)
        tokens = list(tokenizer)
        self.assertEqual(len(tokens), 10)
        t1 = Token(
            word="me speaking", start=26, end=37, sentence=Sentence(indice=0, start=0, end=38)
        )
    ...
    def test_richstringtokenizer_sentences(self):
        ...
        self.assertEqual(
            text[sentences[0].start : sentences[0].end], "Hello everyone, this is   me speaking."
        )
