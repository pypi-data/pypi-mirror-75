...
def demo_0():
    ...
    tr_name = {
        "normalization": [lambda x: remove_after(x, "("), aln.simplify],
    ...
    align(alignset, targetset, 0.4, processings, dpath("demo0_results"))
...
def demo_1():
    ...
    targetset = parsefile(dpath("FR.txt"), indexes=[0, 1, (4, 5)], nbmax=2000)
...
def demo_2():
    targetset = parsefile(
        dpath("FR.txt"), indexes=[0, 1, (4, 5)], formatopt={1: lambda x: x.decode("utf-8")}
    )
    alignset = parsefile(
        dpath("frenchbnf"),
        indexes=[0, 2, (14, 12)],
