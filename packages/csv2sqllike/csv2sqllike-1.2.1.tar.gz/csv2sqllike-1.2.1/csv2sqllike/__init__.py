...
def get_data_from_csv(file_path: str, sep=',', dtype=None, encoding='utf-8', line_terminator=None) -> PsuedoSQLFromCSV:
    pseudo = PsuedoSQLFromCSV(file_path, sep, dtype, encoding, line_terminator)
    return pseudo
...
def to_list_from_df(df: pd.DataFrame, sep=',', dtype=None, encoding='utf-8'):
    ...
    tmp_pseudo_sql = get_data_from_csv(
        "./tmp_tmp.csv", sep=sep, dtype=dtype, encoding=encoding, line_terminator=";;;;;\n")
