...
class Path:
	...
	def rename(self, new_name):
		...
		:rtype: Path
		"""
		if isinstance(new_name, Path):
			new_name = new_name.name_and_extension

		shutil.move(self.absolute_path, (self.parent_directory + new_name).absolute_path)
		self._string = self._string[:-len(self.name_and_extension)] + new_name
		return self
	...
	def copy(self, new_path):
		...
		"""
		if not self.exists():
			raise FileExistsError(f'{self} does not exist!')

		new_path = Path(new_path)
		if new_path.exists() and new_path.is_directory() and self.is_file():
			new_path = new_path + self.name_and_extension
	...
	def get_parent_directory(self, absolute=False):
		...
		if absolute:
			...
			if len(string_except_name_and_extension) > 0:
				return self.__class__(string=string_except_name_and_extension)
			else:
