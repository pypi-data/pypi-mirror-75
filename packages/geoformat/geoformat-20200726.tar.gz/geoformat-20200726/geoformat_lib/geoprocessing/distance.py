...
from geoformat_lib.geoprocessing.connectors.predicates import point_intersects_bbox
...
def point_vs_segment_distance(point, segment, distance_function=euclidean_distance):
    ...
    perpendicular_line_parameters = perpendicular_line_parameters_at_point(segment_line_parameters, point)
    # get intersection
    intersection_point = crossing_point_from_lines_parameters(segment_line_parameters,
                                                                        perpendicular_line_parameters)
    # get segment bbox
    segment_bbox = segment_to_bbox(segment)
    # if point is in bbox (we use bbox for floating point precision consideration)
    if point_intersects_bbox(intersection_point, segment_bbox):
