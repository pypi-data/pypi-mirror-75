...
#             self.loss = self.loss_function(self.preds_prop, torch.cuda.FloatTensor(self.labels.float()))
    ...
    def model_save(self, best, track):
        ...
        if model_improved:
            ...
            if track:
                self.to_be_logged = {
                    'params': [('epoch', self.current_epoch), 
                               ('loss_function', self.loss_function),
                               ('scheduler', self.scheduler)],
                               
                    'metrics': [(key, self.metrics[key][-1]) for key in self.metrics]
                }
    ...
    def loop(self, dataset, best='val_acc', reset_epoch_cnt=True, track=True):
        ...
        if track:
            with mlflow.start_run(run_name=self.model_name) as run:
                for param_name, param_value in self.to_be_logged['params']:
                    mlflow.log_param(param_name, param_value)
                    
                for metric_name, metric_value in self.to_be_logged['metrics']:
