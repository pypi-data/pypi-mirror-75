...
class AnchorTarget(object):
    ...
    def _build_single_target(self, anchors, valid_flags, gt_boxes, gt_class_ids):
        ...
        ---
            ...
            gt_boxes: 真实标注框
            操作流程：
            1.生成跟anchors数量相同的-1标签，-1表示中立样本
            2.计算anchors和真实标注框的交并比对比结果
            3.根据交并比结果筛选正样本和负样本，IOU>0.7正样本，<0.3负样本，其余中立样本
            4.去掉多余的正样本和负样本，正样本不超过128个，正负样本一共256个
            5.计算anchors与真实标注框的(dy,dx,dh,dw)误差
            6.分配分类任务中和回归任务中的权重，分类任务训练正负样本，回归任务只训练正样本
        Returns
        ---
            labels: [num_anchors]
        ...
        # 例如overlaps的shape可能为(261888,6)，261888是anchors数量，6是真实标注框数量
        overlaps = iou.compute_overlaps(anchors, gt_boxes)

        # 取261888个候选框与真实标注框的最大IOU所在索引
        anchor_iou_argmax = tf.argmax(overlaps, axis=1)


        # 取261888个候选框与真实标注框的最大IOU
