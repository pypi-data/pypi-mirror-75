...
class BBoxHead(tf.keras.Model):
    def __init__(self, num_classes, 
        ...
        # self.pool_size默认值为7*7，也就是把7*7的特征图卷积后变成1*1
        # self.rcnn_class_conv1 = layers.Conv2D(1024, self.pool_size, kernel_initializer=init,kernel_regularizer=tf.keras.regularizers.l2(0.0005),
        #                                       padding='valid', name='rcnn_class_conv1')


        # BN层
        # self.rcnn_class_bn1 = layers.BatchNormalization(epsilon=1.001e-5, name='rcnn_class_bn1')
        # 再做一个1*1卷积
        # self.rcnn_class_conv2 = layers.Conv2D(1024, (1, 1), kernel_initializer=init,kernel_regularizer=tf.keras.regularizers.l2(0.0005),
        #                                       name='rcnn_class_conv2')
        # BN层
        # self.rcnn_class_bn2 = layers.BatchNormalization(epsilon=1.001e-5, name='rcnn_class_bn2')

        # center:True
        # decay:0.9990000128746033
