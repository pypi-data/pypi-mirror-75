...
class ProposalTarget(object):
    ...
    def _build_single_target(self, proposals, gt_boxes, gt_class_ids, img_shape, batch_ind):
        ...
        # 得到数值不为0的真实标注框，不过一般情况下真实标注框都是不为0的
        gt_boxes, non_zeros = trim_zeros(gt_boxes)

        # 根据真实标注框的情况筛选真实标注框的标注
        ...
        # top_iou_ind = tf.nn.top_k(roi_iou_max,k=10).indices[:]
        # top_iou = tf.gather(roi_iou_max,top_iou_ind)
        ...
        print('positive_count:',positive_count,'  negative_count:',negative_count,'  P',P)
        ...
        # # 把-1都变成0，tf.tensor_scatter_nd_update取-1位置会报错
        # ids = tf.where(ids == -1,
        #                 tf.zeros((ids.shape),dtype=ids.dtype),
        #                 ids)
        
        # # 把tile_delta_targets的ids位置的数据更新为delta_targets
        # # 把候选框所对应的真实标注框种类的位置更新为候选框与真实标注框的误差delta_targets
        # # 虽然数据集一共有num_classes个种类，但其实在训练的时候一个候选框只对应某一个种类的真实标注框，
        # #   所以其他种类跟这个候选框的误差都可以看成是0。
        # # 执行前delta_targets的shape为(256,4)
        # # 执行后delta_targets的shape为(256,num_classes,4)
