...
class FasterRCNNModel():
    ...
    def fit(self, generator):
        # 定义logger
        self.logger = setup_logger(self.description, self.log_dir, 0)
        # 定义log文件位置
        ...
        for inputs in generator:
            ...
            if steps % self.cfg.SOLVER.LOG_PERIOD == 0:
                # 打印一下loss变化
                self.logger.info('steps:{}, loss:{}'.format(steps,np.mean(loss_history)))
    ...
    # 预测
    def predict(self, inputs, img_metas=None):
