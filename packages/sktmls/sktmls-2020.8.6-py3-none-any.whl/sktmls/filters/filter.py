...
class FilterClient(MLSClient):
    ...
    def __init__(
        ...
        ```
        filter_client = FilterClient(env=MLSENV.STG, username="mls_account", password="mls_password")
        ```
        """
    ...
    def get_filter(self, type: str = None, id: int = None, name: str = None) -> Filter:
        ...
        ```
        filter_by_id = filter_client.get_filter(
            id=3
        )
        filter_by_name = filter_client.get_filter(
            name="hello"
        )
        ```
        """
