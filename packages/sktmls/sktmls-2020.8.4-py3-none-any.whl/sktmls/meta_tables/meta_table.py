...
from sktmls import MLSClient, MLSENV, MLSResponse, MLSClientError
...
class MetaTableClient(MLSClient):
    ...
    def __init__(
        ...
        ```
        meta_table_client = MetaTableClient(env=MLSENV.STG, username="mls_account", password="mls_password")
        ```
        """
    ...
    def get_meta_table(self, id: int = None, name: str = None) -> MetaTable:
        ...
        if len(meta_tables) != 1:
            raise MLSClientError(code=404, msg="메타 테이블이 없거나 여러개 존재합니다.")
    ...
    def update_meta_item(
        self, meta_table: MetaTable, meta_item: MetaItem, item_name: str, item_dict: Dict[str, Any],
    ) -> MetaItem:
        """
        메타 아이템을 수정합니다.
    ...
    def get_meta_item(self, meta_table: MetaTable, id: int = None, name: str = None) -> MetaItem:
        ...
        meta_items = self.list_meta_item(meta_table, id=id, name=name)
        if len(meta_items) != 1:
            raise MLSClientError(code=404, msg="메타 아이템이 없거나 여러개 존재합니다.")
        return meta_items[0]
    ...
    def create_meta_items(self, meta_table: MetaTable, item_dict: Dict[str, Any]) -> List[MetaItem]:
        """
        ...
        - meta_table: (`sktmls.meta_tables.MetaTable`) 메타 아이템이 포함될 메타 테이블 객체
        - item_dict: (dict) Bulk로 추가할 메타 아이템 정보
        ...
        """
        assert type(meta_table) == MetaTable
        assert len(item_dict) <= 1000, "Bulk로 추가할 meta_items은 한번에 최대 1,000건 까지 지원합니다."

        data = {"items": item_dict}
