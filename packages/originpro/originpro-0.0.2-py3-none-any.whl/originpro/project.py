...
from .utils import get_file_ext, active_obj
...
_GETPAGES = {
    _WBOOK_TYPE: po.GetWorksheetPages,
    _MBOOK_TYPE: po.GetMatrixPages,
    _GPAGE_TYPE: po.GetGraphPages,
}
...
def _is_good_sn(str):
    ...
    return True if str.isidentifier() else False

#return PyOrigin Page
def _new_page(type, lname, template, hidden):
    type2 = _get_from_type(_BOOK_TYPES, type)
    if len(template) == 0:
        template = 'Origin'
    else:
...
def new_book(type=_WBOOK_TYPE, lname='', template='', hidden=False):
    """
...
def new_graph(lname='', template='', hidden=False):
    """
...
def new_sheet(type=_WBOOK_TYPE, lname='', template='', hidden=False):
    """
...
def _getpage(type, index):
    try:
        return _get_item(_GETPAGES[type](), index)
    except KeyError:
        raise ValueError('Invalid page type')
    
def _get_sheet(index, type): 
    bk = _getpage(type, index)
    return bk.Layers(0) if bk else None

def getpage(type, index=0):
    """get WPage, MPage, GPage by index"""
    page = _getpage(type, index)
    if page is None:
        return None
    cls = _get_from_type(_PAGE_CLS, type)
    return cls(page)

def find_sheet(type=_WBOOK_TYPE, ref=''):
    """
...
def find_book(type=_WBOOK_TYPE, name=''):
    """
...
def find_graph(name=''):
    ...
    if isinstance(name, int):
        page = _getpage(_GPAGE_TYPE, name)
    else:
        if len(name)==0:
    ...
    if page is None or page.GetType() != po.OPT_GRAPH:
        return None
...
def graph_list(select='f', inc_embed=False):
    """
