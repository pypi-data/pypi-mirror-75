...
def _start_execution(
    ...
    if is_managed and not settings_obj.enable_local_managed:
    ...
    if settings_obj.spark_context is not None:
        ...
        try:
            ...
            run_config = RunConfiguration()
            automl_run = _local_managed(experiment, run_config, settings_obj, fit_params, show_output, parent_run_id)
    elif is_managed:
        logger.info("Submitting local managed run.")
        print("Running on local conda or docker.")
        automl_run = _local_managed(experiment, run_config, settings_obj, fit_params, show_output)
    else:
