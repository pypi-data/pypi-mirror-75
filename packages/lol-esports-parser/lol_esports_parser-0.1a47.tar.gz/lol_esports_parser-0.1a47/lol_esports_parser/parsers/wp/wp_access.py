...
def get_chinese_series(qq_series_id: int, patch: str = None, add_names=True, discrete_mode=True) -> LolSeries:
    ...
    try:
        ...
        for idx, wp_game in enumerate(wp_series["games"]):
            ...
            for team_side in "BLUE", "RED":
                wp_game["teams"][team_side]["bans"] = qq_game["teams"][team_side]["bans"]
                wp_game["teams"][team_side]["bansNames"] = qq_game["teams"][team_side].get("bansNames")

    except Exception as e:
        lol_esports_parser_logger.error(f"Could not properly parse QQ game {qq_series_id}, relying on WP data.")
