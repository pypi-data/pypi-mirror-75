...
class User(object):
    def __init__(
        ...
        audience='https://oidc.plus4u.net/uu-oidcg01-main/99923616732452117-4f06dafc03cb4c7f8c155aa53f0e86be',
        jwtOptions={},
        save_auth=None,
    ):
        ...
        if self._save_auth and os.path.isfile(self._save_auth):
            try:
                ...
                if saved_user.is_expired():
                    ...
                    return False
                if saved_user.is_anonymous():
                    log.warning(
                        "Saved user session is anonymous in {}. Creating a new one.".format(
                            self._save_auth
                        )
                    )
                    return False
    ...
    def _load_jwks(self, force=False):
        if self._jwks is None or force:
            self._jwks = self._session.get(
                'https://oidc.plus4u.net/uu-oidcg01-main/99923616732452117-4f06dafc03cb4c7f8c155aa53f0e86be/getJWKS'
            ).json()
        ...
        if not self.is_authenticated():
            log.warning(
                'Authorization failed. {}'.format(
                    self._response.get('uuAppErrorMap', {})
                    .get('uu-oidc-main/notAuthenticated', {})
                    .get('message', '')
                )
            )
        else:
            if self.is_anonymous():
                log.warning("You've logged in as an anonymous user.")
            self._dump()
