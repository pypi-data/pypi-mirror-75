...
_config_ = {
    "database": {
        "host": "localhost",
        "port": 27017,
        "dbpath": "/opt/sweetheart/database",
        "select": "demo",
    },
    "webapp": {
        "working_dir": "/opt/sweetheart/webpages",
        ...
        "hosts": {
            ...
            "async": async_host },
        "subproc.service": "msterm",
    },
    "cherrypy": {# set default url segments configs:
        "/": "/opt/sweetheart/configuration/cherrypy.conf",
    },
    "bash": {
...
class subproc:
    ...
    @classmethod
    def msterm(cls, cmd:str):
        """start an external service within MS Terminal"""
        os.system('cmd.exe /c start wt.exe ubuntu.exe run {cmd}')

    # select the way for starting external service:
    service = eval(_config_["webapp"]["subproc.service"])
...
def init():
    ...
    subproc.bash("mv cherry.conf /opt/sweetheart/configuration/")
    ...
    with open("sweet","w") as fo:
        L1 = "#!/bin/sh"
        L2_1 = "/opt/sweetheart/programs/envPy/bin/python3"
        L2_2 = "/opt/sweetheart/programs/sweetheart.py/sweet.py $*"
        fo.write( f"{L1}\n{L2_1} {L2_2}" )
...
if not mongo_disabled:
    ...
    os.system(
        f'cmd.exe /c start wt.exe ubuntu.exe run \
        mongod --dbpath="{_dbpath}" &'
    )
    # os.system(
