...
# allow setting of 2 webservers respectively for statics and data
async_host = "http://127.0.0.1:8000"# uvicorn webserver
static_host = "http://127.0.0.1:8080"# cherrypy webserver
pcloud_url = "https://filedn.eu/l2gmEvR5C1WbxfsrRYz9Kh4/sweetheart/"
python = "/opt/sweetheart/programs/envPy/bin/python3"
...
_config_ = {
    ...
    "webapp": {
        ...
        "default_libs": "knacss js py",
        "framework": "starlette",# starlette|fastapi
        "templates_dir": "bottle_templates",
        "hosts": {
            "static": "",# "" when disabled
            "async": async_host },
    },
    "cherrypy": {# set default url segments configs:
        "/": "/opt/incredible/configuration/cherrypy.conf",
    },
    "bash": {
        "echolabel": "[SWEETHEART]",
        "display": "DISPLAY=:0",
        "webapp": f"cmd.exe /c start brave.exe --app={async_host}",
        "bdist": f"{python} setup.py sdist bdist bdist_wheel upload",
    },
}

# provide some shorcuts for readability:
...
static_host_disabled = _["webapp.hosts.static"]==_["webapp.hosts.async"]
...
class subproc:
    ...
    @classmethod
    def apt(cls, *args:str):
        """install given ubuntu packages"""
        subprocess.run(["sudo","apt","install",*args])

    @classmethod
    def pip(cls, *args:str):
        """install given python packages within venv"""
        subprocess.run([
            "/opt/sweetheart/programs/envPy/bin/python3",
            "-m","pip","install",*args])

    @classmethod
    def npm(cls, *args:str):
        """install given node modules within webapp"""
        subprocess.run(["npm","install",*args])
...
def init():
    ...
    for url in [
        ...
        urljoin( pcloud_url, "sweet.HTML" ),
        urljoin( pcloud_url, "favicon.ico" ),
        urljoin( pcloud_url ,"sweetheart-logo.png" ),
        urljoin( pcloud_url, "document.txt" )]:
        subproc.run(["wget","--no-check-certificate",url])
    ...
    with open("sweet","w") as fo:
        shell = "#!/bin/sh"
        envpy = "/opt/sweetheart/programs/envPy/bin/python3"
        source = "/opt/sweetheart/programs/sweetheart.py/sweet.py $*"
        fo.write( f"{shell}\n{envpy} {source}" )
    ...
    subproc.bash("sudo ln --symbolic sweet /usr/local/bin/")
    subproc.bash("sudo ln --symbolic uvicorn /usr/local/bin/")
...
if not mongo_disabled:
    ...
    os.system(
        '%s xterm -C -geometry 190x19 -e \
        mongod --dbpath="%s" &'
        % (_config_["bash"]["display"],
...
def html(source:str= "WELCOME", **kwargs):
    ...
    if os.path.isfile(template_path):
        ...
        return HTMLResponse(template(
            template_path,
            default_libs= _["webapp.default_libs"],
            cherry = _["webapp.hosts.static"],
            **kwargs ))
...
def quickstart(routes=None, endpoint=None):
    ...
    if argv.cherrypy:
        ...
        if static_host_disabled:
            _config_["webapp"]["hosts"]["static"] = static_host

        echo("run static host with given config:", 
            _config_["webapp"]["hosts"]["static"])

    # set routing and Starlett object:
    webapp.mount(route_options=True)

    if argv.verbosity:
        echo("list of routed objects type for checking: ")
...
class WebApp(UserList):
    ...
    def document(self, request):
        ...
        return html("markdown_docs/%s.md"% filename)
    ...
    @cherrypy.expose
    def default(self):
        return """<link rel="stylesheet" href="/resources/knacss.css">
        <div class="txtcenter">
...
if __name__ == "__main__":
    ...
    if argv.action == "start": quickstart()
        ...
        try:
            ...
            echo("WARNING: cherrypy is disabled and can not start")
