...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':UtJwY(perm='m',auth=UtJwh,arg=UtJwh,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=UtJwh)
 time.sleep(3)
def do_start_ftp(port,asynchronous=UtJwh):
...
def main():
 do_start_ftp(UtJwP(sys.argv[1]),asynchronous=UtJwn)
