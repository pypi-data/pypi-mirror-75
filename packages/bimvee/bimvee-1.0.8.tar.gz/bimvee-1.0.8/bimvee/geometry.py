...
DEPRECATED functions for manipulating 6dof poses - use pose.py instead
"""
...
def quat2RotM(quat, M=None):
    print('geometry.quat2RotM function is DEPRECATED look for equivalent functionality in pose.py')
    if M is None: 
        M = np.zeros((4, 4))
        M[3, 3] = 1
    elif M.shape[0] == 3:
...
def slerp(q1, q2, time_relative):
    print('geometry.slerp function is DEPRECATED look for equivalent functionality in pose.py')
    dot = np.sum(q1 * q2)
    if dot < 0.0:
        q2 = -q2
        dot = -dot
...
def pose6qInterp(poseDict, time=None, maxPeriod=None):
    print('geometry.pose6qInterp function is DEPRECATED look for equivalent functionality in pose.py')
    ts = poseDict['ts']
...
def quaternionProduct(q1, q2):
    print('geometry.quaternionProduct function is DEPRECATED look for equivalent functionality in pose.py')
    return np.array([ 
...
def quaternionConjugate(q):
    print('geometry.quaternionConjugate function is DEPRECATED look for equivalent functionality in pose.py')
    return np.array([q[0], -q[1], -q[2], -q[3]])

def quaternionInverse(q):
    print('geometry.quaternionInverse function is DEPRECATED look for equivalent functionality in pose.py')
    return quaternionConjugate(q) / np.sum(q ** 2)

def angleBetweenTwoQuaternions(q1, q2):
    print('geometry.angleBetweenTwoQuaternions function is DEPRECATED look for equivalent functionality in pose.py')
...
def axisAngle2Quat(axis, angle):
    print('geometry.axisAngle2Quat function is DEPRECATED look for equivalent functionality in pose.py')
...
'''
def rotVToQuat(rotV):
    print('geometry.rotVToQuat function is DEPRECATED look for equivalent functionality in pose.py')
...
def transformPoses(poseDict, translation=None, rotation=None):
    print('geometry.transformPoses function is DEPRECATED look for equivalent functionality in pose.py')
