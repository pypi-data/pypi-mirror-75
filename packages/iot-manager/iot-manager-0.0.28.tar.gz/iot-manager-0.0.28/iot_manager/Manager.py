...
class Manager:
    ...
    def __thread_handler(self):
        # logging output
        self.logger.info("(Greenlet Handler) Adding threads to the greenlet pool.")

        # add the core threads to the pool
        self.__background_greenlets.append(gevent.spawn(self.__connection_listener))
        self.__background_greenlets.append(gevent.spawn(self.__heartbeat_checker))
        self.__background_greenlets.append(gevent.spawn(self.__message_listener))

        gevent.joinall(self.__background_greenlets)

        # logging output
        self.logger.info("(Greenlet Handler) Starting greenlets.")

        # logging output
        self.logger.info("(Greenlet Handler) All greenlets started.")
        return
