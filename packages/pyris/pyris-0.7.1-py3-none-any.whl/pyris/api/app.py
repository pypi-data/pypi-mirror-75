...
class IrisListFromQuery(Resource):
    ...
    def get(self):
        ...
        Logger.info("Looking for iris at coordinates %s, %s", coord["lat"], coord["lon"])
        iris = extract.iris_from_coordinate(coord["lon"], coord["lat"])
        if not iris:
            api.abort(404, "IRIS not found from coordinates ({}, {})".format(coord["lat"], coord["lon"]))
...
class IrisFromAddress(Resource):
    ...
    def get(self):
        ...
        if coord['address'] is None:
            api.abort(404, "No address found matching that query")
        res = extract.iris_from_coordinate(coord['lon'], coord['lat'], args['geojson'])
        if not res:
            api.abort(404, "IRIS not found from coordinates ({}, {})".format(coord["lon"], coord["lat"]))
        res.update(coord)
        return res
