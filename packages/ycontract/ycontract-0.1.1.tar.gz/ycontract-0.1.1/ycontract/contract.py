...
def _new_contract_exception(cond_f, *args, **kw):
    return ContractException(
        f"Contract Error:\n    cond_f={cond_f.__name__}\n    args={args},\n    kw={kw}")


def prev_contract(cond, sys_state=SYS_STATE):
        ...
        def wrapped(*args, **kw):
            if sys_state.is_enable:
                if callable(cond):
                    try:
                        if not cond(*args, **kw):
                            raise _new_contract_exception(cond, *args, **kw)
                    except TypeError:
                        f_sig = signature(f)
                        ...
                        var_args = {v: get_arg(v) for v in varname_indexes}
                    ...
                    for varinfo, cond_f in cond.items():
                        if isinstance(varinfo, str):
                            ...
                            if not cond_f(values[ind]):
                                raise _new_contract_exception(cond_f, values[ind])
                        else:
                            inds = []
                            ...
                            if not cond_f(*cond_arguments):
                                raise _new_contract_exception(cond_f, *cond_arguments)
            return f(*args, **kw)
...
def ret_contract(cond_f, sys_state=SYS_STATE):
        ...
        def wrapped(*args, **kw):
            ...
            if sys_state.is_enable:
                if not cond_f(result):
                    raise _new_contract_exception(cond_f, result)
            return result
