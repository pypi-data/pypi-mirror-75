...
class Report(object):
    ...
    def report(self, stand_file: str, xd_file: str, wy_file: str) -> dict:
        """Get the evaluate report.
        :param stand_file: 标准测试数据
    ...
    def whisper_report(self, stand_file: str, xd_file: str) -> dict:
        logging.info("Just loading files...")
        ws = WhisperStatistics(stand_file, xd_file)
        start_time = time.time()
        ws.count_all(self.xd_other_type)
        logging.info(f"Whisper count successfully, cost {int(time.time() - start_time)} seconds.")
        return ws.indicator

    def netease_report(self, stand_file: str, wy_file: str) -> dict:
        logging.info("Just loading files...")
        ns = NetEaseStatistics(stand_file, wy_file)
        start_time = time.time()
        ns.count_all(self.wy_other_type)
        logging.info(f"NetEase count successfully, cost {int(time.time() - start_time)} seconds.")
        return ns.indicator
