...
class Microservices(BaseClass, ABC):
    ...
    def __init__(self, opt):
        ...
        if os.environ.get('REDISVALIDATE', None) is not None:
            self.initRedis()
    ...
    def initRedis(self):
        ...
        if os.environ.get('REDISVALIDATE', None) is None:
            ...
            __pool_redis = redis.ConnectionPool(
                host=os.environ.get("REDISVALIDATE"), port=int(port), db=0)
    ...
    def hasProcessMessage(self, data):
        ...
        if os.environ.get('REDISVALIDATE', None) is None:
            ...
            response_redis = self.redisclient.getset(h.hexdigest(), data)
            if response_redis is None:
                self.redisclient.expire(h.hexdigest(), (10 * 60))
                return False
            return True
    ...
    def _message(self, msg):
        ...
        if self.hasProcessMessage('{}_{}_{}'.format(
            ...
            if rootSpan is None:
                ...
                span = self.___Tracer.start_span(
                    operation_name='replication', child_of=rootSpan)
