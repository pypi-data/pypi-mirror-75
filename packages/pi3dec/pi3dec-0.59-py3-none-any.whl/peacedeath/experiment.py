...
def _write_parameters_OD_thresholds(pe, OD_thresholds=(0, 0, 0, 0, 0, 0, 0)):
...
def start_experiment(pe):
    ...
    print("Death doses:", dict(zip(range(7), pe.parameters_drug_doses)))
    print("OD thresholds:", dict(zip(range(7), pe.parameters_OD_thresholds)))

    schedule.clear()
    schedule.every().minute.at(":00").do(pause_mixers_and_measure_OD, pe, mixer_speed=pe.mixer_speed)
    schedule.every().minute.at(":15").do(dilute_if_necessary, pe, total_dilution_volume=pe.total_dilution_volume)
