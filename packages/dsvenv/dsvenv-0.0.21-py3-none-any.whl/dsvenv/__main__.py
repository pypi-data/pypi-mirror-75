...
def ensure_venv(venv_dir, python_version='system'):
    ...
    subprocess.check_call(
        [vpython, '-m', 'pip', 'install', '--upgrade', 'pip', 'setuptools']
    )

    # Ensure recent recent versions of artifacts-keyring (so that we can use an
    # Azure-hosted PyPi server).
    subprocess.check_call(
        [vpython, '-m', 'pip', 'install', '--upgrade', 'keyring', 'artifacts-keyring']
    )
...
def initialize_venv(venv_dir, reqs_file=None, extra_reqs=None):
    """
