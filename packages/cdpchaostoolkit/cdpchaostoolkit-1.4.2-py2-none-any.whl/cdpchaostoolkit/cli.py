...
# from cdpchaosreport import report
...
from cdpchaostoolkit import __version__, encoder
from cdpchaostoolkit.check import check_newer_version
from cdpchaostoolkit.logging import configure_logger
from cdpchaosslack import push_to_slack_channel
...
@click.option('--log-file', default="/tmp/chaostoolkit.log", show_default=True,
...
@cli.command()
@click.option('--journal-path', default="/tmp/journal.json",
...
def run(ctx: click.Context, source: str, journal_path: str = "journal.json",
    ...
    # if journal:
    #     report(export_format="html", journal=journal)
    #     push_to_slack_channel(journal)
...
with_plugins(iter_entry_points('cdpchaostoolkit.cli_plugins'))(cli)
