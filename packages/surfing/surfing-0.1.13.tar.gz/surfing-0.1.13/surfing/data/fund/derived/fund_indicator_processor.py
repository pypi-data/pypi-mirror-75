...
class FundIndicatorProcessor(object):
    ...
    def _process_fund_indicator_one(self, fund_id, time_range, fund_ret, index_id, index_ret):
        df = fund_ret[[fund_id]].join(index_ret).dropna()
        df = df.rename(columns={index_id:'benchmark',fund_id:'fund'}).reset_index()
        ...
        df = df.replace(np.Inf,None).replace(-np.Inf,None).dropna(subset=['beta', 'alpha','track_err']).reset_index()
    ...
    def calculate_history(self, index_id_list:str=None, cpu_num:int=2):
        ...
        for index_id in index_list:
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=cpu_num) as executor:
                future_to_bti = {executor.submit(self._process_fund_indicator_one, fund_id, time_range, fund_ret, index_id, index_ret): fund_id for fund_id in fund_list}
    ...
    def calculate_update(self):
        ...
        for index_id in index_list:
            ...
            print(f'index {index_id} finish')
        result = [ _ for _ in result if not _ is None]
        self.result = pd.DataFrame(result)
        self.result = self.result.replace(np.Inf,None).replace(-np.Inf,None).dropna(subset=['beta', 'alpha','track_err'])
