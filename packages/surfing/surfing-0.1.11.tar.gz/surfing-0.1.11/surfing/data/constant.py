...
class TradeTrigger:
    ...
    def parse(trigger):
        ...
        _s += 'Fe' if TradeTrigger._is(trigger, TradeTrigger.FundEnd) else '' # Fund End
        if len(_s) == 0:
            _s = 'None'
        return _s
    ...
    def trigger_log(trigger:int, 
        ...
        trigger_result = []
        if TradeTrigger._is(trigger, TradeTrigger.FundEnd):
            dic = {
                'rebalanace_reason': 'Fe',
                'index_id': _fund_rebalance_min_name,
                'rebalance_details': f'fund_rebalance_min_score: {fund_rebalance_min_score} threshold: {JudgeFundRebalance} fund rank :{fund_rank_list}'
            }
