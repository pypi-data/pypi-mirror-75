...
class DashTransformer(dash.Dash):
            ...
            for element in elements:
                for key in callback:
                    if isinstance(element, key):
                        # Check if this is a wild card callback.
                        component_id = element.component_id
                        is_wildcard = isinstance(component_id, dict)
                        if not multi_output and is_wildcard:
    ...
    def _setup_server(self):
        ...
        for callback in callbacks:
            outputs = callback[Output][0] if not callback["multi_output"] else callback[Output]
    ...
    def wrapper(*args):
        ...
        for i, entry in enumerate(input_prop_lists):
            ...
            try:
                ...
                if not callbacks[i]["multi_output"]: # len(callbacks[i][Output]) == 1:
                    outputs_i = [outputs_i]
        ...
        return output_values if len(output_values) > 1 else output_values[0]  # TODO: Check for multi output here?
...
def _pack_outputs(callback):
    ...
    def packed_callback(f):
        @functools.wraps(f)
        def decorated_function(*args):
            multi_output = callback["multi_output"] #len(callback[Output]) > 1
