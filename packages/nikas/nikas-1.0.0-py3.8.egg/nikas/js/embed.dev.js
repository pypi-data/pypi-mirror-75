...
define('app/i18n',["app/config", "app/i18n/en"], function (config, en) {
    "use strict";
...
define('app/utils',["app/i18n"], function (i18n) {
    ...
    var ago = function (localTime, date) {
        ...
        return (
            ...
            (days <= 345 &&
                i18n.pluralize("date-month", Math.floor(days / 30))) ||
...
buf.push("<div" + (jade.attr("id", 'nikas-' + (comment.id) + '', true, false)) + " class=\"nikas-comment\">");
if ( conf.gravatar)
{
...
buf.push("<div class=\"text-wrapper\"><div role=\"meta\" class=\"nikas-comment-header\">");
...
buf.push("<span class=\"spacer\">&bull;</span><a" + (jade.attr("href", '#nikas-' + (comment.id) + '', true, false)) + " class=\"permalink\"><time" + (jade.attr("title", '' + (humanize(comment.created)) + '', true, false)) + (jade.attr("datetime", '' + (datetime(comment.created)) + '', true, false)) + "></time></a><span class=\"note\">" + (jade.escape(null == (jade_interp = comment.mode == 2 ? i18n('comment-queued') : comment.mode == 4 ? i18n('comment-deleted') : '') ? "" : jade_interp)) + "</span></div><div class=\"text\">");
if ( comment.mode == 4)
{
buf.push("<p>&nbsp;</p>");
}
...
buf.push("</div><div class=\"nikas-comment-footer\">");
if ( conf.vote)
{
...
define('app/lib/identicons',["app/lib/promise", "app/config"], function (Q, config) {
    ...
    var generateIdenticon = function (key, padding, size) {
        ...
        Q.when(key, function (key) {
            var hash = pad(
                    (parseInt(key.substr(-16), 16) % Math.pow(2, 18)).toString(
                        2
                    ),
                    18
                ),
...
define('app/nikas',[
    ...
    var insert_loader = function (comment, lastcreated) {
        ...
        if (comment.id === null) {
            ...
            entrypoint = $(
                "#nikas-" + comment.id + " > .text-wrapper > .nikas-follow-up"
            );
    ...
    var insert = function (comment, scrollIntoView) {
        ...
        var footer = $(
                "#nikas-" +
                    comment.id +
                    " > .text-wrapper > .nikas-comment-footer"
            ),
            header = $(
                "#nikas-" +
                    comment.id +
                    " > .text-wrapper > .nikas-comment-header"
            ),
            text = $("#nikas-" + comment.id + " > .text-wrapper > .text");
        ...
        $("a.edit", footer).toggle(
            ...
            function (toggler) {
                ...
                edit.insertAfter(
                    $.new("a.cancel", i18n.translate("comment-cancel"))
                ).on("click", function () {
...
define('app/text/svg',["text!./arrow-down.svg", "text!./arrow-up.svg"], function (
    arrdown,
    arrup
) {
    return {
