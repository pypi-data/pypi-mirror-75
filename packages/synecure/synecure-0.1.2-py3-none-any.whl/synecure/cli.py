...
def main():
    ...
    # Use SSHFS to perform the update
    sshfs: Argument & bool = default(False)
...
def plan_sync(path, remote_name, remotes, directories,
    ...
    if sshfs and remote["type"] == "ssh":
        # Mount in a standard place in .config to ease debugging
        mountpoint = get_config_path("mnt")
        os.makedirs(mountpoint, exist_ok=True)
        commands = [
            ["ssh", remote["url"], "-p", str(remote["port"]), f"mkdir -p {destpath}"],
            ["sshfs", "-o", "Compression=no", dest, "-p", str(remote["port"]), mountpoint]
        ]
        print(f"# MOUNT REMOTE    {mountpoint}")
        commands += _plan_commands(remote, path, mountpoint, mountpoint, dry=dry,
            interactive=interactive)
        umount = shutil.which("fusermount") or "umount"
        commands.append([umount, mountpoint])
        return commands

    else:
        return _plan_commands(remote, path, dest, destpath, dry=dry,
            interactive=interactive)


def _plan_commands(remote, path, dest, destpath, dry, interactive):
