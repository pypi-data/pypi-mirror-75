...
def complete_setup_data(nob):
    ...
    setup_nob['runner'] = {'fallback': nob.fallback[:], 
                           'checklevel': nob.checklvl[:]}

    with open('ihm_setup.yml', 'w') as fout:
        yaml.dump(setup_nob, fout, default_flow_style=False)

    hip_commands = complete_setup(Nob(setup_nob), nob.fallback[:], nob.checklvl[:])
...
def _generate(nob):
    ...
    out = {
        'lower_corner': nob.lower_corner[:],
        'upper_corner': nob.upper_corner[:],
        'resolution': nob.resolution[:],
        'convert2tri': convert2tri,
        'patch_list': nob.patch_list[:],
    }
    if unique_dict_key(nob.dimension[:]) == '3D':
        out_3d = {
            'extru_axis': nob.extru_axis[:],
            'extru_range': (nob.extru_min[:], nob.extru_max[:]),
            'extru_resolution': nob.extru_resolution[:],
        }
        out.update(out_3d)
