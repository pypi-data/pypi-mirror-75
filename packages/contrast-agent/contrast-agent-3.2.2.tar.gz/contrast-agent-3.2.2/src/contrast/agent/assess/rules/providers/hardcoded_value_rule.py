...
class HardcodedValueRule(BaseRule, CodeParserMixin):
    MODULES_TO_IGNORE = [
        "contrast",
        "django",
        "flask",
    ...
    def update_preflight_hash(self, hasher, source_name=None, constant=None):
        """
    ...
    def report_finding(self, cls_or_module, constant):
        ...
        activity = Activity()
        activity.finding_tags = settings.config.get("assess.tags")
        activity.findings.extend([finding])

        settings.send_messages([activity])
