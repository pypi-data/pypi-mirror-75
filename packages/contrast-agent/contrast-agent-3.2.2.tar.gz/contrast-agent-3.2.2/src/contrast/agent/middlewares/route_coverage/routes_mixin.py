...
class RoutesMixin(object):
    ...
    def update_route_information(self, context, route_id, request_method, func):
        ...
        # TODO: PYT-972
        # In the future we want to try to generate a substitute value for missing route
        # signature if possible. We could possibly use some combination of the route ID
        # and uri?
        if not route.route:
            logger.debug(
        ...
        # TODO: PYT-914 deprecate when SR handles this (SPEED-273). Teamserver currently does not
        # allow empty url
