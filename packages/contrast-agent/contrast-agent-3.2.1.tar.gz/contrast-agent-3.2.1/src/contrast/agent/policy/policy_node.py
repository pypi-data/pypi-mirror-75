...
class PolicyNode(object):
    ...
    def ts_represent(self, str_rep):
        ...
        * ALL_ARGS or ALL_KWARGS --> P0
        ...
        if str_rep.startswith(
            (
                policy_constants.KWARG,
                policy_constants.ALL_ARGS,
                policy_constants.ALL_KWARGS,
            )
        ):
            return "P0"
    ...
    def _type_to_action(self, sources_or_targets):
        ...
        if item in (policy_constants.ALL_ARGS, policy_constants.ALL_KWARGS):
            return policy_constants.ALL_TYPE
        ...
        if item == policy_constants.RETURN:
    ...
    def build_action(self):
        ...
        """
        if not self.sources:
            event_action = TraceEvent.Action.Value(policy_constants.CREATION_TYPE)
        elif not self.targets:
            event_action = TraceEvent.Action.Value(policy_constants.TRIGGER_TYPE)
        else:
