...
class ModelSearchCV(GridSearchCV):
    ...
    def _modify_grid_search_attrs(self):
        ...
        # Modifiy cv results
        params, models = [], []
        for param_grid in self.cv_results_['params']:
            updated_param_grid = {}
            for param, value in param_grid.items():
                if param != 'est_name':
                    param = '__'.join(param.split('__')[1:])
                    updated_param_grid[param] = value
            params.append(updated_param_grid)
            models.append(param_grid['est_name'])
