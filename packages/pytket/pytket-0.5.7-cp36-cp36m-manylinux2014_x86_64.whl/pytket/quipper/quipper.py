...
class QuipperTransformer(Transformer):
    def int(self, t: Tree) -> int:
        return int(t[0])

    def float(self, t: Tree) -> float:
        return float(t[0])

    def string(self, t: Tree) -> str:
        return str(t[0][1:-1])

    def wire(self, t: Tree) -> Wire:
        return Wire(t[0])

    wire_list = list

    def wire_string_list(self, t: Tree) -> List[Tuple[Wire, str]]:
        wires = (el for i, el in enumerate(t) if i % 2 == 0)
        labels = (el for i, el in enumerate(t) if i % 2 == 1)
        return list(zip(wires, labels))

    def pos_control_wire(self, t: Tree) -> ControlWire:
        return ControlWire(t[0], False)

    control_wire_list = list

    def neg_control_wire(self, t: Tree) -> ControlWire:
        return ControlWire(t[0], True)

    def type_assignment(self, t: Tree) -> TypeAssignment:
        ty = TypeAssignment_Type.Qbit if t[1] == "Qbit" else TypeAssignment_Type.Cbit
        return TypeAssignment(t[0], ty)

    def arity(self, t: Tree) -> List[Tree]:
        return list(t)

    def qgate(self, t: Tree) -> QGate:
        ops = QGate_Op
        n = t[0]
    ...
    def circuit(self, t: Tree) -> Program:
        return Program(inputs=t[0], gates=t[1:-1], outputs=t[-1])

    def subroutine(self, t: Tree) -> Subroutine:
        if t[2] == "yes":
...
class CircuitMaker:
    ...
    def make_circuit(self, circ: Program) -> Circuit:
        ...
        all_wires = [inp.wire for inp in inps]
        ...
        for gate in gates:
            if isinstance(gate, SubroutineCall):
                ...
                for w in gate.control.controlled:
                    idx, is_neg = w.children[0].wire.i, w.children[0].negative
                    if idx in qbits:
                        tkqidx = tkqbits[idx]
...
def circuit_from_quipper(input_file: str) -> Circuit:
    ...
    * No support for classical operations ('CNot', 'CGate', 'CSwap', 'CInit',
    ...
    maker = CircuitMaker(x.subroutines)

    # Make the tket circuit.
    return maker.make_circuit(x.circuit)
