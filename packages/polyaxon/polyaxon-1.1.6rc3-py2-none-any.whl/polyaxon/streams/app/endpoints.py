...
from polyaxon.streams.controllers.archived_logs import (
...
async def get_logs(request):
    ...
    if last_time:
        operation_logs, last_time = get_tmp_operation_logs(
...
async def _get_logs(request):
    ...
    if not last_file and k8s_operation:
        ...
        last_time = None
        operation_logs, last_file, _ = await get_archived_operation_logs(
            run_uuid=run_uuid, last_file=last_file, check_cache=not force
        )
    if k8s_manager:
