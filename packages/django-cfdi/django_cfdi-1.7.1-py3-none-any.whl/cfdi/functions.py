...
def validar_sello(xml_text, sello, certificado, xslt_path):
    """Regresa un string con el error (si hubo algún error)"""

    some_file_or_file_like_object = BytesIO(xml_text.encode("utf-8"))
...
def extraer_tfd(xml_text):
    return (
        "<tfd:TimbreFiscalDigital " + 
        xml_text.split("<tfd:TimbreFiscalDigital ", 1)[1].split("/>")[0] 
        + "/>"
    )

def get_or_create_certificado(nc):
    from . import models
    try:
...
def validar_cfdi(xml_text):
    ...
    tfd_certificado = get_or_create_certificado(nc).pem


    # Cadena original comprobante
    cfdi_error = validar_sello(xml_text, cfdi_sello, cfdi_certificado, XSLT_PATH_CFDI)
    #tfd_error = validar_sello(tfd_text, tfd_sello, tfd_certificado, XSLT_PATH_TFD)
    tfd_error = validar_sello(xml_text, tfd_sello, tfd_certificado, XSLT_PATH_TFD)
