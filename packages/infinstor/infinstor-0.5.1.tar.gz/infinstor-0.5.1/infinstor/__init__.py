...
def s3downloader(download_task_q, write_pipe, endpoint, glb):
    ...
    while (True):
        ...
        if (op == 'download'):
            ...
            # info('s3downloader: starting download of ' + full_object_key + ' from ' + bucketname)
            obj = client.get_object(Bucket=bucketname, Key=full_object_key)
            # info('s3downloader: finished download of ' + full_object_key + ' from ' + bucketname)
            strbody = obj['Body']
            datetime = obj['LastModified']
            bts = strbody.read()
            key = datetime.strftime('%Y-%m-%d %H:%M:%S') + ' ' + bucketname + '/' + full_object_key
            if (glb == None):
                write_pipe.send(key)
                write_pipe.send(bts)
            else:
...
def download_objects_inner(endpoint, bucketname, parentdir, array_of_files, is_csv, glb):
    ...
    for i in range(num_threads()):
        ...
        p = Process(target=s3downloader, args=(command_q, w1, endpoint, glb,))
        p.start()
        processes.append(p)
        w1.close()
    ...
    with tqdm(total=len(array_of_files)) as pbar:
        ...
        while pipe_from_child:
            ready = wait(pipe_from_child, timeout=2)
            if (ready == None):
                info('>>>>>>>>>>>>>> received None from wait >>>>>>>>>>>>')
            else:
                if (len(ready) == 0):
                    info('>>>>>>>>>>>>>> WARNING: None ready from wait >>>>>>>>>>>>')
                for read_pipe in ready:
                    try:
                        ...
                        if ((files_read % 10) == 0):
                            ...
                            info("Downloaded " + str(files_read))
                            step += 1
                    except EOFError:
