...
#lsl_reception_ov = io.LslReceive('name', 'openvibeSignal', sync='network')
...
chans = select.ChannelSelector(rda_reception.output, 'index', [8, 25])
...
'''
#csv = store.ToCsv(log_epoch.output, 'alpha_band.csv')
#csv_ov = store.ToCsv(lsl_reception_ov.output, 'alpha_band_ov.csv')
