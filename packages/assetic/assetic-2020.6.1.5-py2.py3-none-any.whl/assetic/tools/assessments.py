...
class AssessmentHelper(object):
    ...
    def create_form_from_csv(self,form_repr,csvfile,structure=None):
        ...
        return self.assesstool.create_form(form)
    ...
    def assessment_form_create_prompter(self):
        ...
        try:
            ...
            self.user_input("Error encountered. Press enter to close:")
            result = 1
    ...
    def get_column_purpose_structure(self):
        ...
        purpose["Widget"] = "The Group Control that the form field is assigned to.\n"\
                                              "Values in this column match the Group Control Names "\
                                              "entered above"
        purpose["Type"] = "The control type. Values in this column match the \n"\
                                            "control names entered above"

        purpose["Attributes"] = "Attributes of the control such as length,dropdown \n"\
                                                      "items, stepper settings"
        purpose["Length"] = "Field length (if not using 'Attributes' column)"
        purpose["DropdownItems"] = "Dropdown items (if not using 'Attributes' column)"
        purpose["MinValue"] = "Stepper minimum value (if not using 'Attributes' column"
        purpose["MaxValue"] = "Stepper maximum value (if not using 'Attributes' column"
        purpose["ValueIncrement"] = "Stepper increment (if not using 'Attributes' "\
                                                              "column"
        purpose["ValueFormat"] = "Stepper display format (if not using 'Attributes' "\
                                                       "column"
...
class AssessmentTools(object):
    ...
    def new_combobox_control(self,label,name=None,datatype="string",order=1,items=[],hint=None,visible=True,required=False):
        """
