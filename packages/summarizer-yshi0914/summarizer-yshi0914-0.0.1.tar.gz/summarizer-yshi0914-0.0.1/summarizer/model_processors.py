...
class ModelProcessor(object):
    ...
    def cluster_runner(
        ...
        """
        ## using Bert-pretrained embedding with dimension: #of sentences X 1024
        print("Preparing for the Bert-pretrained embedding...")
        ...
        if use_first and len(hidden_args) > 0:
            if hidden_args[0] != 0:
    ...
    def __run_clusters(self, content: List[str], ratio=0.2, algorithm='DBSCAN', use_first: bool= True) -> List[str]:
        """
    ...
    def __retrieve_summarized_embeddings(
            self, content: List[str], ratio=0.2, algorithm='DBSCAN', use_first: bool= True
    ) -> np.ndarray:
        """
    ...
    def run(
        ...
        :param algorithm: Which clustering algorithm to use. (DBSCAN, gmm)
