...
def push(data=None, *, metadata=None, workbook=_common.DEFAULT_WORKBOOK_PATH,
    ...
    if workbook is not None:
        ...
        if _common.is_guid(workbook):
            ...
            }]), include_inventory=False, status=status.create_inner(quiet=True))
            ...
            if len(search_df) == 0:
                ...
                spy.workbooks.push(primary_workbook, path=workbook_path, include_inventory=False,
                                   datasource=datasource, status=status.create_inner(quiet=True))
            else:
            ...
            if item_type == 'Signal':
                for column in data:
                    try:
                        ...
                        to_do[executor.submit(_push_signal, column, signal_metadata, data, signal_output,
                                              type_mismatches, interrupt_event, status_index,
                                              status_updates)] = status_index
    ...
    workbook_push_df = _push_workbooks(push_result_df, workbook_path, workbooks_to_push, datasource, status)
...
def _push_signal(column, signal_metadata, data, signal_output, type_mismatches, interrupt_event, status_index,
                 status_updates):
    signals_api = SignalsApi(_login.client)
...
def _push_workbooks(push_result_df, workbook_path, workbooks, datasource, status):
    ...
    return spy.workbooks.push(workbooks, path=workbook_path, refresh=False, include_inventory=False,
                              datasource=datasource, status=status.create_inner())
