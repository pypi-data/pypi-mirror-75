...
class Annotation:
    ...
    def pull(self):
        ...
        if not annotation_output:
            raise RuntimeError(f'Annotation item not found on worksheet {self.worksheet}')
    ...
    def save(self, workbook_folder):
        ...
        for image_id_tuple, content in self.images.items():
            image_file = Annotation._get_image_file(workbook_folder, image_id_tuple)
...
class Report(Annotation):
    ...
    def pull(self):
        ...
        if annotation_output.cron_schedule is not None:
            self.schedule = {
