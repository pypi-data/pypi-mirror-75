...
class ClientAgent(TcpClient.TcpClientThread, TransactionManager.TransactionManager):
    ...
    def handleIncomingPacket(self, pdu):
        ...
        try:
            ...
            logging.error('unable to decode new message: %s' % str(e) )
        else:
            try:
                TransactionManager.TransactionManager.onMessage( self, decoded  )
            except Exception as e:
                logging.error('unable to handle new message: %s' % str(e) )
    ...
    def onProxyConnection(self):
        ...
        """
        self.__connected = True
        logging.debug('Proxy initialization...')
        try:
            self.sendProxyHttpRequest()
        except Exception as e:
            logging.error( 'unable to init proxy: %s' % str(e) )
        
    def doRegistration(self, cancelEvent=None):
        """
        Send a registration
        """
        logging.debug('Do registration...')
        data = {
                    'version': self.ver,
        ...
        if rsp is None : 
            logging.debug('Registration failed, no response.')
            self.onRegistrationFailed( 'No response' )
        elif rsp['code'] == b'403':
            logging.debug('Registration refused')
            self.onRegistrationRefused( err = 'Registration refused')
        elif rsp['code'] == b'400':
            logging.debug('Registration failed, conflict name')
            ...
            logging.error('Unknown error on registration: %s' % rsp['code'])
    ...
    def bad (self, tid, body = None):
        ...
        try:
            ...
            try:
                ...
                logging.debug("-> 500 BAD %s" % tid)
            except Exception as e:
                logging.error( '[failed] reply error %s' % str(e) )
    ...
    def failed (self, tid, body = None):
        ...
        try:
            ...
            try:
                TcpClient.TcpClientThread.sendPacket(self, rspEncoded)
                logging.debug("-> 400 FAILED %s" % tid)
            except Exception as e:
                logging.error( '[failed] reply error %s' % str(e) )
    ...
    def forbidden (self, tid, body = None):
        ...
        try:
            ...
            try:
                TcpClient.TcpClientThread.sendPacket(self, rspEncoded)
                logging.debug("-> 403 OK %s" % tid)
            except Exception as e:
                logging.error( '[forbidden] reply error: %s' % str(e) )
    ...
    def ok (self, tid, body = None):
        ...
        try:
            ...
            try:
                TcpClient.TcpClientThread.sendPacket(self, rspEncoded)
                logging.debug("-> 200 OK %s" % tid)
            except Exception as e:
                logging.error( 'unable to send ok message: %s' % str(e) )
    ...
    def notify (self, data):
        ...
        try:
            ...
            try:
                ...
                logging.debug("-> NOTIFY %s" % tid)
            except Exception as e:
                logging.error( '[notify] request notify error %s' % str(e) )
    ...
    def cmd (self, data, timeout=0.0, cancelEvent=None):
        ...
        try:
            ...
            logging.error( 'unable to encode cmd message: %s' % str(e) )
        else:
            try:
                # send packet
                TcpClient.TcpClientThread.sendPacket(self, encoded)
                ...
                if timeout > 0:
                    maxTime = timeout
                logging.debug("-> REQUEST %s, timeout of %s" % (tid,maxTime) )
