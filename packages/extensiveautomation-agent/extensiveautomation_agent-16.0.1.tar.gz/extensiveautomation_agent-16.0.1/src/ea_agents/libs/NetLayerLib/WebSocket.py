...
class WebSocketCodec(object):
    ...
    def createSecWsKey(self):
        ...
        try:
            ...
            logging.error( 'unable to create sec key: %s' % e )         
        return encoded
        ...
        try:
            for hdr in request.splitlines()[1:]:
                ...
                if k.lower().strip() == b'sec-websocket-key':
                    ...
                    if keyLength != 16:
                        logging.error( 'bad key length: %s' % keyLength )     
                        wsKey = False
            ...
            logging.error( 'unable to check req headers: %s' % e )                      
            ...
            for hdr in response.splitlines()[1:]:
                ...
                if k.lower().strip() == b'sec-websocket-accept':
                    ...
                    if hashed == v:
                        ...
                        if self.debug:
                            logging.error('web socket key incorrect computed=%s received=%s key=%s' % (hashed,v, key) )
        except Exception as e:
            logging.error( 'unable to check headers: %s' % e )                      
    ...
    def decodeWsData(self, buffer):
        ...
        try:
            try:
                ...
                if self.debug:
                    logging.debug( "ws header: %s, %s, %s, %s, %s, %s, %s" % (fin, rsv1, rsv2, rsv3, opcode, has_mask, length) )
                ...
                if length == 126:
                    try:
                        ...
                        if self.debug:
                            logging.debug( "ws lenght extended: %s" % length_ext)
                        if fin:
                            if len(buffer) < hdrs_len+hdrs_extended_len+length_ext:
                                if self.debug:
                                    logging.debug( "data extended, need more data (%s/%s)" % (len(buffer), length_ext) )
                                left = buffer # need more data
                                needMore = True
                            else:
                    ...
                    if fin:
                        if len(buffer) < hdrs_len+length:
                            if self.debug:
                                logging.debug( "data, need more data (%s/%s)" % (len(buffer), length) )
            ...
            logging.error( 'unable to decode ws data: %s' % e )
    ...
    def encodeWsData(self, data, opcode):
        ...
        try:
            ...
            logging.error( 'unable to encode ws data: %s' % e )
