...
class TcpClientThread(threading.Thread):
    ...
    def __init__(self,  serverAddress = None, localAddress = ('', 0), inactivityTimeout = 30,
        ...
        if wsSupport: logging.debug('Web socket activated - version %s' % WebSocket.WEBSOCKET_VERSION)       
        ...
        if sslSupport: logging.debug('Ssl activated - version %s' % self.sslVersion)       
        ...
        logging.debug('Tcp Client Thread Initialized')
    ...
    def setServerAddress (self, ip, port):
        ...
        try:
            ...
            if sys.version_info > (3,): # python 3 support
                ...
                logging.error( str(e).decode('iso-8859-15') )
    ...
    def sendProxySocks4Request(self):
        ...
        try:
            ...
            logging.error( "unable to initiate proxy socks4: %s" % str(e) )
    ...
    def sendProxySocks5Request(self, login=None, password=None):
        ...
        try:
            ...
            logging.error( "unable to initiate proxy socks5: %s" % str(e) )
    ...
    def sendProxyHttpRequest(self):
        ...
        try:
            ...
            logging.error( "unable to initiate proxy http: %s" % e )
    ...
    def handshakeWebSocket(self, resource="/", hostport='localhost'):
        ...
        try:
            ...
            logging.error( "unable to initiate web socket: %s" % e )
    ...
    def __startConnection (self):
        ...
        if self.proxyAddress is not None:
            logging.debug( "connecting from %s to %s with the proxy %s" % ( str(self.localAddress), str(self.serverAddress), str(self.proxyAddress) ) )
        else:
            logging.debug( "connecting from %s to %s" % ( str(self.localAddress), str(self.serverAddress)) )
        try:
            self.buf = b''
            ...
            if self.proxyAddress is not None:
                logging.error("socket tcp proxy %s on connection." % ( str(e) ) )
                self.onProxyConnectionTimeout(err = str(e))
            else:
                logging.error("socket tcp %s on connection." % ( str(e) ) )
                self.onConnectionTimeout(err = str(e))
        except Exception as e:
            if self.proxyAddress is not None:
                logging.error("Proxy %s." % ( str(e) ) )
            ...
            if self.running:
                            ...
                            if keepAlive:
                                if self.keepAliveInterval:
                                    if time.time() - self.lastKeepAliveTimestamp > self.keepAliveInterval:
                                        ...
                                        logging.debug("sending ws ping message id=%s" % pingId)                                        
                        ...
                        while not self.queue.empty():
                            ...
                            if self.socket in e:
                                ...
                                try:
                                    ...
                                    logging.error("unable to send message: " + str(e))
                            else:
                                break
                except EOFError as e:
                    if "Inactivity timeout" in str(e):
                        logging.error( "disconnecting, inactivity timeout" )
                        self.inactivityServer = True
                        # self.onInactivityTimeout()
                        self.event.clear()
                        self.closeSocket = True
                    else:
                        logging.error( "disconnected by the server: %s" % str(e) )
                        self.onDisconnection(byServer=True)
                        self.event.clear()
                
                # new with python3
                except ConnectionAbortedError as e:
                    logging.error( "connection aborted by peer" )
                    self.onDisconnection(byServer=True)
                    self.event.clear()  
                except ConnectionRefusedError as e:
                    logging.error( "connection refused by peer" )
                    self.onDisconnection(byServer=True)
                    self.event.clear()  
                except ConnectionResetError as e:
                    logging.error( "connection reseted by peer" )
                    self.onDisconnection(byServer=True)
                    self.event.clear()
                # end of new
                ...
                # end of new 
                    ...
                    if "[Errno 10054]" in str(e):
                        ...
                        logging.error( "generic error on run: %s" % str(e) )
                        self.closeSocket = True
                        self.event.clear()
            
            # close socket
            if self.closeSocket:
                logging.debug("cleanup socked")
                if self.socket is not None:
                    # cleanup the queue
                    while not self.queue.empty():
                        try:
        ...
        try:
            if self.running:
                ...
                if self.proxyAddress is not None and not self.proxyConnectSuccess and self.buf:
                    logging.debug( 'data received for proxy handshake of len %s' %  len(self.buf) )
                ...
                if self.wsSupport and not self.wsHandshakeSuccess and self.buf:           
                    if not readTrueData and not self.proxyConnectSuccess and self.proxyAddress is not None: 
                        ...
                        logging.debug( 'data received for ws handshake of len %s' %  len(self.buf) )
                        ...
                        if not needmore:
                            if opcode == WebSocket.WEBSOCKET_OPCODE_TEXT:
                                ...
                                if opcode == WebSocket.WEBSOCKET_OPCODE_PONG:
                                    logging.debug("received ws pong message id=%s" % data)
                                elif opcode == WebSocket.WEBSOCKET_OPCODE_PING:
                                    logging.debug("received ws ping message id=%s" % data)     
                                    wspong = self.wsCodec.encodePong(data=data)
                                    self.queue.put(wspong)
                                    logging.debug("sending pong message id=%s" % data)                                 
                                else:
                                    logging.error('unknown ws opcode received: %s' % opcode) 
            ...
            logging.error("error on incoming data: %s" % e )
    ...
    def readBuffer(self):
        ...
        for pdu in pdus[:-1]:
            if not pdu == self.keepAlivePdu:
                ...
                logging.debug("received keep-alive from server")
        self.buf = pdus[-1]
    ...
    def decodeWsHandshake(self):
        ...
        try:
            if self.buf.find(b"\r\n\r\n")!=-1:
                ...
                rsp = datasplitted[0]
                logging.debug('ws complete response received')
                statusline = rsp.splitlines()[0].split(b" ",2)
                if statusline[0] not in (b"HTTP/1.1"):
                    self.buf = b''
                    self.closeConnection()
                    logging.error( "Malformed HTTP ws message: %s" % statusline )
                    ...
                    if statuscode != 101:
                        ...
                        if not self.wsCodec.checkingWsHeaders(response=rsp, key=self.wsKey):
                            self.buf = b''
                            self.closeConnection()
                            logging.error( "Handshake ws refused, invalid headers" )
                            self.onWsHanshakeError( err="Handshake ws refused")
                        else:
                            logging.debug('Ws handshake accepted')
        ...
        try:
            ...
            if self.proxyType == PROXY_TYPE_SOCKS4:
                if ord(self.buf[0]) != 0: # bad version on response
                    self.buf = b''
                    self.closeConnection()
                    logging.error( "Socks4: bad response from proxy: %s" % self.buf[0] )
                    self.onProxyConnectionError( err="Socks4: bad response from proxy: %s" % self.buf[0])
                else:
                    if  ord(self.buf[1]) != 90: # granted
                        self.buf = b''
                        logging.error( "Socks4 proxy refused the connection!" )
                        self.closeConnection()
            ...
            # handle http
                ...
                logging.debug("http proxy activated")
                logging.debug("response received: %s" % self.buf)
                ...
                if self.buf.find(b"\r\n\r\n")!=-1:
                    ...
                    if statusline[0] not in (b"HTTP/1.0", b"HTTP/1.1"):
                        self.buf = b''
                        self.closeConnection()
                        logging.error( "Bad http response from proxy: %s" % statusline )
                        ...
                        if statuscode != 200:
                            ...
                            logging.error( "Http proxy refuses the connection: %s" % statusline )
                            ...
                            if self.sslSupport:
                                ...
                                try:
                                    ...
                                    self.buf = b''
                                    self.closeConnection()
                                    logging.error( "SSL Http proxy refuses to establish the tunnel: %s" % e )
            ...
            # handle socks5: not implemented
                ...
                if ord(self.buf[0]) != 5: # bad version on response
                    logging.error( "Socks5: bad response from proxy: %s" % self.buf[0] )
    ...
    def stop(self):
        ...
        self.running = False
        self.event.set()
        logging.debug('Tcp Client Thread Stopped')
