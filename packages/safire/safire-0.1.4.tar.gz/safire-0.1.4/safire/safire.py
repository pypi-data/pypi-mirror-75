...
class List(ut.Help):
    ...
    def drives(self, filter="", file_tag="", token=cf.token, prt=True):
        ...
        if not prt:
            return drivelist
        self._export(drivelist, filter, file_tag, ["id", "name"], "drives",
                     prt)
    ...
    def groups(self,
        ...
        grouplist = svc.groups().list(
            customer="my_customer").execute()["groups"]
    ...
    def members(self,
        ...
        for group in group_list:
            try:
                ...
                continue
            member_summary += [
                f"  {len(response)} members found in group: {group}"
            ]
        if not prt:
            return sorted(
                [i["email"] for i in memberslist if i["role"] != "OWNER"])
        self._export(memberslist, filter, file_tag, ["email", "role"],
                     "members")
    ...
    def jsons(self, sa_path=cf.sa_path, filter="", file_tag="", prt=True):
        ...
        if filter:
            json_keys = [
                key for key in json_keys if str(filter) in key["project_id"]
            ]
        if prt and json_keys:
            self._export(json_keys, filter, file_tag, ["client_email"],
                         "json_keys", prt)
...
class Add(ut.Help):
    ...
    def projects(
        ...
        while num_new_projects and retry:
            ...
            for proj in new_projs:
                batch.add(cloud.projects().create(body={
                    "project_id": proj,
                    "name": proj
                }))
            batch.execute()
    ...
    def apis(self,
             filter="",
             svcs_to_enable=cf.svcs_to_enable,
             token=cf.token,
             prt=False):
        ...
        for project in projId_list:
            for svc1 in svcs_to_enable:
                ...
                batch.add(svcusage.services().enable(
    ...
    def sas(
        ...
        for project in projId_list:
            ...
            sa_emails, _ = self._list.sas(project, True, file_tag, cf.token,
                                          False)
            ...
            print(len(sa_emails), "SAs exist. Creating", count,
                  "SAs in project", project)
            # if count == 0:
            #     continue
            new_sas = []
            while count and retry:
                for _ in range(count):
                    while [
                            s for s in all_sas
                            if str(next_sa_num) in s.split("@")[0]
                    ]:
                        next_sa_num += 1
                ...
                batch.execute()
                retry -= 1
                sa_emails, _ = self._list.sas(project, True, file_tag,
                                              cf.token, False)
    ...
    def jsons(
        ...
        for project in sorted(projId_list):
            batch = ut.BatchJob(iam)
            _, sa_uniqueId = self._list.sas(project, True, file_tag, token,
                                            prt)
            num_sas = len(sa_uniqueId)
            print(
                f"Downloading {str(len(sa_uniqueId))} SA keys in project {project}"
            )
            resp = []
    ...
    def drive(self, td_name):
        ...
        body = {"name": td_name}
        driveId = (drive.drives().create(body=body,
                                         requestId=str(uuid.uuid4()),
                                         fields="id,name").execute().get("id"))
    ...
    def members(
        ...
        projId_list = self._list.projects(project_filter, file_tag, cf.token,
                                          prt)
        group_list = self._list.groups(group_filter, file_tag, group_token,
                                       prt)
        for group in group_list:
            for project in projId_list:
                ...
                if len(sa_emails) == 0:
                    ...
                    continue
                group_members = self._list.members(group, file_tag,
                                                   group_token, prt)
                ...
                while add_sas and retry:
                    ...
                    for email in add_sas:
                        batch.add(admin.members().insert(groupKey=group,
                                                         body={
                                                             "email": email,
                                                             "role": "MEMBER"
                                                         }))
    ...
    def user(self, td_id, user, role="organizer"):
        ...
        return (drive.permissions().create(body=body,
                                           fileId=td_id,
...
class Remove(ut.Help):
    ...
    def sas(self,
        ...
        for project in projId_list:
            ...
            while sas and retry:
                if len(sas) == 0:
                    print(
                        f"0 service accounts in {project}. Moving to next project"
                    )
                    continue
                batch = ut.BatchJob(iam)
                ...
                for i in sas:
                    ...
                    batch.add(
                        iam.projects().serviceAccounts().delete(name=name))
                batch.execute()
                sas, _ = self._list.sas(project, True, file_tag, token, False)
    ...
    def jsons(self, filter="", sa_path=cf.sa_path):
        ...
        delete_files = [
            i for i in sorted(files) if ".json" and str(filter) in i
        ]
    ...
    def members(
        ...
        group_list = self._list.groups(group_filter, file_tag, group_token,
                                       prt)
        for group in group_list:
            group_members = self._list.members(group, file_tag, group_token,
                                               prt)
            while len(group_members) > 1 and retry:
                ...
                for member in group_members[:batch_size]:
                    batch.add(admin.members().delete(groupKey=group,
                                                     memberKey=member))
                batch.execute()
                batch = ut.BatchJob(admin)
                group_members = self._list.members(group, file_tag,
    ...
    def user(self, td_id, user, role="organizer", token=cf.token):
        ...
        drvsvc = self._svc(*cf.DRIVE, token)
        return (drvsvc.permissions().delete(permissionId=user,
                                            fileId=td_id,
                                            supportsAllDrives=True,
                                            fields="id").execute().get("id"))
                ...
                try:
                    with open(dir + file) as f:
                        ...
                        if rename_type == "email":
                            new_name = data["client_email"].split(
                                "@")[0] + ".json"
