...
%if char.type == 'string':
${ t.padAfter("#define {0}_set_{1}(val)".format(obj.prefix,char.name.lower()) , 45)}${"(&{0}_svc.m{1}, (uint8_t*)(val), strlen(val))".format(obj.prefix, t.camelCase(char.name))}
%else:
...
${"{0}* {1}_get_{2}_cache();".format(t.cTypeDict[char.type], obj.prefix,char.name.lower())}
%else:
