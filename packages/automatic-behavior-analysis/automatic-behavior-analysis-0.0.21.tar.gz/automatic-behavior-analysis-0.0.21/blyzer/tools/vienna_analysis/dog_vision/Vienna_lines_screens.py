...
def get_co(borders_tl_x, borders_tl_y, borders_br_x, borders_br_y):
    """
    :param borders_tl_x: X coordinate of Top-Left point of the experiment space
    :param borders_tl_y:
    :param borders_br_x:
    :param borders_br_y:
...
def main():
    ...
    '''
    img = get_image(r"C:\Users\lielm\Documents\dog_vision\Velvet_session_1_trial_1DLC_resnet50_ViennaJan15shuffle1_34000_labeled.mp4", 1)
    data = get_data(img)
    print(get_data(img))
    ...
    p1 = Point(0, data[1])
    p2 = Point(1280, data[1])
    print("tl: ", get_borders(img)[0] ,"br: ", get_borders(img)[1])
    vert = Line(p1, p2)
    #print(vert.get_p2().getX(), vert.get_p2().getY())
    #vert = vert.rotate_line(math.radians(1))############################################################################
    #print(vert.get_p2().getX(), vert.get_p2().getY())
    cv2.line(img, (vert.get_p1().getX(), vert.get_p1().getY()), (vert.get_p2().getX(), vert.get_p2().getY()), 1)
    cv2.imshow('test', img)
    while True:
        if cv2.waitKey(1) & 0xFF == ord('q'):
            cv2.destroyAllWindows()
            break
    get_image(r"C:\Users\lielm\Documents\dog_vision\Velvet_session_1_trial_1DLC_resnet50_ViennaJan15shuffle1_34000_labeled.mp4", 300)
