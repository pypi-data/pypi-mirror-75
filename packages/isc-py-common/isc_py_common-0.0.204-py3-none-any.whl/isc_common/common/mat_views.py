...
from isc_common.common import blinkString, uuid4
...
def create_insert_update_delete_function_of_table(
    ...
    with connection.cursor() as cursor:
        ...
        rows_without_pk_field = [row for row in rows if row[0] not in pk_names]
        ...
        def func_where_clause_str1(param_item, prefix1=None, prefix2=None):
            prefix2 = '' if prefix2 == None else f'{prefix2}.'
        ...
        fields = ','.join([f'"{row[0]}"' for row in rows])
