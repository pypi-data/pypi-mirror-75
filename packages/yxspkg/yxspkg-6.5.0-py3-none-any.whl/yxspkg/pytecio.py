...
GLOBAL_DLL = [None]  
def get_dll():
    p = Path(__file__).parent / 'lib_file'
    if not p.is_dir():
        os.makedirs(p)

    if sys.platform.startswith('win'):
        dll_path = p / '2017r2_tecio_dll.py'

    elif sys.platform.startswith('linux'):
        dll_path = p / '2017r2_tecio_so.py'
        ...
        for i,zone_name in enumerate(dataset.nameZones):
            ...
            for i_key in dataset.nameVars:
                if i_key in zone_set:
                    _ = zone_set[i_key]
            info['passiveVarList'] = [0 if zone_set.get(i,empty).any() else 1 for i in dataset.nameVars]
