...
def generate_poster_all(dirname,time_interval,shuffle):
    for root,_,_ in os.walk(dirname):
        generate_poster(root,time_interval,shuffle)
    generate_poster(dirname,time_interval,shuffle)
@click.command()
@click.argument('dirname')
@click.option('--time_interval','-t',default=15.0,help="视频截图时间戳")
@click.option('--shuffle',default=False,help="打乱文件顺序",is_flag=True)
def main(dirname,time_interval,shuffle=False):
    if dirname:
        return generate_poster_all(dirname,time_interval,shuffle)
