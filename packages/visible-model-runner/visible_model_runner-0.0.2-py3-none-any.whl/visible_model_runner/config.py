...
from visible_model_runner import development
...
# file be the same order as the sections appear with `visible_model_runner config show`)
...
def _create_option(
    ...
    is requested. To prevent this, use the `visible_model_runner.util.memoize` decorator as
    follows:
...
_create_option(
    ...
    description="Hide deprecation warnings in the visible_model_runner app.",
    visibility="hidden",
    default_val=False,
    type_=bool,
)
...
def show_config():
    ...
    out.append(
        ...
        ~/.visible_model_runner/config.toml.
    """
        )
    )
...
def parse_config_file(force=False):
    ...
    # Read ~/.visible_model_runner/config.toml, and then overlay
    # $CWD/.visible_model_runner/config.toml if it exists.
    config_filenames = [
        file_util.get_visible_model_runner_file_path("config.toml"),
        file_util.get_project_visible_model_runner_file_path("config.toml"),
    ]
