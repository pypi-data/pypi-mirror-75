...
How to use Streamlit in 3 seconds:
    ...
    2. Run your app
    $ visible_model_runner run my_script.py
...
Streamlit can do:
    ...
    >>> dir(visible_model_runner)

Or try running our "Hello World":

    $ visible_model_runner hello

For more detailed info, see https://docs.visible_model_runner.io.
"""
...
from visible_model_runner import logger as _logger
from visible_model_runner import config as _config
...
# This used to be pkg_resources.require('visible_model_runner') but it would cause
# pex files to fail. See #394 for more details.
__version__ = _pkg_resources.get_distribution("visible_model_runner").version
...
from visible_model_runner import code_util as _code_util
from visible_model_runner import env_util as _env_util
from visible_model_runner import source_util as _source_util
from visible_model_runner import string_util as _string_util
from visible_model_runner import type_util as _type_util
from visible_model_runner.DeltaGenerator import DeltaGenerator as _DeltaGenerator
from visible_model_runner.ReportThread import add_report_ctx as _add_report_ctx
from visible_model_runner.ReportThread import get_report_ctx as _get_report_ctx
from visible_model_runner.errors import StreamlitAPIException
from visible_model_runner.proto import BlockPath_pb2 as _BlockPath_pb2
from visible_model_runner.util import functools_wraps as _functools_wraps

# Modules that the user should have access to. These are imported with "as"
# syntax pass mypy checking with implicit_reexport disabled.
from visible_model_runner.caching import cache as cache  # noqa: F401
...
_is_running_with_visible_model_runner = False
...
def set_option(key, value):
    ...
    Run `visible_model_runner config show` in the terminal to see all available options.

    Parameters
    ----------
    key : str
        The config option key of the form "section.optionName". To see all
        available options, run `visible_model_runner config show` on a terminal.
    ...
    raise StreamlitAPIException(
        "{key} cannot be set on the fly. Set as command line option, e.g. visible_model_runner run script.py --{key}, or in config.toml instead.".format(
            key=key
        )
    )
...
def write(*args, **kwargs):
    ...
    *args : any
    ...
    .. output::
       https://share.visible_model_runner.io/0.50.2-ZWk9/index.html?id=Pn5sjhgNs4a8ZbiUoSTRxE
       height: 50px
    ...
    .. output::
       https://share.visible_model_runner.io/0.25.0-2JkNY/index.html?id=FCp9AMJHwHRsWSiqMgUZGD
       height: 250px
    ...
    .. output::
       https://share.visible_model_runner.io/0.25.0-2JkNY/index.html?id=DHkcU72sxYcGarkFbf4kK1
       height: 300px
...
def experimental_show(*args):
    ...
    https://docs.visible_model_runner.io/en/latest/pre_release_features.html for more information.
...
def spinner(text="In progress..."):
    ...
    """
    import visible_model_runner.caching as caching

    # @st.cache optionally uses spinner for long-running computations.
    # Normally, visible_model_runner warns the user when they call st functions
...
# 'visible_model_runner run', but we need to make sure the warning appears only once no
        ...
        if _env_util.is_repl():
            _LOGGER.warning(
                ...
                Will not generate Streamlit app
                  ...
                  $ visible_model_runner run [FILE_NAME] [ARGUMENTS]
            ...
            _LOGGER.warning(
                ...
                Will not generate Streamlit App
                  ...
                  $ visible_model_runner run %s [ARGUMENTS]
