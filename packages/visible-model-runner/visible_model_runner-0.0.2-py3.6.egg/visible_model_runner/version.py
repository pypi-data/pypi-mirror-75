...
PYPI_STREAMLIT_URL = "https://pypi.org/pypi/visible_model_runner/json"

# Probability that we'll make a network call to PyPI to check
# the latest version of visible_model_runner. This is used each time
...
def _get_installed_visible_model_runner_version():
    """Return the visible_model_runner version string from setup.py.
    ...
    """
    version_str = pkg_resources.get_distribution("visible_model_runner").version
    return _version_str_to_obj(version_str)


def _get_latest_visible_model_runner_version(timeout=None):
    """Request the latest visible_model_runner version string from PyPI.
    ...
    -------
    str
        The version string for the latest version of visible_model_runner
        on PyPI.
...
def should_show_new_version_notice():
    """True if visible_model_runner should show a 'new version!' notice to the user.

    We need to make a network call to PyPI to determine the latest visible_model_runner
    version. Since we don't want to do this every time visible_model_runner is run,
    ...
    -------
    bool
        True if we should tell the user that their visible_model_runner is out of date.
    ...
    try:
        installed_version = _get_installed_visible_model_runner_version()
        latest_version = _get_latest_visible_model_runner_version(timeout=1)
