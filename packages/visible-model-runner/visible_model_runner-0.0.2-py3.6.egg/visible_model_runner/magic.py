...
def _build_st_import_statement():
    """Build AST node for `import visible_model_runner as __visible_model_runner__`."""
    return ast.Import(names=[ast.alias(name="visible_model_runner", asname="__visible_model_runner__")])


def _build_st_write_call(nodes):
    """Build AST node for `__visible_model_runner__._transparent_write(*nodes)`."""
    return ast.Call(
        func=ast.Attribute(
            attr="_transparent_write",
            value=ast.Name(id="__visible_model_runner__", ctx=ast.Load()),
            ctx=ast.Load(),
        ),
        args=nodes,
        keywords=[],
