...
if env_util.IS_WINDOWS:
    _CONFIG_FILE_PATH = r"%userprofile%/.visible_model_runner/config.toml"
else:
    _CONFIG_FILE_PATH = "~/.visible_model_runner/config.toml"
...
""" % {
    ...
    "link": click.style("https://visible_model_runner.io/privacy-policy", underline=True),
...
""" % {
    ...
    "hello": click.style("visible_model_runner hello", bold=True),
}
...
class Credentials(object):
    ...
    def load(self, auto_resolve=False) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                'Credentials not found. Please run "visible_model_runner activate".'
            )
            ...
            raise Exception(
                ...
                Run "visible_model_runner reset" and try again.
                """
                )
                % (self._conf_file)
            )
    ...
    def _check_activated(self, auto_resolve=True):
        """Check if visible_model_runner is activated.

        Used by `visible_model_runner run script.py`
        """
        try:
    ...
    def reset(cls):
        ...
        This is used by `visible_model_runner activate reset` in case a user wants
        to start over.
        """
    ...
    def activate(self, show_instructions: bool = True) -> None:
        ...
        Used by `visible_model_runner activate`.
        """
        try:
            self.load()
        except RuntimeError:
        ...
        if self.activation:
            if self.activation.is_valid:
                ...
                _exit(
                    ...
                    "`visible_model_runner activate reset` then `visible_model_runner activate`"
                )
        else:
            activated = False
...
def _get_credential_file_path():
    return file_util.get_visible_model_runner_file_path("credentials.toml")
