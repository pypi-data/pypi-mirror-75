...
import visible_model_runner
from visible_model_runner.credentials import Credentials, check_credentials
from visible_model_runner import version
import visible_model_runner.bootstrap as bootstrap
from visible_model_runner.case_converters import to_snake_case
...
NEW_VERSION_TEXT = """
  ...
  See what's new at https://discuss.visible_model_runner.io/c/announcements
...
""" % {
    ...
    "command": click.style("pip install visible_model_runner --upgrade", bold=True),
}
...
def _apply_config_options_from_cli(kwargs):
    """The "visible_model_runner run" command supports passing Streamlit's config options
    as flags.
...
def main(ctx, log_level="info"):
    ...
    Or use the line below to run your own script:
        ...
        $ visible_model_runner run your_script.py
    """

    if log_level:
        import visible_model_runner.logger

        visible_model_runner.logger.set_log_level(log_level.upper())
...
def help(ctx):
    ...
    # Pretend user typed 'visible_model_runner --help' instead of 'visible_model_runner help'.
    import sys
...
def main_version(ctx):
    ...
    # Pretend user typed 'visible_model_runner --version' instead of 'visible_model_runner version'
    import sys
...
def main_docs():
    ...
    from visible_model_runner import util

    util.open_browser("https://docs.visible_model_runner.io")
...
def main_hello(**kwargs):
    ...
    from visible_model_runner.hello import hello
...
def main_run(target, args=None, **kwargs):
    ...
    # 添加适配现有打包工具解压模型文件代码
    model_zip_path = '/app/model/model.zip'
    model_dir = '/app/model'
    ZipFile(model_zip_path).extractall(model_dir)
    os.system('mv /app/model/model/scripts/user_provided/* /app')
    ...
    if extension[1:] not in ACCEPTED_FILE_EXTENSIONS:
        raise click.BadArgumentUsage(
            "Streamlit requires raw Python (.py) files, not %s.\nFor more information, please see https://docs.visible_model_runner.io"
            % extension
        )

    if url(target):
        from visible_model_runner.temporary_directory import TemporaryDirectory

        with TemporaryDirectory() as temp_dir:
            from urllib.parse import urlparse
            from visible_model_runner import url_util
...
def _main_run(file, args=[]):
    ...
    # Set a global flag indicating that we're "within" visible_model_runner.
    visible_model_runner._is_running_with_visible_model_runner = True

    # Check credentials.
    check_credentials()

    # Notify if visible_model_runner is out of date.
...
def cache_clear():
    ...
    import visible_model_runner.caching

    result = visible_model_runner.caching.clear_cache()
    cache_path = visible_model_runner.caching.get_cache_path()
    if result:
