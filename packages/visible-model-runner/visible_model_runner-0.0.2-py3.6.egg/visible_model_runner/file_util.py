...
from visible_model_runner import env_util
from visible_model_runner import util
from visible_model_runner.string_util import is_binary_string

# Configuration and credentials are stored inside the ~/.visible_model_runner folder
CONFIG_FOLDER_NAME = ".visible_model_runner"
...
def visible_model_runner_read(path, binary=False):
    """Opens a context to read this file relative to the visible_model_runner path.

    For example:

    with visible_model_runner_read('foo.txt') as foo:
        ...

    opens the file `%s/foo.txt`

    path   - the path to write to (within the visible_model_runner directory)
    binary - set to True for binary IO
    """ % CONFIG_FOLDER_NAME
    filename = get_visible_model_runner_file_path(path)
...
@contextlib.contextmanager
def visible_model_runner_write(path, binary=False):
    """
    Opens a file for writing within the visible_model_runner path, and
    ensuring that the path exists. For example:

        with visible_model_runner_write('foo/bar.txt') as bar:
            ...
    ...
    if binary:
        mode += "b"
    path = get_visible_model_runner_file_path(path)
...
def get_visible_model_runner_file_path(*filepath):
    """Return the full path to a file in ~/.visible_model_runner.
...
def get_project_visible_model_runner_file_path(*filepath):
    """Return the full path to a filepath in ${CWD}/.visible_model_runner.
