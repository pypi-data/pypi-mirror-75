...
def _publish_message2_pubsub(topic_name, message_json):
    ...
    while True:
        try:
            ...
            topic_path = publisher.topic_path(env.project_id, topic_name)
...
class ManageUserSlackChatMessage:
    def __init__(self, user, creds, action='start'):
        self.creds = creds
        self.action = action
    ...
    def send(self):
        ...
        _publish_message2_pubsub(env.manage_user_slack_chat_client_topic, message_json)
...
class PatchBotsMessage:
    ...
    def send(self):
        message_json = json.dumps(self.__dict__, ensure_ascii=False, default=_json_converter)
        _publish_message2_pubsub(env.bot_manager_pubsub_topic, message_json)
...
class SmtpMessage:
    ...
    def send(self):
        message_json = json.dumps(self.__dict__, ensure_ascii=False, default=_json_converter)
        _publish_message2_pubsub(env.smtp_pubsub_topic, message_json)
...
class AnalyticsMessage:
    ...
    def send(self):
        message_json = json.dumps(self.__dict__, ensure_ascii=False, default=_json_converter)
        _publish_message2_pubsub(env.analytics_pubsub_topic, message_json)
