...
from arkindex_cli.commands.imports.base import get_global_report


def add_report_parser(subcommands) -> None:
    report_parser = subcommands.add_parser(
        "report",
        description="Build a global ML report for a DataImport",
        help="Build a global ML report for a DataImport",
    )
    report_parser.add_argument(
        "import_id", type=UUID, help="ID of the DataImport to retrieve the status from"
    )
    report_parser.add_argument(
        "-r",
        "--run",
