...
class JsonStoreBase(ABC):
    ...
    def insert_many(self, insert_list: Union[str, List[Dict]]):
        insert_list = dumps(insert_list)
        query = f"for $i in {insert_list} return insert json $i into jn:doc({self.db_name},{self.name}) at position 0"
        # query = "insert json (1,2,3) into [] at position 0"
