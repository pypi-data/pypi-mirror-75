...
def supplement_paper_processing(ds, src_data, dest_data, timestamp=None):
    ...
    ds.get_numeric_writer(flt_asmts, 'prediction_score', 'float32').write(scores)
    ...
    ds.get_numeric_writer(flt_asmts, 'prediction', 'uint8').write(predictions)
    ...
    for k in flt_asmts.keys():
        with utils.Timer(f"applying selected_assessment indices to {k} "):
            ...
            ds.apply_index(max_score_indices, reader,
