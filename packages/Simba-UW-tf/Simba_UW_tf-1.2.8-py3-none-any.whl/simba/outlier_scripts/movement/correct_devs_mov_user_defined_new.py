...
bodyPartNameArrayForMeans = np.empty((animal_no, 7), dtype=object)
for animal in range(animal_no):
    ...
    animalName = '_' + str(animal+1)
    bodyPartNameArrayForMeans[animal] = [animalName, bodyPart_1_X, bodyPart_1_Y, bodyPart_2_X, bodyPart_2_Y, str(bodyPart_1_X + '_shifted'), str(bodyPart_1_Y + '_shifted')]
...
if animalIDlist[0] != '':
    for animal in range(len(animalIDlist)):
        ...
        bodyPartNameArrayForMeans[animal][1:7] = [currentID + x for x in bodyPartNameArrayForMeans[animal][1:7]]
else:
...
########### CREATE SHIFTED DATAFRAME FOR DISTANCE CALCULATIONS ###########################################
    ...
    for animal in range(len(bodyPartNameArrayForMeans)):
        ...
        mean1size = statistics.mean(np.sqrt((csv_df[bodyPartNameArrayForMeans[animal][1]] - csv_df[bodyPartNameArrayForMeans[animal][3]]) ** 2 + (csv_df[bodyPartNameArrayForMeans[animal][2]] - csv_df[bodyPartNameArrayForMeans[animal][4]]) ** 2)) * criterion
        bplist1x, bplist1y, bpcorrected_list1x, bpcorrected_list1y = [], [], [], []
        currXcols, currYcols, currPcols = [i for i in x_cols if animalIDlist[animal] in i], [i for i in y_cols if animalIDlist[animal] in i], [i for i in p_cols if animalIDlist[animal] in i]
        for bp in currXcols:
            bplist1x.append(bp)
        ...
        for idx, col1x in enumerate(bplist1x):
            ...
            csv_df_combined.reset_index(inplace=True)
        print(csv_df_combined)
        for cols in range(len(currXcols)):
            csv_out = pd.concat([csv_out, csv_df_combined[currXcols[cols]], csv_df_combined[currYcols[cols]], df_p_cols[currPcols[cols]]], axis=1)
