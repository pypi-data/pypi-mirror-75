...
def plotsklearnresult(iniFile,videoSetting, frameSetting):
    ...
    if multiAnimalIDList[0] != '': multiAnimalStatus = True
    else: multiAnimalStatus = False
        ...
        while (cap.isOpened()):
            ...
            if ret == True:
                for currAnimal in range(animalsNo):
                    ...
                    currNoBps, hullColor = len(currentDict['X_bps']), currentDict['colors'][0]
                    IDappendFlag = False
                    for bp in range(currNoBps):
                        ...
                        currAnimal = np.array(currAnimal).astype(int)
                        currAnimal = np.reshape(currAnimal, (-1, 2))
                        M1polyglon_array_hull = cv2.convexHull((currAnimal.astype(int)))
                        cv2.drawContours(frame, M1polyglon_array_hull, 0, hullColor, circleScale)
                        if (multiAnimalStatus == True):
                            if ('Centroid' in currXheader) or ('Center' in currXheader) or ('centroid' in currXheader) or ('center' in currXheader):
                                IDlabelLoc.append([currAnimal[0][0], currAnimal[0][1]])
                                IDappendFlag = True
