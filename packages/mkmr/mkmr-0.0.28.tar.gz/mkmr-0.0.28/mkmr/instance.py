...
class Instance:
    ...
    def __init__(self, options, origin: bool = False, offline: bool = False):
        ...
        if origin:
            if not self.config.get("origin") or self.config.get("origin") and options.save:
                self.config.set("origin", options.origin)
        else:
            if hasattr(options, "remote"):
                if not self.config.get("upstream") or self.config.get("upstream") and options.save:
        ...
        if hasattr(options, "yes"):
            if options.yes is None:
                ...
                self.config.getboolean("yes", subsection=__BIN__) is None
                or options.save
                and self.config.getboolean("yes", subsection=__BIN__) != options.yes
            ):
        ...
        if hasattr(options, "quiet"):
            if options.quiet is None:
                ...
                self.config.getboolean("quiet", subsection=__BIN__) is None
                or options.save
                and self.config.getboolean("quiet", subsection=__BIN__) != options.quiet
            ):
        ...
        if hasattr(options, "edit"):
            if options.edit is None:
                ...
                self.config.getboolean("edit", subsection=__BIN__) is None
                or options.save
                and self.config.getboolean("edit", subsection=__BIN__) != options.edit
            ):
        ...
        if hasattr(options, "timeout"):
            if options.timeout is None:
                ...
                self.config.getint("timeout", subsection=__BIN__) is None
                or options.save
                and self.config.getint("timeout", subsection=__BIN__) != options.timeout
            ):
        ...
        if hasattr(options, "wait"):
            if options.wait is None:
                ...
                if options.wait is None:
                    options.wait = 4  # Default value
            elif (
                self.config.getint("wait", subsection=__BIN__) is None
                or options.save
                and self.config.getint("wait", subsection=__BIN__) != options.wait
            ):
