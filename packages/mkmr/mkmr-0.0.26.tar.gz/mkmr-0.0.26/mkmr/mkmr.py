...
def main():
    ...
    except (Instance.BranchNoCache, Instance.BranchNothingCleaned):
        # We pass here instead of warning the user that there is no cache because
        # --origin is almost always never has a cache to begin with as merge requests
        # are made against --upstream and are stored in --upstream namespace
        pass
    except Instance.BranchCacheCleaned:
        msg("Removed dangling branch references for remote '{}'".format(options.origin))
    else:
        options.origin = origin.remote

    try:
        options.remote = options.upstream
        upstream = Instance(options, remote=options.upstream)
    except (ValueError, AttributeError) as e:
        err(e)
        sys.exit(1)
    except Instance.ProjectIdReset:
        msg("Reset project-id for remote '{}'".format(options.upstream))
    except Instance.BranchNoCache:
        msg("No branch cache for remote '{}'".format(options.upstream))
    except Instance.BranchNothingCleaned:
        msg("Nothing in the cache needs to cleaned for remote '{}'".format(options.upstream))
    except Instance.BranchCacheCleaned:
        msg("Removed dangling branch references for remote '{}'".format(options.upstream))
    else:
        options.upstream = upstream.remote

    if options.clean_branch_cache or options.reset_project_id:
        sys.exit(0)
