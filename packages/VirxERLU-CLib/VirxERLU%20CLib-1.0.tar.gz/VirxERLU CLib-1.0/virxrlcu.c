...
struct opti_norm normalize(Vector vec) {
    ...
    r.vector = (r.magnitude == 0) ? (Vector){0, 0, 0} : (Vector){
        .x=vec.x/r.magnitude,
        .y=vec.y/r.magnitude,
        .z=vec.z/r.magnitude
    };
...
Vector clamp(Vector vec, Vector start, Vector end) {
    ...
    _Bool right = dot(s, cross(end, (Vector){.x=0, .y=0, .z=-1})) < 0;
    _Bool left = dot(s, cross(start, (Vector){.x=0, .y=0, .z=-1})) > 0;

    if ((dot(end, cross(start, (Vector){.x=0, .y=0, .z=-1})) > 0) ? (right && left) : (right || left))
        return vec;
...
struct post_correction correct_for_posts(Vector ball_location, Vector left_target, Vector right_target) {
    ...
    Vector goal_line_perp = cross(sub(right_target, left_target), (Vector){.x=0, .y=0, .z=1});
    Vector left = add(left_target, cross(normalize(sub(left_target, ball_location)).vector, multiply((Vector){.x=0, .y=0, .z=1}, double_to_vector(ball_radius))));
    Vector right = add(right_target, cross(normalize(sub(right_target, ball_location)).vector, multiply((Vector){.x=0, .y=0, .z=1}, double_to_vector(ball_radius))));
    ...
    r.swapped = dot(cross(normalize(sub(left, ball_location)).vector, (Vector){.x=0, .y=0, .z=1}), normalize(sub(right, ball_location)).vector) > -0.1;
...
_Bool is_fast_shot(Vector car_location, Vector ball_intercept, double time_remaining, double cap_) {
    return time_remaining <= min(cap_, distance(car_location, ball_intercept));
}

double find_slope(Vector shot_vector, Vector car_to_target) {
    double d = dot(shot_vector, car_to_target);
    double e = fabs(dot(cross(shot_vector, (Vector){.x=0, .y=0, .z=1}), car_to_target));
...
struct jump_shot parse_slice_for_jump_shot_with_target(double time_remaining, Vector ball_location, Vector car_location, Vector car_forward, int car_boost, Vector left_target, Vector right_target, double cap_) {
    ...
    if (forward_flag || backward_flag) {
        ...
        if (!pst_crrctn.swapped) {
            ...
            if (in_field(sub(ball_location, multiply(r.best_shot_vector, double_to_vector(200))), 1) && is_fast_shot(car_location, ball_location, time_remaining, cap_)) {
...
struct jump_shot parse_slice_for_jump_shot(double time_remaining, Vector ball_location, Vector car_location, Vector car_forward, int car_boost, double cap_) {
    ...
    if ((forward_flag || backward_flag) && in_field(sub(ball_location, multiply(r.best_shot_vector, double_to_vector(200))), 1) && is_fast_shot(car_location, ball_location, time_remaining, cap_)) {
        double slope = find_slope(r.best_shot_vector, car_to_ball);
        if (forward_flag && ball_location.z <= 275 && slope > 0) {
            r.found = 1;
            r.direction = 1;
