...
from .structs import Demand, Commodity, EMPTY_ENV
...
class Table:
    ...
    __slots__ = ("attrs", "columns_headers", "rows")
...
def generate_tables(
    ...
    *,
    ...
    Unless `single_table` is True, each procedure will produce *at least* a
    table.  If `single_table` is True all procedures are *collapsed* into a
    single table; the first column of the table will be procedures' names.

    .. warning:: The `single_table` argument is not stable and we may remove
       it.

       This is only used to produce a single sheet if spreadsheet files, even
       if you pass several procedures.  However, I believe this argument
       complicates a bit the code of this module for little value.  The caller
       may tweak the `table_format` to be sure all procedures follow the same
       column pattern and either choose to create a single sheet or several as
       it deems suitable.
    ...
    if single_table:
        ...
        if alltables:
            yield Table(
                ...
                (row for table in alltables for row in table.rows),
            )
    else:
        yield from tables
...
def _chunk_and_resolve_in_rust(
    ...
    for chunk in slides(rows, chunk_size, fill=Unset):
        ...
        # a procedure and the allows to replace PriceCell in `results`.
        tally: Dict[Procedure, Tuple[List[Tuple[int, int]], List[Demand]]] = {}
        programs: Dict[Procedure, Program] = ProgramDict()
        result: List[List[Any]] = []  # accumulated rows
        for rowindex, row in enumerate(row for row in chunk if row is not Unset):
            result.append(list(row))
            for colindex, cell in enumerate(row):
