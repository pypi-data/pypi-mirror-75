...
class Demand(BaseDemand[Commodity]):
    ...
    def to_unitary_demand(self, strict=False) -> UnitaryDemand:
        """Return a unitary demand from this one if possible.

        If this is not an unitary demand (one with a single request) and
        strict is True raise a ValueError, it `strict` is False return None.
        ...
        if len(self.requests) == 1:
            try:
                ...
                attrs = {
                    ...
                    and isinstance(
                        value, (datetime, timedelta, int, float, str, BaseModel)
                    )
                }
                ...
                logger.exception("Couldn't convert the Demand to Unitary")
                raise
        else:
