...
@pytest.fixture()
def event_with_query():
    event = {'query': {'geo': ['northamerica'], 'id': ['uuid']},
             'credentials': {'username': 'myUser', 'password': 'myPassword!', 'api_location': 'northamerica'}}
    return event
...
def test_get_servers_with_filters(mock_http_client_function, cloud_control_client, event_with_query):
    json_file = open(os.path.join(module_dir, 'json/servers_all_pages.json'), 'r')
    mock_http_client_function.return_value = json.load(json_file)

    of_type = cloud_control_client.get_servers(event_with_query)
...
def test_get_servers_with_multi_value_filters(mock_http_client_function, cloud_control_client):
    ...
    mock_http_client_function.return_value = json.load(json_file)
    event = {'query': {'geo': ['northamerica'], 'id': ['uuid', 'uuid2']},
             'credentials': {'username': 'myUser', 'password': 'myPassword!', 'api_location': 'northamerica'}}
...
def test_get_servers_with_ignored_filters_excluded(mock_http_client_function, cloud_control_client):
    json_file = open(os.path.join(module_dir, 'json/servers_all_pages.json'), 'r')
    mock_http_client_function.return_value = json.load(json_file)
    event = {'query': {'geo': ['northamerica'], 'provider_id': ['provider_id'], 'account_id': ['account_id'],
...
@patch('cloud_control_common.http_client.HttpClient.get_with_filters')
def test_get_datacenters_with_filters(mock_http_client_function, cloud_control_client, event_with_query):
    json_file = open(os.path.join(module_dir, 'json/datacenters.json'), 'r')
    mock_http_client_function.return_value = json.load(json_file)

    of_type = cloud_control_client.get_datacenters(event_with_query)
