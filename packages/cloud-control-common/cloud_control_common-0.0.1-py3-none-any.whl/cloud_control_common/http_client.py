...
class HttpClient:
    ...
    def get_page(self, path, page_number, page_size=250):
        filters = {'pageNumber': page_number, 'pageSize': page_size}
        response = self._session.request('GET', '{0}{1}'.format(self._get_base_path(), path), params=filters)
        if response.status_code != 200:
            self._report_error_status(response)
        else:
            return response.json()
