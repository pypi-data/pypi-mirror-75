...
from azure.mgmt.monitor.v2018_01_01.models import MetricValue, Response
...
class AzureAppService(AppService):
    ...
    def _is_route_healthy(self, ag: ApplicationGateway, route: IngressRouteRequest) -> bool:
        ...
        backend_settings_pool = self._get_route_backend_pool(ag, route)

        # timeout = next((a.healthcheck_timeout for a in self._config.apps if a.name == identifier.name))
        try:
            Utils.wait_for(func=lambda: self._is_backend_pool_healthy(ag, backend_settings_pool, route),
                           interval_sec=30,
                           max_retries=5)
    ...
    @staticmethod
    def _get_route_backend_pool(ag: ApplicationGateway, route: IngressRouteRequest) -> str:
        ...
        backend_settings_pool = f'{backend_address_pool.name} {backend_http_settings.name}'

        return backend_settings_pool

    def _is_backend_pool_healthy(self, ag: ApplicationGateway, backend_settings_pool: str,
                                 route: IngressRouteRequest) -> bool:

        query_filter = f"BackendSettingsPool eq '{backend_settings_pool}'"
        ...
        if not metrics_data.value[0].timeseries.any():
            return False
        last_probe: MetricValue = metrics_data.value[0].timeseries[0].data[-1]
