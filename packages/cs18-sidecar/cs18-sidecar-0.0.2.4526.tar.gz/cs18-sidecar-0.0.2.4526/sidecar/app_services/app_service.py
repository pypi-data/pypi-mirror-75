...
from azure.mgmt.monitor.v2018_01_01.models import MetricValue, Response, TimeSeriesElement
from azure.mgmt.network.v2018_08_01.models import ApplicationGateway
...
from sidecar.status_maintainer import IngressRouteRequest
...
class AppService:
    ...
    @abstractmethod
    def ingress_healthcheck(self, identifier: AppInstanceIdentifier) -> bool:
        raise NotImplementedError
...
class AzureAppService(AppService):
    def __init__(self, logger: Logger,
        ...
        self._monitor_client = clients_manager.monitor_client
    ...
    def ingress_healthcheck(self, identifier: AppInstanceIdentifier) -> bool:
        ...
        ag = self._get_running_application_gateway(resource_group_name=rg)
        if ag is None:
            return False

        for route in routes:
            healthy = self._is_route_healthy(ag, route)
            if not healthy:
                return False

        return True

    def _get_running_application_gateway(self, resource_group_name) -> Optional[ApplicationGateway]:
        timeout = 600
        ...
        while True:
            ...
            self._logger.info(f"[IngressHealthcheck] ({attempt}) App gateway {ag.id} operational_state"
                              f" {ag.operational_state}")

            if ag.operational_state == 'Running':
                return ag
            else:
                if datetime.datetime.utcnow() > end:
                    self._logger.info(
                        f"[IngressHealthcheck] Ingress Healthech Failed.\n"
                        f"App gateway {ag.id} operational_state not become Running during {timeout} seconds.")
                    return None
                time.sleep(20)

    def _is_route_healthy(self, ag: ApplicationGateway, route: IngressRouteRequest) -> bool:
        ...
        (backend_address_pool, backend_http_settings) = self._get_route_backend_pool(ag, route)

        try:
            Utils.wait_for(func=lambda: self._is_backend_pool_healthy(ag, backend_address_pool, backend_http_settings),
                           interval_sec=30,
                           max_retries=10)
    ...
    # get BackendSettingsPool measure value to filter application gateway metrics
    @staticmethod
    def _get_route_backend_pool(ag: ApplicationGateway, route: IngressRouteRequest) -> (str, str):
        ...
        backend_address_pool = next((x for x in ag.backend_address_pools
                                     if x.id == path_rule.backend_address_pool.id))
        backend_http_settings = next((x for x in ag.backend_http_settings_collection
                                      if x.id == path_rule.backend_http_settings.id))

        return backend_address_pool.name, backend_http_settings.name

    def _is_backend_pool_healthy(self, ag: ApplicationGateway,
                                 backend_address_pool: str, backend_http_settings: str) -> bool:

        # Measure value may be in formats:
        # '{backend_address_pool} {backend_http_settings}' or
        # '{backend_address_pool}~{backend_http_settings}'
        # I did not found what ~ means and when to use ' ' and when '~'
        # May be it somehow related to internet facing true/false or v1/v2 application gateway
        # So can't use it in filter and it's total crap!
        query_filter = f"BackendSettingsPool eq '*'"
        end = datetime.datetime.now().replace(microsecond=0)
        start = end - datetime.timedelta(minutes=5)

        self._logger.info(f"[IngressHealthcheck] Checking application gateway backend pool metrics\n"
                          f"filter '{query_filter}'")

        metrics_data: Response = self._monitor_client.metrics.list(
            resource_uri=ag.id,
            filter=query_filter,
        ...
        self._logger.info(f'[IngressHealthcheck] Got metric data response\n{metrics_data.serialize()}\n'
                          f'Check values for backend_address_pool:{backend_address_pool} and '
                          f'backend_http_settings:{backend_http_settings}')

        if not any(metrics_data.value[0].timeseries):
            return False
        # See comment regarding query_filter above
        timeseries: Optional[TimeSeriesElement] = \
            next((x for x in metrics_data.value[0].timeseries
                  if x.metadatavalues[0].value == f'{backend_address_pool} {backend_http_settings}' or
                  x.metadatavalues[0].value == f'{backend_address_pool}~{backend_http_settings}'))
        if timeseries is None or not any(timeseries.data):
            return False

        last_probe: MetricValue = timeseries.data[-1]
        self._logger.info(f'[IngressHealthcheck] Last metric value\n{last_probe.serialize()}')

        healthy = last_probe.average == 0
        self._logger.info(f"[IngressHealthcheck] backend_settings_pool healty: {healthy}")
        return healthy
