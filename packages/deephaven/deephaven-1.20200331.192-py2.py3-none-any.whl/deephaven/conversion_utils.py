...
#
"""
Utility functions for converting java objects to python object, or vice versa. 
"""
...
def convertToJavaList(input):
    ...
    Note that the user has most control over type in providing a :class:`numpy.ndarray`, otherwise the
...
def convertToJavaHashMap(input1, input2=None):
    ...
    The user has most control over type in providing keys and values as :class:`numpy.ndarray`. Otherwise, the
...
class NULL_CONVERSION(object):
    ...
    ERROR = 0  #: **[default]** raise exception for integer type *(i.e. byte, short, int, or long)* primitive null values
    PASS = 1  #: do not interpret integer type primitive nulls - leave as the reserved constant value
    CONVERT = 2  #: do pandas style conversion by casting integer type to floating point type, and replacing null values with ``NaN``
