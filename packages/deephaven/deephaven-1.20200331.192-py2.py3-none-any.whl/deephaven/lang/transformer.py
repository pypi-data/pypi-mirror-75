...
class Transformer(NodeTransformer):
    ...
    def next_func_name(cls):
        ...
        Visible for testing"""
        return '__vec{}'.format(cls.unique)
    ...
    def __init__(self, analyzed, return_type, ref_values=None, mode=VecMode.default_mode):
        ...
        self.func_name = '__vec{}'.format(Transformer.unique)
        Transformer.unique += 1

        self.key_result = Transformer.key_result_default if analyzed.last_assign is None \
            else analyzed.last_assign.targets[0].id
        self.index = read(Transformer.key_index)
    ...
    def visit_Module(self, node):
        ...
        if self.mode is VecMode.vectorize:
            ...
            for_loop = For(target=write(Transformer.key_index), iter=shape_range, body=for_body, orelse=[])
            # print our loop
