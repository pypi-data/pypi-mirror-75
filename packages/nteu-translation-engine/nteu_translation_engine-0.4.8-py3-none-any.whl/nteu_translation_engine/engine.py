...
class Engine:
    def __init__(self, config: Dict, log_file: str = None, dbug: bool = False):
        self._logger = self.setup_log(log_file, dbug)
        self._config = config
        self._pipeline = Pipeline(self._config, self._logger)
    ...
    def setup_log(self, log_file: str, dbug: bool) -> "Logger":
    ...
    def log_translation(self, translation: "Translation"):
        if self._logging and self._lvl == logging.DEBUG:
