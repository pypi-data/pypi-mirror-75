        ...
        with conn3:
            ...
            if param == 'T':
                q = """select distinct a.symbol from stock_all a join benchmark_all b 
                        ...
                        where b.is_active=true and b.prio=3 fetch first 5 rows only"""
            else:
                q = """select distinct a.symbol from stock_all a join benchmark_all b 
                        on a.index_code=b.symbol
                        where b.is_active=true and b.prio=3"""
            cursor3.execute(q)
            ...
            hq = """select symbol, count(*) as cnt from stock_history
                    ...
                    group by symbol"""
            if len(val)>0:
                cursor3.execute(hq, (val,))
                hval = cursor3.fetchall()
                # print(hval)
                ...
                print("No stocks in prio3 available in stock_all")
                stklst = []
            if param == 'T':
                q = """select distinct b.symbol from benchmark_all b 
                        where b.is_active=true and b.prio=3 fetch first 5 records only"""
            else:
                q = """select distinct b.symbol from benchmark_all b 
                                    where b.is_active=true and b.prio=3"""
            cursor3.execute(q)
            ...
            if len(val)>0:
                ...
                print("No active benchmark in prio3 available in benchmark_all")
                bmklst = []
            ...
            for i in range(len(stklst)):
                ...
                cursor3.execute(chk,(stk,))
                chkc = cursor3.fetchone()
                chkc = chkc[0]
                if chkc != 0:
                    getlogreturn(stk, 'stock_history', cursor3)
                else:
                    getcompletelogreturn(stk, 'stock_history', cursor3,'tmp3')
                ...
                chk50 = """select count(*) from stock_history
                    ...
                    and price_date > current_date - 31"""
                cursor3.execute(chk50,(stk,))
                chk50c = cursor3.fetchone()
                chk50c = chk50c[0]
                if chk50c != 0:
                    getlastrollingmean.rollingmean50d(stk,'stock_history', cursor3)
                else:
                    getrollingmean.rollingmean50d(stk,'stock_history', cursor3,'tmp3')
                ...
                chk200 = """select count(*) from stock_history
                    ...
                    and price_date > current_date - 31"""
                cursor3.execute(chk200,(stk,))
                chk200c = cursor3.fetchone()
                chk200c = chk200c[0]
                if chk200c != 0:
                    getlastrollingmean.rollingmean200d(stk,'stock_history', cursor3)
                else:
                    getrollingmean.rollingmean200d(stk,'stock_history', cursor3,'tmp3')
            ...
            for i in range(len(bmklst)):
                ...
                chkb = """select count(*) from benchmark_history
                    where symbol=%s and price_return is not null
                    and price_date > current_date - 31"""
                cursor3.execute(chkb,(bmk,))
                chkbc = cursor3.fetchone()
                chkbc = chkbc[0]
                if chkbc != 0:
                    getlogreturn(bmk, 'benchmark_history', cursor3)
                else:
                    getcompletelogreturn(bmk, 'benchmark_history', cursor3,'tmp3')
            ...
            chkretb = """select symbol from dbo.benchmark_history where price_return is null and symbol in %s
                       group by symbol having count(*)>1"""
            if len(stklst)>0:
                cursor3.execute(chkrets, (stklst,))
                failstk = cursor3.fetchall()
                #print(failstk)
                print(failstk, "\n are the list of ", len(failstk), " records from ", len(stklst),
                      " stock history that have no calculated return")
            else:
                print("No need to check empty returns as stock list for prio-3 is empty")
            if len(bmklst)>0:
                cursor3.execute(chkretb, (bmklst,))
                failbmk = cursor3.fetchall()
                print(failbmk, "\n are the list of ",len(failbmk)," records from ",len(bmklst)," benchmark history that have no calculated return")
            else:
                print("No need to calculate empty returns as benchmark list is empty fo prio-3")
    ...
    def statistics(param):
        ...
        with conn3:
            if param == 'T':
                ...
                where rf.prio=3 fetch first 5 rows only"""
            else:
                statqry = """select mas.symbol, mas.exchange, mas.currency,
                            rf.abbr as abbr,
                            rf.country as country, mas.source_table,rf.symbol as bmk_ticker
                            ...
                            where rf.prio=3"""
            try:
                cursor3.execute(statqry)
                statres = cursor3.fetchall()
            ...
            if len(statres)>0:
                for i in range(len(statres)):
                    ...
                    if symbol is not None and bsymbol is not None and country is not None:
                        allstatistics(symbol, bsymbol, country, sourcetable, cursor3)
                        ...
                        cursor3.execute(delq, (symbol, pdate))
                        selq = """select * from stock_statistics where symbol=%s"""
                        cursor3.execute(selq, (symbol, ))
                        statdata = cursor3.fetchone()
