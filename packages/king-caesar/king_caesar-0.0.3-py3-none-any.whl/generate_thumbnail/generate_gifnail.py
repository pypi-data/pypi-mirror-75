...
@timestamp
def generate_gifnail(video_path: str, output_path: str):
    """
    Args:
        - video_path: str
          Video absolute path
    ...
    imageio.mimsave(output_path, buffers, fps=24)
...
def frames_to_buffers(frames: List[Any]) -> List[Any]:
    ...
    for frame in frames:
        ...
        if (width >= size):
            ...
            buffers.append(cv2.resize(rgb_frame, max_size, interpolation=cv2.INTER_AREA))
...
if __name__ == '__main__':
    ...
    generate_gifnail(args.video_path, args.output_path)