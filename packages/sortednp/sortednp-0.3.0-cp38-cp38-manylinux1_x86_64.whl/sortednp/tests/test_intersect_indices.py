...
class Base(metaclass=ABCMeta):
    ...
    def test_pair_dup_w_DROP(self):
        """Check arrays with duplicates in the input and DROP treatment."""
        a = np.array([1, 5, 5, 5, 10], dtype=self.get_dtype())
        b = np.array([5, 5, 7, 10, 21], dtype=self.get_dtype())

        i, (i_a, i_b) = snp.intersect(a, b, duplicates=snp.DROP, indices=True)
        self.assertEqual(list(i), [5, 10])

        # Indices are from first occurrence
        self.assertEqual(list(i_a), [1, 4])
        self.assertEqual(list(i_b), [0, 3])

    def test_pair_dup_w_KEEP_MIN_N(self):
        """Check arrays with duplicates in the input and KEEP_MIN_N treatment."""
        a = np.array([1, 5, 5, 5, 10], dtype=self.get_dtype())
        b = np.array([5, 5, 7, 10, 21], dtype=self.get_dtype())

        i, (i_a, i_b) = snp.intersect(a, b, duplicates=snp.KEEP_MIN_N,
                                      indices=True)
        self.assertEqual(list(i), [5, 5, 10])

        self.assertEqual(list(i_a), [1, 2, 4])
        self.assertEqual(list(i_b), [0, 1, 3])

    def test_pair_dup_w_KEEP_MAX_N(self):
        """Check arrays with duplicates in the input and KEEP_MAX_N treatment."""
        a = np.array([1, 5, 5, 5, 10], dtype=self.get_dtype())
        b = np.array([5, 5, 7, 10, 21], dtype=self.get_dtype())

        i, (i_a, i_b) = snp.intersect(a, b, duplicates=snp.KEEP_MAX_N,
                                      indices=True)
        self.assertEqual(list(i), [5, 5, 5, 10])

        # Last index is duplicated
        self.assertEqual(list(i_a), [1, 2, 3, 4])
        self.assertEqual(list(i_b), [0, 1, 1, 3])
