...
class MergeBase(metaclass=ABCMeta):
    ...
    def test_all_unique_w_DROP(self):
        """Check arrays with unique values and DROP treatment."""
        a = np.array([1, 6, 10], dtype=self.get_dtype())
        b = np.array([5, 7, 11, 21], dtype=self.get_dtype())

        m = snp.merge(a, b, duplicates=snp.DROP)
        self.assertEqual(list(m), [1, 5, 6, 7, 10, 11, 21])

    def test_all_unique_w_DROP_IN_INPUT(self):
        """Check arrays with unique values and DROP_IN_INPUT treatment."""
        a = np.array([1, 6, 10], dtype=self.get_dtype())
        b = np.array([5, 7, 11, 21], dtype=self.get_dtype())

        m = snp.merge(a, b, duplicates=snp.DROP_IN_INPUT)
        self.assertEqual(list(m), [1, 5, 6, 7, 10, 11, 21])

    def test_all_unique_w_KEEP(self):
        """Check arrays with unique values and KEEP treatment."""
        a = np.array([1, 6, 10], dtype=self.get_dtype())
        b = np.array([5, 7, 11, 21], dtype=self.get_dtype())

        m = snp.merge(a, b, duplicates=snp.KEEP)
        self.assertEqual(list(m), [1, 5, 6, 7, 10, 11, 21])

    def test_input_dup_w_DROP(self):
        """Check arrays with duplicates in the input and DROP treatment."""
        a = np.array([1, 6, 10], dtype=self.get_dtype())
        b = np.array([5, 7, 7, 7, 21], dtype=self.get_dtype())

        m = snp.merge(a, b, duplicates=snp.DROP)
        self.assertEqual(list(m), [1, 5, 6, 7, 10, 21])

    def test_input_dup_w_DROP_IN_INPUT(self):
        """Check arrays with duplicates in the input and DROP_IN_INPUT treatment."""
        a = np.array([1, 6, 10], dtype=self.get_dtype())
        b = np.array([5, 7, 7, 7, 21], dtype=self.get_dtype())
    ...
    def test_input_dup_w_KEEP(self):
        """Check arrays with duplicates in the input and KEEP treatment."""
        a = np.array([1, 6, 10], dtype=self.get_dtype())
        b = np.array([5, 7, 7, 7, 21], dtype=self.get_dtype())

        m = snp.merge(a, b, duplicates=snp.KEEP)
        self.assertEqual(list(m), [1, 5, 6, 7, 7, 7, 10, 21])
    ...
    def test_both_dup_w_DROP_IN_INPUT(self):
        ...
        a = np.array([1, 7, 7, 7, 7, 10], dtype=self.get_dtype())
        b = np.array([5, 7, 7, 7, 21], dtype=self.get_dtype())
    ...
    def test_dub_single_w_DROP(self):
        ...
        m = snp.merge(b, a, duplicates=snp.DROP)
        self.assertEqual(list(m), [1, 2, 7, 9])
    ...
    def test_dub_single_w_DROP_IN_INPUT(self):
        ...
        m = snp.merge(b, a, duplicates=snp.DROP_IN_INPUT)
        self.assertEqual(list(m), [1, 2, 7, 9])
    ...
    def test_dub_single_w_KEEP(self):
        ...
        m = snp.merge(b, a, duplicates=snp.KEEP)
        self.assertEqual(list(m), [1, 2, 7, 7, 7, 9])
