...
freedesktop_naming_spec_list = [
		...
		"call-start",
		# The icon used for initiating or accepting a call. Should be similar to the standard cellular call pickup icon, a green handset with ear and mouth pieces facing upward.
		"call-stop",
		# The icon used for stopping a current call. Should be similar to the standard cellular call hangup icon, a red handset with ear and mouth pieces facing downward.
		...
		"process-stop",
		# The icon used for the “Stop” action in applications with actions that may take a while to process, such as web page loading in a browser.
		...
		"process-working",
		# This is the standard spinner animation for web browsers and file managers to show that the location is loading.
		...
		"image-loading",
		# The icon used when another image is being loaded, such as thumnails for larger images in the file manager.
		...
		"network-error",
		# The icon used when an error occurs trying to intialize the network connection of the computing device. This icon should be two computers, one in the background, with the screens of both computers, colored black, and with the theme's style element for errors, overlayed on top of the icon.
		"network-idle",
		...
		"security-low",
		# The icon used to indicate that the security level of a connection is presumed to be insecure, either by using weak encryption, or by using a certificate that the could not be automatically verified, and which the user has not chosent to trust.
...
def test_random_icons(theme):
	...
	print(icon, icon.path if icon else '')
	icon = theme.find_icon("address-book-new", 24, None)
	print(icon, icon.path if icon else '')
	icon = theme.find_icon("application-exit", 24, None)
	print(icon, icon.path if icon else '')
	icon = theme.find_icon("appointment-new", 24, None)
	print(icon, icon.path if icon else '')
	icon = theme.find_icon("edit-cut", 24, None)
	print(icon, icon.path if icon else '')
	icon = theme.find_icon("help-faq", 24, None)
	print(icon, icon.path if icon else '')
	icon = theme.find_icon("dialog-apply", 32, None)
	print(icon, icon.path if icon else '')
	icon = theme.find_icon("audio-card", 32, None)
	print(icon, icon.path if icon else '')
