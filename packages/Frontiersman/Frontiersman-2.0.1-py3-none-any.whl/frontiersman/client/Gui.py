...
class card:
    def __init__(self, location, resource='Sheep'):
...
class card_animation:
    def __init__(self, card_obj, callback, init, dest, steps, delta_time):
        self.card = card_obj
        ...
        actual_frame_rate = 1 / delta_time

        try:
            self.step_size_x = (self.dest[0] - self.init[0]) / steps * (assumed_frame_rate / actual_frame_rate)
            self.step_size_y = (self.dest[1] - self.init[1]) / steps * (assumed_frame_rate / actual_frame_rate)
            self.total_steps *= (actual_frame_rate / assumed_frame_rate)
        except ZeroDivisionError: 
            self.step_size_x = (self.dest[0] - self.init[0]) / steps
            self.step_size_y = (self.dest[1] - self.init[1]) / steps
...
class Client:
        ...
        for card_t in card_type:
            if list_cards[index] > 0:
                ...
                for anything in range(0, list_cards[index]):
                    self.animations.append(
                        card_animation(card_lost, None, location_start, location_end, location_steps, self.time_delta))
            ...
            if list_cards[index] < 0:
                ...
                for anything in range(list_cards[index], 0):
                    self.animations.append(
                        card_animation(card_gained, self.add_card, location_end, location_start, location_steps, self.time_delta))
            index += 1
    ...
    def initialize_gui(self):
        ...
        self.font = pygame.font.Font('./data/fonts/LDFComicSans.ttf', font_size)

        font_size = int(45 * PROTOTYPE_SCALE)
        #self.font_bank = pygame.font.SysFont('Comic Sans MS', font_size)
        self.font_bank = pygame.font.Font('./data/fonts/LDFComicSans.ttf', font_size)
        ...
        if event.user_type == pg_g.UI_BUTTON_PRESSED:
            ...
            if event.ui_element == self.help_button:
                if self.help_window is None:
                    self.help_window = HelpWindow((80, 80), self.manager)
                else:
    ...
    def update_frame(self, clock, time_delta):
