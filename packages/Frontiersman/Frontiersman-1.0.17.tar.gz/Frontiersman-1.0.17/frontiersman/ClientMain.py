    ...
    async def run(self, url, game_id, player_id, player_name, player_color, screen_num, lobby_obj, host=False):
        ...
        def get_longest_road():
            ...
            if (self.client_instance.player.get_longest_road(max_value)):
                self.client_instance.enemy_list[max_index].visibleVictoryPoints = str(int(self.client_instance.enemy_list[max_index].visibleVictoryPoints) - 2)
            ...
            if Response == "quit" or Response == "game not found":  # add more quit contigency!!!!
                ...
                while True:
                    ...
                    if message == "quit":
                        ...
                        self.client_instance.game_board.set_robber((int(message.split(',')[1]), int(message.split(',')[2])))
                        ...
                        if (
                            ...
                            await send_message('winner,' + self.client_instance.player.name)
...
def game_thread_run(client, url, game_code, player_code, name, color, screen_num, window, host):
    ...
    print('thread is dead , baby, thread is dead')
...
def main():
    ...
    def join_game(lobby_obj):
        ...
        threading.Thread(target=game_thread_run, args=(cli, url,game_code,player_code,lobby_obj.name, lobby_obj.color, lobby_obj.screen_num, lobby_obj, lobby_obj.is_host)).start()
        return True
    ...
    test = GameLobby((1600, 900), callbacks=[create_game, join_game, leave_lobby, start])
    ...
    if (code == "create"):
        ...
        color = response[2]
        screen_num = response[3]
        # r = s.get(url+'/get/'+game_code+'/'+player_code)
        # print(r.text)


    else:
        data = {'name': name}
        r = s.post(url + '/join/' + code, data=data)
        print(r.text)
        if (r.text == "game not found"):
        ...
        response = r.text.split('|')
        player_code = response[0].split(',')[0]
        color = response[0].split(',')[1]
        screen_num = response[0].split(',')[2]
        for x in range(1, len(response)):
            self.enemy_list.append(EnemyPlayer(response[x].split(',')[0], response[x].split(',')[1]))
            print(response[x].split(',')[0], response[x].split(',')[1])

    loop = asyncio.get_event_loop()
    try:
        loop.run_until_complete(client().run( url, game_code, player_code, name, color, screen_num, host))
