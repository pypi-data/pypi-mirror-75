...
class CornerNode:
    @staticmethod
    def translate_settlement_old(location, origin, radius, sideLength=3):
        toReturnX = 0
        toReturnY = 0
    ...
    def translate_settlement(location, origin, radius, sideLength=3):
        ...
        x_step = location[0] - 5
        x_factor = 3 ** 0.5 / 2
        x_off = x_factor * x_step * radius

        y_zero = origin[1] - 4 * radius
        y_step = 1 / 2 * radius

        if x_step in (-5, -3, -1, 1, 3, 5):
            y_bonus = location[1] // 2
            y_off = y_step + location[1] * radius + y_bonus * radius
        else:
            y_bonus = (location[1] + 1) // 2
            y_off = location[1] * radius + y_bonus * radius

        return int(origin[0] + x_off + x_diff), int(y_off + y_diff + y_zero)
...
class EdgeNode:
    @staticmethod
    def translate_road_old(location, origin, radius, sideLength=3):
        toReturnX = 0
        toReturnY = 0
        edgeDist = (radius / 2) * math.sqrt(3)
        cornerDist = radius
    ...
    def translate_road(location, origin, radius, sideLength=3):
        ...
        x_step = location[0] - 10
        x_factor = 3 ** 0.5 / 4
        x_offset = x_factor * radius * x_step

        y_step = location[1] - 5
        y_factor = 3 / 4
        y_offset = y_factor * radius * y_step

        return int(x_offset + origin[0] + x_diff), int(y_offset + origin[1] + y_diff)
