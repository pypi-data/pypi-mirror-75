def build_road_check(actor, free=False):
...
def build_road_available(actor, free=False):
    to_return = list()
    if build_road_check(actor, free):
        for edgeBase in actor.ownedRoads:
            for corner in edgeBase.corners:
                for edge in corner.edges:
                    if edge is not None:
                        if edge.available and edge not in to_return:
...
def build_settlement_check(actor):
    hand = actor.resourceHand
...
def build_settlement_available(actor):
    toReturn = list()
    if build_settlement_check(actor):
        for edgeBase in actor.ownedRoads:
            for corner in edgeBase.corners:
...
def build_city_available(actor):
    toReturn = list()
    if build_city_check(actor):
...
def buy_development_check(actor):
    hand = actor.resourceHand
...
def buy_development(actor, bank):
    if buy_development_check(actor):
        '''
