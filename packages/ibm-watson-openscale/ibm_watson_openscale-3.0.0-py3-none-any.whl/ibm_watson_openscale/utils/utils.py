...
def get_instance_guid(authenticator):
    import requests
    import json
    ...
    iam_headers = {
        ...
        'Authorization': 'Bearer %s' % authenticator.token_manager.get_token()
    }

    if 'test' in authenticator.token_manager.url:
        resources = json.loads(
            requests.get('https://resource-controller.test.cloud.ibm.com/v2/resource_instances', headers=iam_headers).text)[
            'resources']
    else:
        resources = json.loads(
