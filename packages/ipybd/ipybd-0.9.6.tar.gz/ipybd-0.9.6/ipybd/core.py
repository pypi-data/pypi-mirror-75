...
STD_TERMS_ALIAS_PATH = os.path.join(HERE, 'std_fields_alias.json')
PERSONAL_TEMPLATE_PATH = os.path.join(HERE, 'personal_table_map.json')
...
class FormatTable:
    with open(STD_TERMS_ALIAS_PATH, encoding="utf-8") as std_alias:
        stdfield_alias = json.load(std_alias)

    def __init__(self, io:"excel or csv file path"):
        self.df = self.read_table(io)
        self.columns = dict.fromkeys(self.df.columns)
        self.raw_columns = tuple(self.df.columns)
        self.size = self.df.size
        self.classname = self.__class__.__base__.__name__

    def columns_mapping(self, mapping:dict):
    ...
    def __split_txt(self, txt, splitters:"sequence like")->list:
        result = [txt]
        for splitter in splitters:
            try:
                if splitter == "$":
                    # 中英文分列
                    match = re.match(r"[\s\"\']*[\u4e00-\u9fa5][^a-zA-Z]*", result[-1])
                    if match:
                        result[-1:] = [match.group(0).strip(), result[-1][match.span()[1]:]]
                    else:
                        match = re.match(r"[\s\"\']*[a-zA-Z][^\u4e00-\u9fa5]*",result[-1])
                        if match:
                            result[-1:] = [match.group(0).strip(), result[-1][match.span()[1]:]]
                else:
    ...
    def __merge2pairs(self, columns, new_header, typ='dict'):
    ...
    def merge_columns(self, columns, separators, new_header):
        if separators == 'r':
            mergers = self.__merge2pairs(columns, new_header, typ='rowList')
        elif separators == 'o':
            mergers = self.__merge2pairs(columns, new_header, typ='jsonObject')
        elif separators == 'a':
            mergers = self.__merge2pairs(columns, new_header, typ='jsonArray')
        elif separators == 'l':
            mergers = self.__merge2pairs(columns, new_header, typ='list')
    ...
    def __unmapped_columns2std(self, unmapped_fields:list):
        ...
        func: 用户输入的表达式以指定列名的拆分/合并/转换的映射关系，其规则如下：
            ...
            数字之间的标点符号，为数据列按照映射关系进行拆分或合并所依据的分隔符号；
            拆分表达式 = 号之前只能由一个列名序号，合并表达式 = 号之后只能有一个
            标准列名序号。
            手动处理获得得最新映射关系将被存储在 self.columns 中。
        ...
        expression = prompt("\n\n请输入列名转换表达式，录入 0 忽略所有：", 
                                ...
                                self.raw_columns, 
                                self._stdfields, 
                                FormatTable.stdfield_alias[self.classname]), 
                                complete_while_typing=False
                                )
        # 如果忽略所有，这些列名将映射到自身
        if expression.strip() == "0":
            for field in unmapped_fields:
                self.columns[field] = field
            return None
        p_raw2std = re.compile(r"(\d+)([^\d]*)")
        ...
        if elements[-2][1].strip() == "=" and elements[-1][1].strip() == "":
            nums2merge = [txt_num[0] for txt_num in elements[0:-1]]
            std_field_num = elements[-1][0]
            try:
            ...
            for field in fields2merge:
                try:
                    ...
                    pass
                # 若该字段之前已经被映射，删除之前的映射，并将相关字段重新加入未映射
                unmapped_fields.extend(self.__mapping_clean(field))
            # 分隔符是空格的，保留一个空格，非空格的分隔符去除两侧空格
            separators = tuple(sep[1].strip() if set(sep[1]) != {" "} else " " for sep in elements[0:-2])
            if separators == ():
                # 如果合并的列名只有一个，为一对一映射
                self.columns.update({fields2merge[0]:std_field})
            else:
                fields2merge.append(separators)
                self.columns.update({tuple(fields2merge):std_field})
        # 单个列名分拆为多个标准列名
        elif elements[0][1].strip() == "=" and elements[-1][1].strip() == "":
            num2split = elements[0][0]
            num2stdfields = [num[0] for num in elements[1:]]
            ...
            # 分隔符是空格的，保留一个空格，非空格的分隔符去除两侧空格
            splitters = tuple(spl[1].strip() if set(spl[1]) != {" "} else " " for spl in elements[1:-1])
            field2split.append(splitters)
            self.columns.update({tuple(field2split):std_fields})
            #print(self.columns)
        else:
            print("\n录入的表达式有误，请重新输入\n")
        return unmapped_fields
            ...
            if std_field:
                ...
                while True:
                    try:
                        ...
                        print("\n录入的数字有误，请重新输入\n")
                # 解除重复的映射关系，并将映射错误的原始列名写入无对应关系的列名序列内
                for raw_field in raw_fields:
                    try:
                        del self.columns[raw_field]
                    except KeyError:
                        pass
    ...
    def __check_duplicate_mapping(self):
        ...
        for std_field in set(mapping_fields):
            ...
            if mapping_fields.count(std_field) > 1 or std_field == None:
                raw_fields = []
                for rawfield, map_field in self.columns.items():
    ...
    def __auto2std_field(self, raw_field):
        # 首先找出表格列名中使用标准名称的列名
        if raw_field in FormatTable.stdfield_alias[self.classname]:
            self.columns[raw_field] = raw_field
        else:
            # 如果列名用的不是标准名子，逐个比较别名库，找到对应的标准名称
            std_options = []
            for k, v in FormatTable.stdfield_alias[self.classname].items():
                if raw_field in v:
                    std_options.append(k)
            if len(std_options) == 1:
                self.columns[raw_field] = std_options[0]
            elif len(std_options) == 0:
                self.columns[raw_field] = None
            # 如果有多个标准名与该列名对应，咨询用户，让其手动确定需要对应哪一个
                ...
                while True:
                    ...
                    if choice2std.strip() == "0":
                        ...
                        break
                    try:
                        self.columns[raw_field] = std_options[int(choice2std)-1]
                        break
                    except (IndexError, ValueError):
                        print("\n所选值有误，请重新输入\n")

    def build_mapping_template(self):
        for raw_field in tqdm(self.columns, desc="列名映射", ascii=True):
            self.__auto2std_field(raw_field)
        invalid_mapping = self.__check_duplicate_mapping()
        self._stdfields = tuple(FormatTable.stdfield_alias[self.classname].keys())
        ...
        print(self.columns)
        json_template = {str(k):str(v) for k, v in self.columns.items()}
    ...
    def read_table(self, io):
        print("\n开始载入数据表格...\n")
        print("\n如果数据表格太大，此处可能会耗时很长...\n")
        print("\n如果长时间无法成功载入，请将 Excel 数据表转换为 CSV 格式后重新尝试...\n")
        try:
...
class RestructureTable(FormatTable):
    ...
    def __check_old_mapping(self):
        ...
        if set(old_columns) == set(self.raw_columns):
            ...
            self.columns = old_mapping          
            return False
        else:
            return True

    def rebuild_columns(self):
        """ 为构建的 cls 实例按照 std_table_terms 定义的 key 生成属性
        实例属性的名称为枚举的 key.name
        实例属性的值为数据表中与之对应的 Series 经进一步合并/拆分、校验而成
        """
        for field in self.std_columns:
            ...
            if isinstance(field.value[0], (str, tuple, list)):
                ...
                if args_name:
                    ...
                    if len(args_name) == 1:
                        ...
                        self.df.rename(columns={args_name[0]:field.name}, inplace=True)
                    else:
                        # 进一步合并已经被映射转换过的列
                        self.merge_columns(std_fields, field.value[-1], field.name)
                elif self.fill_column:
                    # 如果原表中无法找到相应的列，用 None 填充新列
                    columns = [name for name, obj in self.std_columns.__members__.items() if obj is field]
                    for col in columns:
                ...
                #try:
                args_name = self.build_args(field.name, field.value[1:1+field.value[-1]])
                if args_name:
                    # 模版中的参数包括数据和相应类的初始化参数
                    # 数据都是以 Series 形式传递，若需一次处理多列，则传递多个 Series
                    # 若需要将多列先合并为单列再做传递，可以在模版定义中先合并多列
                    args = [self.df[arg_name] for arg_name in args_name] + list(field.value[field.value[-1]+1:-1])
                    new_cols = field.value[0](*args)()
                    new_cols.columns = [name for name, obj in self.std_columns.__members__.items() if obj is field]
    ...
    def build_args(self, title, params):
        ...
        return: None 或 list,list内包含真实可调用的数据表表头名
        """
        arguments = []
        for param in params:
            ...
            for n, clm in enumerate(columns): 
                ...
                # 找到的字段若需进一步合并，先执行合并
                    ...
                    self.merge_columns(clm[:1], clm[-1], param[n])
                    columns[n] = param[n]
                elif isinstance(clm, tuple) and isinstance(param, str):
                    self.merge_columns(clm[:1], clm[-1], param)
                    columns[n] = param
            # 对整个位置参数的列名映射做进一步处理
            if len(columns) == 1:
    ...
    def get_arg(self, param):
        ''' 将位置参数转换为真实可调用的表名
        param: std_table_terms 定义的单个数据位置参数，可能是个str字段名，也可能是 tuple
             如果是tuple，tuple 中的元素都为 str，表示该位置参数需由多个标准字段先合并组成，
             tuple 最后一个元素为合并字段之间的连接符组成的元组，其他元素为参与合并的字段名。
        return: 返回与 param 对应的真实可调用列名组成的 list，list 是由 str 或 
                tuple 元素组成的列表；
                如果找不到对应列名，返回 None
        '''
        column_names = []
        # 如果ReStructureTable已经与FormatTable一致，则直接用原始表中与之对应列
        if param in self.columns.values() or param[0:-1] in self.columns.values():
            column_names.append(self.get_key(param))
            del self.columns[column_names[0]]
        # 对于ReStructureTable与FormatTable不一致的字段转换，综合比较后以前者为
        # 主修改后者，前者未覆盖的则依据后者处理
        else:
            if isinstance(param, str):
                param = (param, "")
            for n, clm in enumerate(param[0:-1]):
                for field in list(self.columns.values()):
                    if type(field) == str and clm == field:
                        column_names.append(self.get_key(field))
                        del self.columns[self.get_key(field)]
                        ...
                        for header in field:
                            if header != clm:
                                self.columns.update({header:header})
                        column_names.append(clm)
                if len(column_names) < n + 1:
                    # 对于需要进行多次映射处理的字段，self.columns 只会记录字段的首次映射
                    # 关系，因此若 self.columns 无法找到 clm，则进一步寻找实际表格字段名
                    if clm in self.df.columns:
                        column_names.append(clm)
                    # 一些 tuple 型位置参数在实际表中并非每个字段都可以找到，缺失字段名会
                    # 用 None 占位，但不会强制用户补全。位置参数中如行政区划就是由
                    # (country, stateProvince, city, county, ",") 进一步组合而成。
                    # 其中的 country 在实际表格中确实有可能就不存在，这种标准结构字段，有
                    # 些确实会因部分字段缺失导致信息不可用，比如学名中的属名，有些则 对数据
                    # 的进 一步校验影响不大。但无论如何对于缺失的字段，这里都不会强求用户补
                    # 全，而会在具体的数值校验程序中进行处理。
                    else:
