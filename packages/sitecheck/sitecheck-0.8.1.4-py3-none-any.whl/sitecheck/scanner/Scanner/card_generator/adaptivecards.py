...
def store(project, data_list):
    ...
    :rtype: None
    """
    file_path = f'{temp}//{project}_temp.txt'
...
class Generator:
    ...
    def __init__(self, current_project):
        ...
        self.store_path = (os.environ.get('TEMP') + '//' + self.project +
                           '_temp.txt')
        self.generator_output = options.Output + '//' + self.project + ".json"
        self.url = 'https://' + self.project + '.geo-instruments.com/index.php'
    ...
    def generate_empty_template(self):
        ...
        with open(self.generator_output, 'w') as gen_file:
            ...
            data_info = SensorDataFormatter(
                'No Issues', 'good', 'Up-to-date', str(date)
                )
            print(data_info, file=gen_file)
            print(template.sensor_suffix, file=gen_file)
            print(
