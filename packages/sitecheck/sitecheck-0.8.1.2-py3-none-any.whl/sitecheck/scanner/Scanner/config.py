...
from sitecheck.scanner.Scanner import logger, ROOT_DIR, projectstore
...
def edit_project():
    ...
    if os.path.exists(projectstore):
        ...
        generate_default()
    return subprocess.run('notepad.exe ' + projectstore).returncode
...
def file_dialog():
    ...
    try:
        ...
        options['filetypes'] = [('all files', '.*'), ('Json config', '.json'),
                                ('ini config files', '.ini')]
        options['initialdir'] = ROOT_DIR
        options['initialfile'] = projectstore
...
def read_config_file():
    ...
    try:
        ...
        logger.warn(
            'Duplicate Section Error found in config, Please locate the error in notepad \n' + str(
                e))
        edit_project()
        ...
        logger.warn(
            'Duplicate Setup found in config, Please locate the error in notepad \n' + str(
                e))
        edit_project()
        logger.warn("Rerunning config check..")
        read_config_file()
