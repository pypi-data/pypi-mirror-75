...
def get(engine, ref):
    ...
    result = response_validator.validate(response, engine)
    raw_result = getattr(engine, 'raw_result', False)
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'Theme'], returns_list=False, raw_result=raw_result)

def get_all(engine, active=None, page_offset=None, page_size=None):
    """
    ...
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
    url = "/resources/json/delphix/theme"
    query_params = {"active": active, "pageOffset": page_offset, "pageSize": page_size}
    ...
    result = response_validator.validate(response, engine)
    raw_result = getattr(engine, 'raw_result', False)
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'Theme'], returns_list=True, raw_result=raw_result)
