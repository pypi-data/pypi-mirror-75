...
from delphixpy.v1_11_0.web.system.verification.reports import steps
try:
...
def get(engine, ref):
    ...
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'UpgradeVerificationReport'], returns_list=False, raw_result=raw_result)

def get_all(engine, page_offset=None, version=None, sort_by=None, page_size=None, ascending=None):
    """
    ...
    query_params = {"pageOffset": page_offset, "version": version, "sortBy": sort_by, "pageSize": page_size, "ascending": ascending}
    ...
    result = response_validator.validate(response, engine)
    raw_result = getattr(engine, 'raw_result', False)
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'UpgradeVerificationReport'], returns_list=True, raw_result=raw_result)
