...
class Project:
    ...
    def set_default_mode(self, mode):
        """Sets the default mode for this project

        :param mode: (BuildMode) default mode
        """
        config_element_open = '<Configuration Condition=" \'$(Configuration)\' == \'\' ">'
        config_element_open_re = re.sub(r'(\$|\(|\))', r'\\\1', config_element_open)
        config_element_close = '</Configuration>'
        assert_true(
            File(self._path).update(
                lambda x: config_element_open in x,
                ReReplacement(
                    f'{config_element_open_re}.*?{config_element_close}',
                    f'{config_element_open}{mode.name.title()}{config_element_close}',
                )), 'Default configuration not found in ' + self._path)
