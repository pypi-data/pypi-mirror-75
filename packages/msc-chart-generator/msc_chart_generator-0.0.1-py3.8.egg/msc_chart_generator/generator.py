...
class MSCChart(object):
    ...
    def __init__(
        ...
        if type == ChartType.PIE:
            self.generate_msc_pie_chart(r, filepath)
        elif type == ChartType.STACKED_AREA:
            self.generate_stacked_area_chart(r, filepath)

    def generate_stacked_area_chart(self, r: Repository, filepath: str):
        ...
        for week in weeks:
            ...
            for index, msc in enumerate(mscs):
                msc_state = self.get_msc_state_at_time(msc, msc_events[index], week)
    ...
    def generate_msc_pie_chart(self, r: Repository, filepath: str):
