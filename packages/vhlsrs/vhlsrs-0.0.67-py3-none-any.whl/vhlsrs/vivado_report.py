...
def parse_impl_report(impl_report_file, timing_report_file, need_filter):
    ...
    mdelay_filter = lambda x : not x.startswith('Max Delay Paths')
    nofilter = lambda x : False
    line_filter = mdelay_filter if need_filter else nofilter
