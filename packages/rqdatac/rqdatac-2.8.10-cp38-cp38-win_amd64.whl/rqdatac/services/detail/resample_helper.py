...
def resample_week_df(df, fields):
    ...
    res1 = df[df['volume'] > 0].groupby(level='order_book_id').resample('W-Fri', level=1).agg(hows)
    res2 = df[df['volume'] == 0].groupby(level='order_book_id').resample('W-Fri', level=1).agg(hows)
