...
def get_price(
    ...
    **kwargs
    ...
    if not expect_df and not is_panel_removed and frequency != "w":
        ...
        fields, has_dominant_id = _ensure_fields(fields, WEEKBAR_FIELDS, stocks, funds, futures, futures888, spots, options, convertibles, indexes, repos)
...
def _ensure_fields(fields, fields_dict, stocks, funds, futures, futures888, spots, options, convertibles, indexes, repos):
...
def _adjust_pf(
    ...
    if len(order_book_ids) == len(futures888) or has_dominant_id:
...
def add_dominant_id(result, futures888, frequency):
    """
    有对应的dominant_id填dominant_id，没有则填nan
    """
    d = {}
    for order_book_id in result.minor_axis:
        ...
        if order_book_id in futures888.keys() and not df.empty:
            s = get_dominant_future(futures888[order_book_id], df.index[0], df.index[-1])
            if frequency == "d":
                df["dominant_id"] = s
            else:
