...
def get_industry(industry, source='citics', date=None, market="cn"):
    ...
    :param source: 分类来源。citics 以及 citics2019: 中信, gildata: 聚源
    :param date: 查询日期，默认为当前最新日期
    :param market:  (Default value = "cn")
    :return: 所属目标行业的order_book_id list or None
    """
    ...
    source = ensure_string_in(source, ["sws", "citics", "gildata", "citics_2019"], "source")
...
@export_as_api
def get_industry_mapping(source="citics", market="cn"):
    """获取行业分类列表
    ...
    """
    source = ensure_string_in(source, ["sws", "citics", "gildata", "citics_2019"], "source")
    res = get_client().execute("get_industry_mapping_v2", source, market=market)
    df = pd.DataFrame(res)
