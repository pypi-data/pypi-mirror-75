...
"""See https://github.com/zebracorn-engine/zebracorn/issues/98"""
...
class RegWriteSignExt(regress.RegressTest):
    ...
    def runTest(self):
        ...
        mu.reg_write(zebracorn.x86_const.UC_X86_REG_EBX, ADDR)

        mu.mem_map(0x10000000, 1024 * 4)
        # jmp ebx
        mu.mem_write(0x10000000, b'\xff\xe3')

        mu.hook_add(zebracorn.UC_HOOK_MEM_FETCH_UNMAPPED | zebracorn.UC_HOOK_MEM_FETCH_PROT, hook_mem_invalid)
