...
"""Declares :class:`~TwitterAccountPrincipal`."""
from .externalidentifierprincipal import ExternalIdentifierPrincipal


class TwitterAccountPrincipal(ExternalIdentifierPrincipal):
    """A :term:`Principal` that identifies a :term:`Subject` using a
    Twitter Account ID.
    """
    resource_class = 'TwitterAccountPrincipal'
    platform = 'twitter'

    @classmethod
    def fromoauth2(cls, response):
        """Instantiate a new :class:`TwitterAccountPrincipal` from the OAuth2
        response received by Twitter.

        Args:
            response (:obj:`dict`): a dictionary holding the response data.

        Returns:
            :class:`TwitterAccountPrincipal`
        """
        return cls(response['access_token']['user_id'])
