...
def perform_registration(acme, config, tos_cb):
    ...
    if config.zerossl_api_key:
        zerossl_eab_credentials = requests.post('https://api.zerossl.com/acme/eab-credentials?access_key=%s' % config.zerossl_api_key).json()
        config.eab_hmac_key = zerossl_eab_credentials["eab_hmac_key"]
        config.eab_kid = zerossl_eab_credentials["eab_kid"]
