...
default_expirations = {
    ChampionRotationData: datetime.timedelta(hours=3),
    Realms: datetime.timedelta(hours=6),
    Versions: datetime.timedelta(hours=6),
    Champion: datetime.timedelta(hours=3),
    Rune: datetime.timedelta(hours=3),
    Item: datetime.timedelta(hours=3),
    SummonerSpell: datetime.timedelta(hours=6),
    Map: datetime.timedelta(hours=6),
    ProfileIcon: datetime.timedelta(hours=3),
    Locales: datetime.timedelta(hours=6),
    LanguageStrings: datetime.timedelta(hours=6),
    SummonerSpells: datetime.timedelta(hours=6),
    Items: datetime.timedelta(hours=3),
    Champions: datetime.timedelta(hours=3),
    Runes: datetime.timedelta(hours=3),
    Maps: datetime.timedelta(hours=6),
    ProfileIcons: datetime.timedelta(hours=3),
    ChampionMastery: 0,
    ChampionMasteries: 0,
    LeagueSummonerEntries: 0,
    League: 0,
    ChallengerLeague: 0,
    GrandmasterLeague: 0,
    MasterLeague: 0,
...
class Omnistone(DataSource, DataSink):
    def __init__(self, expirations: Mapping[type, float] = None, max_entries: int = None, cull_frecuency: int = None, logs_enabled: bool = False, safe_check: bool = True) -> None:
        self._cache = OmnistoneBackend(max_entries, cull_frecuency, logs_enabled)
        ...
        for key, value in list(self._expirations.items()):
            if isinstance(key, str):
                if key.endswith("Dto"):
                    raise NotImplementedError("Omnistone is only designed to increase performance by caching 'cassiopeia.core' objects.")
                new_key = globals()[key]
            ...
            if value != 0 and key in [obj for obj, val in default_expirations.items() if val == 0] and safe_check:
                raise RuntimeError(f"Secure check is enabled: You are trying to cache {key.__name__} in Omnistone, "+
                                    "it might slow down performance, you can disable this check by setting 'safe_check' = False")
    ...
    def _put_many(self, type: Type[T], items: Iterable[T], key_function: Callable[[T], Any], context: PipelineContext = None) -> None:
        ...
        for key, item in Omnistone._put_many_generator(items, key_function):
            self._cache.put((type, key), item, expire_seconds)
