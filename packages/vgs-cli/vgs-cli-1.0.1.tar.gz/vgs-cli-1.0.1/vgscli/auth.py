...
def auto_login(ctx, environment):
    ...
    try:
        ...
        __auto_login(ctx, auth_server, client_id, secret, username, password)

    return True


def __auto_login(ctx, auth_server, client_id, secret, username, password):
    try:
        auth_server.auto_login(client_id, secret, username, password)
    except ClientError:
