...
def mainmasterprogfundamental():
    ...
    with conn0:
        ...
        jobloadlist =[]
        cursor0.execute(seljobload,(jday,))
        jobtbl = cursor0.fetchall()
        if jobtbl is None:
            jobtbl = []
        else:
            pass
        ...
        stktblq = """select distinct sa.symbol,ba.prio from stock_all sa join benchmark_all ba
                    ...
                    where ba.prio=2 and ba.symbol='TA100.INDX' order by ba.prio"""
        cursor0.execute(stktblq)
        tbl = cursor0.fetchall()
        if tbl is None:
            tbl = []
        else:
            pass
        ...
        if lt>0:
            for i in range(lt):
                ...
                if symbol in jobloadlist:
                    ...
                    cursor0.execute(insjob,(symbol,'mfundamental',jday,'WIP','stock_all'))
                    loadstockfundamentals(symbol, stocktable, fxr, prio, jday, cursor0)
        else:
            print("no stocks in stock_All with prio=2. check if someone deleted the stock_All entries")
        bmktblq = """select distinct ba.symbol,ba.prio from benchmark_all ba
                    join benchmark_master bm on
                    bm.symbol=ba.symbol
                    where ba.prio=2 and ba.symbol='TA100.INDX' order by ba.prio"""
        cursor0.execute(bmktblq)
        btbl = cursor0.fetchall()
        if btbl is None:
            btbl = []
        else:
            pass
        ...
        if blt>0:
            for i in range(blt):
                ...
                if bsymbol in jobloadlist:
                    ...
                    cursor0.execute(insjob,(bsymbol,'mfundamental',jday,'WIP','benchmark_all'))
                    loadstockfundamentals(bsymbol, bstocktable, fxr, bprio, jday, cursor0)
        else:
            print("no stocks in stock_All with prio=2. check if someone deleted the stock_All entries")
...
def mainmasterprog():
    ...
    with conn0:
        ...
        stktblq = """select distinct substring(sa.symbol,position('.' in sa.symbol)+1) as exch,
                    ...
                    on sa.index_code=ba.symbol where ba.prio=2 and ba.symbol='TA100.INDX'
                    order by ba.prio"""
        cursor0.execute(stktblq)
        tbl = cursor0.fetchall()
        if tbl is None:
        ...
        if lt>0:
            for i in range(lt):
                ...
                if symbol in jobloadlist:
                    ...
                    print("Loading:",symbol, stocktable, prio)
                    cursor0.execute(insjob,((symbol+'-'+str(prio)),'mprice',jday,'WIP','stock_all'))
                    loadstockprice(symbol, stocktable, jday, prio, cursor0)
        else:
            print("no stocks in stock_All with prio=2. check if someone deleted the stock_All entries")
        bmrk = """select distinct symbol,prio from benchmark_all
        where is_active=True and prio=2 and symbol='TA100.INDX' order by prio"""
        cursor0.execute(bmrk)
        btbl = cursor0.fetchall()
        if btbl is None:
            btbl = []
        else:
        ...
        if blt>0:
            for i in range(blt):
                ...
                if bsymbol in jobloadlist:
                    ...
                    print("Loading:",bsymbol, bstocktable, bprio)
                    cursor0.execute(insjob,(bsymbol,'mprice',jday,'WIP','benchmark_all'))
                    loadstockprice(bsymbol, bstocktable, jday, bprio, cursor0)
        else:
            print("no benchmark in benchmark_All with prio=2. check if someone deleted the benchmark_All entries")
...
def stocksplits():
    stk = """select distinct symbol,prio from
            ...
            where b.symbol='TA100.INDX' and b.prio=2) a
            where a.mr>1"""
