...
h = reg.regularization_matrix_from_mapper(mapper=mapper)
Rop = nufft_lop.Regularization(pixels=h.shape[0], h=h)

start = time.time()

for i in range(repeats):
    x = pylops.NormalEquationsInversion(
        Op=Op, Regs=None, epsNRs=[1.0], NRegs=[Rop], data=y
    )
