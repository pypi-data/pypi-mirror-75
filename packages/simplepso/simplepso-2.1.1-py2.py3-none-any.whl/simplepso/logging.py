...
LOG_LEVEL_ENV_VAR = 'SIMPLEPSO_LOG'
BASE_LOGGER_NAME = 'simplepso'
EXTENDED_DEBUG = 5
...
def formatter(time_utc=False):
    ...
    Returns
    -------
    A logging.Formatter object for simplePSO logging
    """
...
def setup_logger(level=logging.INFO, console_output=True, file_output=False,
    ...
    """
    Set up a new logging.Logger for simplePSO logging

    Calling this method will override any existing handlers, formatters etc.
    attached to the simplePSO logger. Typically, :func:`get_logger` should be
    used instead, which returns the existing simplePSO logger if it has already
    been set up, and can handle simplePSO submodule namespaces.
    ...
    Returns
    -------
    A logging.Logger object for simplePSO logging.
    :func:`get_logger`.
    """
        ...
        try:
            ...
            if level_name in NAMED_LOG_LEVELS.keys():
                ...
                raise ValueError(
                    ...
                    'If set, its value must be one of {} (case-sensitive) or '
                    'an integer log level.'.format(
    ...
    log.info('Logging started on simplepso')
    if time_utc:
...
def get_logger(logger_name=BASE_LOGGER_NAME, log_level=None,
               **kwargs):
    """
    Returns (if extant) or creates a simplePSO logger

    If the simplePSO base logger has already been set up, this method will
    return it or any of its descendant loggers without overriding the settings
     - i.e. any values supplied as kwargs will be ignored.
    ...
    **kwargs : kwargs
        ...
        the simplePSO logger hasn't been set up yet (i.e. there have been no
    ...
    if BASE_LOGGER_NAME not in logging.Logger.manager.loggerDict.keys():
        ...
        warnings.warn('simplepso logger already exists, ignoring keyword '
