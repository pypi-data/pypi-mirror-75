    ...
    if instance_id:
        if authentication_token:
            instance_details = {'cloud_provider': 'Azure',
                                ...
                                'account_id': account_id, 'spectrum_dimensions': ['instance_id']}
        else:
...
def getAzureSpotInstanceDetails(config, authentication_token, account_id, log, debug):
    instance_details = None
    if debug:
        log.warning("spotinst-agent is running with mock credentials for debug purposes")
...
def init__configuration(config, cloud_provider, debug=False):
    ...
    if cloud_provider.lower() == 'azure':
        ...
        instance_details = getAzureSpotInstanceDetails(config, authentication_token, account_id, log, debug)
