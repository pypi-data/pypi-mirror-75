...
class ZiplineRepoValidator(object):
    ...
    def has_no_change(self, obj: object) -> bool:
        old_obj = self._get_old_obj(type(obj), obj.name)
        if old_obj and not self._get_diff(obj, old_obj, skipkeys=[]):
            return True
        return False
    ...
    def approve_diff(self, obj: object) -> bool:
        ...
        if old_obj and not self._safe_to_overwrite(old_obj):
            diff = self._get_diff(obj, old_obj)
            if diff:
                ans = get_input(CONFIRM_OVERWRITE_PROMPT.format(**locals()))
                return ans.strip().lower() in ["y", "yes"]
        return True

    def _get_diff(self, obj: object, old_obj: object, skipkeys=STATUS_FIELDS) -> str:
        new_json = json.loads(thrift_simple_json(obj))
        old_json = json.loads(thrift_simple_json(old_obj))
        try:
            temp_dir = tempfile.mkdtemp()
            with open(os.path.join(temp_dir, 'old.json'), mode='w') as old, \
                 open(os.path.join(temp_dir, 'new.json'), mode='w') as new:
                old.write(json.dumps(old_json, skipkeys=skipkeys, sort_keys=True, indent=2))
                new.write(json.dumps(new_json, skipkeys=skipkeys, sort_keys=True, indent=2))
    ...
    def _validate_group_by(self, group_by: GroupBy) -> List[str]:
        ...
        if group_by.production is False and prod_joins:
