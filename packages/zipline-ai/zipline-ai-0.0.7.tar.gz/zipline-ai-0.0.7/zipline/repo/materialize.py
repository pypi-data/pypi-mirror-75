    ...
    for name, obj in results.items():
        if _write_obj(full_output_root, validator, name, obj, log_level, force_overwrite):
            ...
            if obj_class is LeftOuterJoin and obj.online:
                online_group_bys = {f"groupby.{rt.groupBy.name}": rt.groupBy for rt in obj.rightParts}
...
def _write_obj(full_output_root: str,
    ...
    output_path = os.path.join(full_output_root, obj_folder_name)
    output_file = get_output_file_name(output_path, name)
    if validator.has_no_change(obj):
        _print_warning(f"Skipping {class_name} {name}: No new changes.")
        return False
...
def get_output_file_name(output_path: str, name: str) -> str:
    team_name = name.split(".")[0]
