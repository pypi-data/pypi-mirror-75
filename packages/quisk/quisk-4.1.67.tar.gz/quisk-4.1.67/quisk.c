	...
	case 0:	 // start sound thread
		if (SetPriorityClass(GetCurrentProcess(), HIGH_PRIORITY_CLASS) == 0)
			if (quisk_sound_state.verbose_sound)
				printf("Failed to set class priority\n");
		if (SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_HIGHEST) == 0)
			if (quisk_sound_state.verbose_sound)
				printf("Failed to set thread priority\n");
		if (AvSetMmThreadCharacteristics(TEXT("Pro Audio"), &taskIndex) == NULL && quisk_sound_state.verbose_sound)
			printf("Failed to set thread to Pro Audio\n");
		...
		if (high_res < 0 ) {    // not started
			...
			if ( ! high_res) {
				...
				if (timeBeginPeriod(timer_res) != TIMERR_NOERROR && quisk_sound_state.verbose_sound)
					printf ("Failed to set timer resolution to %u\n", timer_res);
				//if (quisk_sound_state.verbose_sound)
				//	printf("Change sound thread to high resolution\n");
			}
		}
		else {
			if (high_res) {
				...
				if (timeEndPeriod(timer_res) != TIMERR_NOERROR && quisk_sound_state.verbose_sound)
					printf ("Failed to clear timer resolution\n");
				//if (quisk_sound_state.verbose_sound)
				//	printf("Change sound thread to low resolution\n");
			}
		}
		break;
	case 2:	 // stop sound thread
		...
		if (high_res == 1) {
			if (timeEndPeriod(timer_res) != TIMERR_NOERROR && quisk_sound_state.verbose_sound)
				printf ("Failed to clear timer resolution\n");
			//if (quisk_sound_state.verbose_sound)
			//	printf("Close sound thread to low resolution\n");
		}
		high_res = -1;
		break;
	}
...
static void set_stone(void)
{
	...
	else if (quisk_Playback.driver == DEV_DRIVER_SOUNDIO)
...
void quisk_set_play_state(int init)
{
	...
	//printf("quisk_play_state end %d ptt %d\n", quisk_play_state, is_PTT_down);
}
