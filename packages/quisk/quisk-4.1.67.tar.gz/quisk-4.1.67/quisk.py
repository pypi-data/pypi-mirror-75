...
class ConfigStatus(wx.ScrolledWindow):
  ...
  def __init__(self, parent, width, fft_size):
    ...
    try:
      ...
      self.tfg_color = wx.Colour(self.GetForegroundColour())
    self.width = width
...
The new code supports multiple corrections per band.""")
  ...
  def MakeSoundDeviceList(self):
    ...
    if conf.pulse_audio_verbose_output > 1:
      print ("Sound devices:")
    ...
    for name, is_play, index in sound_cards:
      ...
      if sys.platform == 'win32':
        ...
        if not found_desc:
          found_desc = value
          found_dev = value
        if conf.use_fast_sound and name in ("name_of_sound_play", "name_of_mic_play"):
