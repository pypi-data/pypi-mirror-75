...
class stock_interface:
    ...
    def setDebugMode(mode):
        stock_interface.__debugMode = mode

    def message(text):
        if(stock_interface.__debugMode):
            print(text)
    ...
    def initializStockData(sym):
        ...
        try:
            ...
            xml = stock_interface.scraper(url)
            
            url = "https://finance.yahoo.com/quote/"+sym+"/history?period1=1&period2=2000000000&interval=1d&filter=history&frequency=1d"
            xmlHistorical = stock_interface.scraper(url)
            stock_interface.message("Success initializing: " + sym)

        except:
            stock_interface.message("Error initializing: " + sym)
            return None
        stock = Stock(xml, xmlHistorical, sym)

        stock_interface.__initStockData(stock)

        stock_interface.stocks[sym] = stock

        stock_interface.writeToJson(sym)

        return True

    def __initStockData(stock):
        stock_interface.__initStockPriceAtClose(stock)
        stock_interface.__initStockPriceAfterHours(stock)
        stock_interface.__initChangeAtClose(stock)
        stock_interface.__initChangeAfterHours(stock)
        stock_interface.__initPreviousClose(stock)
        stock_interface.__initOpen(stock)
        stock_interface.__initDayRange(stock)
        stock_interface.__init52WeekRange(stock)
        stock_interface.__initVolume(stock)
        stock_interface.__initAverageVolume(stock)
        stock_interface.__initMarketCap(stock)
        stock_interface.__initHistoricalDataAll(stock)


    def writeToJson(sym):
        stock = stock_interface.getInitializedStock(sym)
        if(stock is None):
            return None
    ...
    def initializStockDataFromJson(sym):
        ...
        try:
            ...
            stock_interface.message("error trying to read data from " + sym + "_data.json")
            return None
    ...
    def __initStockPriceAtClose(stock):
        try:
            ...
            stock_interface.message(stock.symbol + " close price data not currently available")
    ...
    def __initStockPriceAfterHours(stock):
        try:
            ...
            stock_interface.message(stock.symbol + " after hours data not currently available")
    ...
    def __initChangeAtClose(stock):
        try:
            ...
            stock_interface.message(stock.symbol + " change at close data not currently available")
    ...
    def __initChangeAfterHours(stock):
        try:
            ...
            stock_interface.message(stock.symbol + " change after hours data not currently available")
    ...
    def __initPreviousClose(stock):
        try:
            ...
            stock_interface.message(stock.symbol + " previous close data not currently available")
    ...
    def __initOpen(stock):
        try:
            ...
            stock_interface.message(stock.symbol + " open price data not currently available")
    ...
    def __initDayRange(stock):
        ...
        try:
            ...
            stock_interface.message(stock.symbol + " day range data not currently available")
    ...
    def __init52WeekRange(stock):
        ...
        try:
            ...
            stock_interface.message("52 week range data not currently available")
    ...
    def getInitializedStock(sym):
        try:
            stock = stock_interface.stocks[sym]
            return stock
        except:
            stock_interface.message("Error trying to access: " + sym + ". Try initializing the stock first")
            return None
    ...
    def getStockPriceAtClose(sym):
        ...
        if(price is None):
            return None            
        stock_interface.message(sym + " Price of at close: " + str(price))
        return price
    ...
    def getStockPriceAfterHours(sym):
        ...
        if(price is None):
            return None
        stock_interface.message(sym + " Price after hours: " + str(price))
        return price
    ...
    def getPointChangeAtClose(sym):
        change = stock_interface.getChangeAtClose(sym)
        if(change is None):
            return None
        stock_interface.message(sym + " point change at close: " + str(change[0]))
        return change[0]
    ...
    def getPercentageChangeAtClose(sym):
        ...
        if(change is None):
            return None
        stock_interface.message( sym + " percentage change at close: " + str(change[1]) + "%")
        return change[1]
    ...
    def getPointChangeAfterHours(sym):
        change = stock_interface.getChangeAfterHours(sym)
        if(change is None):
            return None
        stock_interface.message(sym + " point change after hours: " + str(change[0]))
        return change[0]
    ...
    def getPercentageChangeAfterHours(sym):
        ...
        if(change is None):
            return None
        stock_interface.message( sym + " percentage change after hours: " + str(change[1]) + "%")
        return change[1]
    ...
    def getPreviousClose(sym):
        ...
        if(previousClose is None):
            return None
        stock_interface.message(sym + " previous close: " + str(previousClose))
    ...
    def getOpen(sym):
        ...
        if(openPrice is None):
            return None
        stock_interface.message(sym + " open: " + str(openPrice))
        return openPrice
    ...
    def getDayLow(sym):
        ...
        if(stock.dayRange is None):
            stock_interface.getDayRange(sym)
            stock = stock_interface.getInitializedStock(sym)
            if(stock is None or stock.dayRange is None):
                return None
        stock_interface.message(sym + " day low: " + str(stock.dayRange[0]))  
    ...
    def getDayHigh(sym):
        ...
        if(stock.dayRange is None):
            ...
            if(stock is None or stock.dayRange is None):
                return None
        stock_interface.message(sym + " day high: " + str(stock.dayRange[1]))  
    ...
    def get52WeekLow(sym):
        ...
        if(stock.yearRange is None):
            stock_interface.get52WeekRange(sym)
            stock = stock_interface.getInitializedStock(sym)
            if(stock is None or stock.yearRange is None):
                return None
        stock_interface.message(sym + " 52 week low: " + str(stock.yearRange[0]))  
    ...
    def get52WeekHigh(sym):
        ...
        if(stock.yearRange is None):
            ...
            if(stock is None or stock.yearRange is None):
                return None
        stock_interface.message(sym + " 52 week high: " + str(stock.yearRange[1]))  
    ...
    def getVolume(sym):
        ...
        if(volume is None):
            return None
        stock_interface.message(sym + " volume: " + str(volume))
        return volume
    ...
    def getAverageVolume(sym):
        ...
        if(averageVolume is None):
            return None
        stock_interface.message(sym + " average volume: " + str(averageVolume))
    ...
    def getMarketCap(sym):
        ...
        if(marketCap is None):
            return None
        stock_interface.message(sym + " market cap: " + str(marketCap))
        return marketCap
    ...
    def getHistoricalDataPastXTradingDays(sym, num):
        return stock_interface.getHistoricalDataRangeTradingDays(sym, 0, num)
    ...
    def getHistoricalDataPast5TradingDays(sym):
        return stock_interface.getHistoricalDataPastXTradingDays(sym, 5)
    ...
    def getHistoricalDataPast30TradingDays(sym):
        return stock_interface.getHistoricalDataPastXTradingDays(sym, 30)
    ...
    def getHistoricalDataRangeOfDates(sym, date1, date2):
        ...
        try:
            ...
            stock_interface.message("error trying to access data from dates given")
            return None
        return stock_interface.getHistoricalDataRangeTradingDays(sym, d1, d2)
...
class Stock:
    ...
    def getDataElement(self, id, message):
        try:
            ...
            stock_interface.message(message)
            return None    
