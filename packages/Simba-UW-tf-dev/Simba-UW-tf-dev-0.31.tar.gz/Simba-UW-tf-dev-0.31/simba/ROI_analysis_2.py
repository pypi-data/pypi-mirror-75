...
def roiAnalysis(inifile,inputcsv):
    ...
    bpcsv = (os.path.join(os.path.dirname(inifile), 'logs', 'measures', 'pose_configs', 'bp_names', 'project_bp_names.csv'))
    bplist = []
    ...
    for CurrVidFn in filesFound:
        CurrentVideoName = os.path.basename(CurrVidFn).replace('.' + wfileType, '')
