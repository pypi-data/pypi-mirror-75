...
def create_plotly_container(configini, probabilityTables, timebinTables, sklearnTables, severityTables):
    ...
    if sklearnTables == 1:
        ...
        for file in sklearnFiles:
            sklearnDf = pd.read_csv(file, index_col=0)
            fileDfName = os.path.basename(file).replace('.' + wfileType, '')
            identifier = 'SklearnData/' + fileDfName
            storageFile[identifier] = sklearnDf
    if severityTables == 1:
...
filename = r"Z:\Classifiers\OLD_DLC_MODEL\Attack\project_folder\logs\SimBA_dash_file_20200804125743.h5"
with pd.HDFStore(filename) as hdf:
    # This prints a list of all group names:
    print(hdf.keys())

currDf = pd.read_hdf(filename, key='/VideoData/Video_0910_CLIP_shorten')
