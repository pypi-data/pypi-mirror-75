...
def ROI_process_movement(configini):
    ...
    logFileName = os.path.join(projectPath, 'logs', log_fn)
    log_df = pd.DataFrame()
    ...
    for currentFile in filesFound:
        ...
        videoSettings = vidinfDf.loc[vidinfDf['Video'] == str(currVidName.replace('.' + wfileType, ''))]
        try:
        ...
        csv_df = csv_df.drop(['scorer'], axis=1, errors='ignore')
    ...
    log_df.to_csv(logFileName, index=False)
