...
from azureml.train.hyperdrive.exceptions import HyperDriveConfigException, HyperDriveScenarioNotSupportedException, \
...
class PrimaryMetricGoal(enum.Enum):
    ...
    def from_str(goal):
        ...
        raise HyperDriveRehydrateException("Unknown goal")
...
class HyperDriveConfig(object):
    ...
    def __init__(self,
        ...
        if self._estimator is None and self._run_config is None and self._pipeline is None:
            raise HyperDriveConfigException("One of 'estimator', 'run_config' and 'pipeline' arguments are mandatory.")

        if len([value for value in [self._estimator, self._run_config, self._pipeline] if value is not None]) != 1:
            raise HyperDriveConfigException("Only one of 'estimator', 'run_config' and 'pipeline' can be present.")

        if self._run_config is not None and not isinstance(self._run_config, ScriptRunConfig):
            raise HyperDriveConfigException("Got invalid 'run_config' type, expected 'ScriptRunConfig'.")
        ...
        if isinstance(hyperparameter_sampling, BayesianParameterSampling) and \
            ...
            raise HyperDriveScenarioNotSupportedException(
                "No early termination policy is currently supported with Bayesian sampling. ")
        ...
        if resume_from is not None:
            ...
            if len(resume_from) > MAX_PARENT_RUNS:
                raise HyperDriveConfigException(("The number of parent runs should "
    ...
    def _validate_resume_child_runs(self, resume_child_runs, is_bayesian):
        ...
        if len(resume_child_runs) > self._max_total_runs:
            raise HyperDriveConfigException("The number of resume_child_runs cannot "
        ...
        for run in resume_child_runs:
            if run.id in child_set:
                raise HyperDriveConfigException("resume_child_runs cannot contain duplicate runs.")
            child_set.add(run.id)

            if not isinstance(run, Run):
                raise HyperDriveConfigException("resume_child_runs must be azureml.core.run.Run objects")
            if run.get_status() not in TERMINAL_STATES:
                raise HyperDriveConfigException(("All resume_child_runs should be in a terminal state. Found "
                                                 "a run with state '{}'.").format(run.get_status))
            if run.parent is None:
                raise HyperDriveConfigException("Every run in resume_child_runs should have a parent run.")

            parent_dict[run.parent.id] = run.parent

        if len(parent_dict) > MAX_PARENT_RUNS:
            raise HyperDriveConfigException(("resume_child_runs should not come from more than {} unique parents. "
    ...
    def _validate_warm_start(self, resume_list, is_bayesian, is_resume_from=False):
        ...
        for hdrun in resume_list:
            ...
            if hdrun._runtype != "hyperdrive":
                raise HyperDriveConfigException("All parent runs should be HyperDriveRun objects.")

            if hdrun.hyperdrive_config._primary_metric_config["name"] != self._primary_metric_config["name"]:
                raise HyperDriveConfigException(("The primary_metric_name should be the same for all runs. The "
                                                 "current run's primary_metric_name is '{}', while a run with "
                                                 "primary_metric_name '{}' was found.").format(
            ...
            if hdrun.hyperdrive_config._primary_metric_config["goal"] != self._primary_metric_config["goal"]:
                raise HyperDriveConfigException(("The primary_metric_goal should be the same for all runs. The "
                                                 "current run's primary_metric_goal is '{}', while a run with "
                                                 "primary_metric_goal '{}' was found.").format(
            ...
            if hdrun.hyperdrive_config._platform_config["ServiceAddress"] \
                ...
                raise HyperDriveConfigException("The host should be the same for all runs.")

            if is_bayesian:
                if hdrun.hyperdrive_config._generator_config["parameter_space"] \
                        != self._generator_config["parameter_space"]:
                    raise HyperDriveConfigException("The parameter_space should be the same "
