...
class BlobAccessor(object):
    ...
    def _check_dataprep():
        ...
        try:
            ...
            if version.parse(dataprep_version) < version.parse("1.4.0") \
                    and version.parse(dataprep_version) >= version.parse("0.1.1"):
                warnings.warn(
                    "Please upgrade azureml-dataprep to >= 1.4.0 " +
                    "using pip install --upgrade azureml-dataprep",
                    Warning)
        except:
            # it is possible to have no azureml-dataprep installed
            dataprep_version = ''
            warnings.warn(
                "Please ensure azureml-dataprep is installed.", Warning)
