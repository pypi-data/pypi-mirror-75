...
class Pymata4(threading.Thread):
    ...
    def __init__(self, com_port=None, baud_rate=115200,
        ...
        self.the_serial_monitor_thread = threading.Thread(target=self._serial_monitor)
        ...
        if self.serial_port:
            print("[22] Arduino compatible device found and connected to %s"%(self.serial_port.port))
    ...
    def _manual_open(self):
        ...
        try:
            ...
            print('[20] Waiting %d seconds(arduino_wait) for Arduino devices to reset...'%(self.arduino_wait))
            time.sleep(self.arduino_wait)
            # time.sleep(self.arduino_wait)
    ...
    def neopixel_config(self,pin,num):
        if num > 7:
          return
        data = [pin, num & 0x7f, (num >> 7) & 0x7f]
        self._send_sysex(PrivateConstants.NEOPIXEL_CONFIG, data)

    def neopixel_write(self, n, color):
        if n > 7:
          return
        data = [n,
                color[0] & 0x7f,(color[0] >> 7) & 0x7f,
                color[1] & 0x7f,(color[1] >> 7) & 0x7f,
                color[2] & 0x7f,(color[2] >> 7) & 0x7f,
                ]
        self._send_sysex(PrivateConstants.NEOPIXEL_DATA, data)
        ...
        if data[7] == 1:  # data[9] is config flag
            ...
            if data[6] == 0:
                ...
                # dht 11
                    ...
                    raise RuntimeError("Unknown DHT Sensor type reported: %d"%data[2])
