...
def BarrellAndSears(l, T=288.15, p=101325, H=0):
	'''
	...
	T = K2C(T)
	P = hPA2mmHg(p/100)
	R = (0.378125 + 0.0021414 / l**2 + 0.00001793 / l**4) * (P*(1 + (1.049 - 0.0157*T) * P * 1e-6)/(1 + 0.003661*T)) - (0.0624 - 0.000680 / l**2) * H / (1 + 0.003661*T)
	n = R/1e6 + 1
	return n
...
def Owens(l, T=288.15, p=101325, H=0):
	'''
		From Owens, 1967
		Equation 32
	...
	T = K2C(T)
	s = 1 / l
	R = 8342.13 + 2406030 / (130 - s**2) + 15997 / (38.9 - s**2) * (P/720.775) * ((1 + P * (0.817 - 0.0133) * 1e-6)/(1 + 0.0036610*T)) - H * (5.722 - 0.0457*s**2)
	n = R/1e8 + 1
	return n

def BonschPotulski(l, T=288.15, p=101325, CO2=450):
	'''
	...
	T = K2C(T)
	s = 1 / l
	# Rs = 8092.33 + 2333983 / (130 - s**2) + 15518 / (38.9 - s**2)
	Rs = 8091.37 + 2333983 / (130 - s**2) + 15518 / (38.9 - s**2)
	Rs = Rs / 1e8
	Rx = Rs * (1 + 0.5327 * (x - 0.0004))
	Rtp = Rx * p / 93214.60 * (1 + 1e-8 * (0.5953 - 0.009876 * T)*p) / (1 + 0.0036610 * T)
	n = Rtp + 1
	return n
...
def slalib(l, T=288.15, p=101325, H=0, h=0, lat=-24, tlr=0.0065, eps=1e-10):
	'''
	...
	if type(l) == float:
		l = [l]
	for wl in l:
		R.append(slalib.sla_refro(zObs, h, T, p, H, wl, lat, tlr, eps))
	if len(l) == 1:
		return R[0]+1
	else:
...
def Mathar(l, T=288.15, p=101325, H=0):
	'''
