...
from draugr.torch_utilities.optimisation.parameters import named_trainable_parameters, normal_init_weights, trainable_parameters
from warg import ContextWrapper


def loss_grad_check(model: torch.nn.Module, loss_fn: callable, input, target, epsilon: float = 1e-6, error_tolerance: float = 1e-5):
  '''
  ...
  loss.backward()
  with ContextWrapper(torch.no_grad(), True):
    with TorchEvalSession(model):
      for (n, c_p), p in zip(named_trainable_parameters(c_model).items(), trainable_parameters(model)):
        for i, c_p_o in enumerate(c_p):
          ...
          if len(a) > 0:
            for j in range(a[0]):
              ...
              loss_p = loss_fn(c_model(input.clone()), target.clone()).clone()

              c_p.data = cp_orig

              c_p[i][j] -= epsilon  # negative
              loss_n = loss_fn(c_model(input.clone()), target.clone()).clone()

              c_p.data = cp_orig

              if True: #TODO: make check based on the entire set of parameters at once
                grad_approx = (loss_p - loss_n) / (2 * epsilon)

                denom = math.sqrt(grad_approx ** 2) + math.sqrt(p.grad[i][j] ** 2)
                if denom > 0:
                  deviance = math.sqrt((grad_approx - p.grad[i][j]) ** 2) / denom
                  # assert torch.sign(grad_approx) == torch.sign(p.grad[i][j]), f'apprx: {grad_approx}, analytical {p.grad[i][j]}'
                  assert deviance <= error_tolerance, f'Numerical gradient approximation of parameter {n} deviates larger than tolerance {error_tolerance}, deviance: {deviance}, approx:{grad_approx, loss_p, loss_n}, p.grad[i][j]:{p.grad[i][j]}'
                else:
                  pass
...
#    from torch.testing import _get_default_tolerance
    ...
    model = torch.nn.Sequential(torch.nn.Linear(5, 50), torch.nn.ReLU(), torch.nn.Linear(50, 50), torch.nn.Linear(50, 5)).double()
