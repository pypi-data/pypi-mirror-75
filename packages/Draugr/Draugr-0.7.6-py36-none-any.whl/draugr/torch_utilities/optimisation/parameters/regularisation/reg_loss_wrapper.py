...
from draugr.torch_utilities.optimisation.parameters.initialisation import normal_init_weights
...
class RegLossWrapper(torch.nn.Module):
  ...
  def forward(self, *loss, **kwargs) -> torch.Tensor:
    return self.loss(*loss) + self.factor * sum([self.l1_crit(p, self.a) for p in self.params])
