...
def plot_grad_flow(model: torch.nn.Module, lines: bool = True, alpha: float = 0.5, line_width: float = 1.0) -> None:
  """
  ...
  if lines:
    ...
    pyplot.bar(numpy.arange(len(max_grads)), max_grads, alpha=alpha, linewidth=line_width, color="r")
    pyplot.bar(numpy.arange(len(max_grads)), ave_grads, alpha=alpha, linewidth=line_width, color="g")
  ...
  max_g = max(max_grads)
  margin = max_g * 1.1
  pyplot.ylim(bottom=max_g - margin, top=margin)  # zoom in on the lower gradient regions
  pyplot.xlabel("Layers")
  pyplot.ylabel("Gradient Magnitude")
  pyplot.title("Gradient Flow")
  pyplot.grid(True)
  pyplot.legend([Line2D([0], [0], color="c", lw=4),
...
if __name__ == '__main__':
  def a():
    ...
    model = torch.nn.Sequential(torch.nn.Linear(50, 50),
                                torch.nn.ReLU(),
