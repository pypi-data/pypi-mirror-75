...
from draugr.torch_utilities.operations.sizes.replication import replicate

__all__ = ['transp_conv2d_output_shape', 'transp_conv2d_padding_sizes']


def transp_conv2d_output_shape(h_w: Union[int, Tuple[int, int]],
  ...
  (h_w, kernel_size, stride, pad, dilation, out_pad) = (replicate(h_w),
                                                        replicate(kernel_size),
                                                        replicate(stride),
                                                        replicate(pad),
                                                        replicate(dilation), replicate(out_pad))
...
def transp_conv2d_padding_sizes(h_w_in: Union[int, Tuple[int, int]],
  ...
  (h_w_in, h_w_out, kernel_size, stride, dilation, out_pad) = (replicate(h_w_in),
                                                               replicate(h_w_out),
                                                               replicate(kernel_size),
                                                               replicate(stride),
                                                               replicate(dilation),
