...
from draugr.torch_utilities.datasets.supervised.utilities.shallow_category import build_shallow_categorical_dataset
from draugr.torch_utilities.datasets.supervised.splitting import Split,select_split
...
class SplitDictDatasetFolder(VisionDataset):
  ...
  def __init__(self,
    ...
    self._data_categories = select_split(build_shallow_categorical_dataset(self.root, extensions=extensions, testing_percentage=test_percentage, validation_percentage=valid_percentage), split)
  ...
  def sample(self, target, index) -> Tuple:
    sample = self.loader(self._data_categories[target][index % self.category_sizes[target]])
...
class DictDatasetFolder(VisionDataset):
  ...
  def __init__(self,
    ...
    super().__init__(root,
                     ...
                     target_transform=target_transform)
    # TODO: merge Split and non split common in a base class
    self._data = build_flat_dataset(self.root, extensions, is_valid_file)
