...
def set_timedelta(time_str:str) -> timedelta:
    time_splitter = r':'
...
def convert_time_float(time_float: float) -> timedelta:
    """Convert the time float from output.json and create a timedelta"""
    microseconds = str(time_float).split(".")[-1]
    minutes, raw_seconds = divmod(
        time_float, 60
...
class Job:
    ...
    def from_srt(cls, filepath: str):
        ...
        for index, marker in enumerate(marker_text):
            ...
            start_time = set_timedelta(start_time)
...
{start_time=} starts before {previous_entry.end_time=}')
            ...
            end_time = set_timedelta(end_time)
