...
from neodroidagent.utilities import TransitionBuffer
...
def collect_samples(pid, queue, env, policy, custom_reward, mean_action,
  ...
  while num_steps < min_batch_size:
    ...
    for t in range(10000):
      state_var = to_tensor(state, device='cpu').unsqueeze(0)
      if mean_action:
        action = policy(state_var)[0].data[0].numpy()
      else:
        action = policy.select_action(state_var)[0].numpy()
      action = int(action) if policy.is_disc_action else action.astype(numpy.float64)
      ...
      memory.add_transition_point(state, action, mask, next_state, reward)
...
class Agent:
  ...
  def collect_samples(self, min_batch_size):
    ...
    for worker_memory in worker_memories:
      memory.add_transition_points(worker_memory)
    ...
    log['action_mean'] = numpy.mean(numpy.vstack(batch.action), axis=0)
    log['action_min'] = numpy.min(numpy.vstack(batch.action), axis=0)
    log['action_max'] = numpy.max(numpy.vstack(batch.action), axis=0)
    return batch, log
