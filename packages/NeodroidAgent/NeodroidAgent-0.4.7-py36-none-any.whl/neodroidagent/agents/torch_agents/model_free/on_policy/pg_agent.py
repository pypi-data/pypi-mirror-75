...
"""
  ...
  def __init__(
    ...
    scheduler_spec: GDKC = GDKC(constructor=
...
"""
    ...
    if distributional_regressor:
      ...
      if action_space.is_discrete:
        self._policy_arch_spec = GDKC(constructor=CategoricalMLP, kwargs=self._policy_arch_spec.kwargs
                                      )
      else:
        self._policy_arch_spec = GDKC(constructor=
                                      MultiDimensionalNormalMLP, kwargs=self._policy_arch_spec.kwargs
                                      )

      self._policy_arch_spec.kwargs["output_shape"] = self._output_shape

      self.distributional_regressor:Module = self._policy_arch_spec().to(self._device)
