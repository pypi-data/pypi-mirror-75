...
class Neo4jOp():
    def __init__(self, host='', port='', user='', password='', label=''):
        if label:
            host = gt.config(label, 'host')
            port = gt.config(label, 'port')
            user = gt.config(label, 'user')
            password = gt.config(label, 'pass')
        self._graph = Graph('http://' + host + ':' + port, username=user, password=password)
        # self._graph = Graph('bolt://' + host + ':' + port, username=user, password=password)
        self._node_map = dict()
        self._edge_map = dict()
