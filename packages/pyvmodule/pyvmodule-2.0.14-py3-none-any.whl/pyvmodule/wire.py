...
__all__ = ['Parameter','Index','Wire','Reg','Fetch']
def e_unhandled_type(x):raise TypeError(type(x))

class Parameter(Expr,NamingNode):
    def __init__(self,value,width=None,name=None,io='auto',T=int,reverse=False,bypass=False):
        NamingNode.__init__(self,name=name,reverse=reverse,bypass=bypass)
        self.typename = 'parameter'
        self.value = T(value)
...
class Wire(Expr,NamingNode):
    random_init = False
    verilator_pragmas_enabled = True
    xilinx_pragmas_enabled = False
    synopsys_pragmas_enabled = False
    @staticmethod
    def disable_pragma_style(style):
        if style == 'verilator':Wire.verilator_pragmas_enabled = False
        elif style == 'xilinx':Wire.xilinx_pragmas_enabled = False
        elif style == 'synopsys':Wire.synopsys_pragmas_enabled = False
        else:raise ValueError('Pragma style "%s" is not supported.'%style)
    @staticmethod
    def enable_pragma_style(style):
        if style == 'verilator':Wire.verilator_pragmas_enabled = True
        elif style == 'xilinx':Wire.xilinx_pragmas_enabled = True
        elif style == 'synopsys':Wire.synopsys_pragmas_enabled = True
        else:raise ValueError('Pragma style "%s" is not supported.'%style)
    ...
    @property
    def width(self):
        return self._width if self._width_expr is None else self._width_expr
    @width.setter
    ...
    def _node_clone(self):
        ...
        return other
    def __len__(self):return int(self.width) if self.length == 1 else int(self.length)
    def __int__(self):
    ...
    def __getitem__(self,key):
        key = Index(key,self)
        if self.length == 1:
            if key.typename=='range' and key.start==0 and key.stop==self.width:return self
            else:return Fetch(self,key)
        else:
            if key.width > 1:raise KeyError('Invalid slicing of 2D-array.')
    ...
    def _verilog_gen_assignment(self,key,val):
        ...
        lines[-1].append(';')
        return lines
    def _synopsys_gen_pragmas(self,myindent=''):
        lines = []
        for key,val in self.pragmas.items():
            if val is False:continue
            word = self._synopsys_gen_pragma(key,val)
            if not word is None:lines.append(myindent + word)
        return lines
    @staticmethod
    def _synopsys_gen_pragma(key,val):
        if Wire.synopsys_pragmas_enabled and key in {'async_set_reset','sync_set_reset'}:
            if isinstance(val,str):return '//synopsys %s %s'%(key,val)
            if isinstance(val,bool):return '//synopsys %s'%key if val else None
            if isinstance(val,int):return '//synopsys %s %d'%(key,val)
            if val is None:return '//synopsys %s'%key
            raise TypeError()
        else:return None
    def _verilator_gen_pragmas(self):
        words = []
        for key,val in self.pragmas.items():
            if val is False:continue
            word = self._verilator_gen_pragma(key,val)
            if not word is None:words.append(word)
        return words
    @staticmethod
    def _verilator_gen_pragma(key,val):
        if Wire.verilator_pragmas_enabled and key in {'clock_enable'}:
            if isinstance(val,str):return '/*verilator %s %s*/'%(key,val)
            if isinstance(val,bool):return '/*verilator %s*/'%key if val else None
            if isinstance(val,int):return '/*verilator %s %d*/'%(key,val)
            if val is None:return '/*verilator %s*/'%key
            raise TypeError()
        else:return None
    def _verilog_gen_pragmas(self,myindent=''):
        ...
        for key,val in self.pragmas.items():
            if val is False:continue
            word = self._verilog_gen_pragma(key,val)
            if not word is None:lines.append(myindent + word)
        return lines
    @staticmethod
    def _verilog_gen_pragma(key,val):
        if Wire.xilinx_pragmas_enabled and key in {'ram_style'}:
            if isinstance(val,str):return '(* %s = "%s" *)'%(key,val)
            if isinstance(val,bool):return '(* %s *)'%key if val else None
            if isinstance(val,int):return '(* %s = %d *)'%(key,val)
            if val is None:return '(* %s *)'%key
            raise TypeError()
        else:return None
...
class Reg(Wire):
    ...
    def _refresh_driven(self):
        if self._driven_by_extra:
            self._driven = (1<<len(self)) - 1
            return
        self._driven = 0
        repeating = set()
        i = 0
...
def check_expr_index(key,width,context):
    if isinstance(key,Wire):
        if key.length > 1:raise ValueError('Ram "%s" could not be used as index.'%str(key))
    if width != (1<<len(key)):
        if width < ((1<<len(key))>>1) or width < ((1<<len(key))<<1):
            raise ValueError('Index "%s"(width=%d) with "%s"(width=%d).'%(str(context),len(context),str(key),len(key)))
class Index(ASTNode):
    def __init__(self,key,context):
        ...
        if isinstance(key,(int,ASTNode)):
            if isinstance(key,int) or key.typename=='const':
                ...
                check_expr_index(key,width,context)
                self.width = 1
                self.typename = '+:'
                self.start = key
                self.stop  = None
            ...
            if key.step is None:
                ...
                if is_const_type(loc):
                    ...
                    if key.start is None:
                        raise IndexError('Cannot fetch with "[:expr:width]",do you mean "[expr::width]"?')
                    self.typename = '+:'
                    check_expr_index(key.start,width - self.width + 1,context)
                    self.start = key.start
                    self.stop  = None
    ...
    def _generate(self,indent=0,p_precedence=99):
        if self.typename=='range':
            ...
            if self.width==1:return self.start._generate(indent)
                ...
                contents[-1].append(str(self.width))
                return contents
    ...
    @property
    def _is_propagatable(self):return True
...
def select_word(self,addr,count=1):
    blksz = len(self)>>len(addr)
    assert len(self)==(blksz<<len(addr))
    assert count > 0 and count <= (1<<len(addr))
    sel = Wire(count*blksz)
    sel.enable_ssa('lv')
    sel.lv = Wire(len(self)+blksz*count-blksz)
    sel.lv[:len(self)] = self
    sel.lv[len(self):] = 0
    for i in range(len(addr)-1,-1,-1):
        lv = Wire(len(sel.lv) - (blksz<<i))
        lv[:] = addr[i].mux(sel.lv[blksz<<i:],sel.lv[:-blksz<<i])
        sel.lv = lv
    sel[:] = sel.lv
    return sel
Expr.select_word = select_word
