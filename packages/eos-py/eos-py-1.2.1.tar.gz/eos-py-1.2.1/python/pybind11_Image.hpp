...
struct type_caster<eos::core::Image3u>
{
    bool load(handle src, bool)
    {
        ...
        // Todo: We should probably check that buf.strides(i) is "default", by dividing it by the Scalar type
        // or something.
    ...
    static handle cast(const eos::core::Image3u& src, return_value_policy /* policy */, handle /* parent */)
    {
        ...
        std::vector<std::size_t> shape = {static_cast<std::size_t>(src.height()),
                                          static_cast<std::size_t>(src.width()), num_channels};
        ...
        std::vector<std::size_t> strides = {num_channels * src.width(), num_channels,
                                            1}; // This seems to work with the row-major Image class. I just
                                                // returned the same strides as the ones we got from NumPy.
        // Note: I think with the change to the new Image class (July 2018), which is now row-major by
        // default, the strides here might have changed. I didn't check. Also, since the new Image stores a
        // vector of Pixels, the question is whether there's any padding added by the compiler, but probably
        // not, since all types that we use are 1 byte or a multiple thereof. numpy: 'f' = fortran = col-major
...
struct type_caster<eos::core::Image4u>
{
    ...
    static handle cast(const eos::core::Image4u& src, return_value_policy /* policy */, handle /* parent */)
    {
        ...
        // Below line works now. In numpy the strides are (2048, 4, 1) though. I think a copy gets created
        // nevertheless?
        std::vector<size_t> strides = {num_channels * src.width(), num_channels,
                                       1}; // might be cols or rows...? I think rows?
        // Note: I think with the change to the new Image class (July 2018), which is now row-major by
        // default, the strides here might have changed. I didn't check. Also, since the new Image stores a
        // vector of Pixels, the question is whether there's any padding added by the compiler, but probably
        // not, since all types that we use are 1 byte or a multiple thereof.
        return array(pybind11::dtype::of<std::uint8_t>(), shape, strides, &src(0, 0).data()[0]).release();
    };
