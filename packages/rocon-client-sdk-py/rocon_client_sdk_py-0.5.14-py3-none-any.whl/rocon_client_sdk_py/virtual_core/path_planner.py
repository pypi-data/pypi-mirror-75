...
class PathPlanner(metaclass=SingletonMetaClass):
    ...
    def init_map(self):
        ...
        self._maps = self._context.map_manager.get_map_list()
    ...
    def path_to_trajectory(self, worker_location, path, backward_progress=False):
        ...
        if backward_progress is False:
            trajectory = self.prepare_changing_direction_before_trajectory(worker_location, path)
        else:
            trajectory = []
    ...
    def prepare_changing_direction_before_trajectory(self, worker_location, path):
        pre_trajectory = []

        try:

            init_direction = pydash.get(worker_location, 'pose2d.theta')
            target_idx = 0
