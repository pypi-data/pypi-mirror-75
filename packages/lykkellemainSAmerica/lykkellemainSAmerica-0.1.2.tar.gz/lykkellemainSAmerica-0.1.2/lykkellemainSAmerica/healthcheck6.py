    ...
    def __init__(self, param):
        if param == 'T':
            sqlafrica = """select distinct m.symbol,b.prio from stock_master m join stock_all a 
                ...
                where b.prio=6 fetch first 5 rows only"""
            sqlafricab = """select distinct b.symbol,ba.prio from benchmark_master b join
                benchmark_all ba on b.symbol=ba.symbol where ba.prio=6 fetch first 5 rows only"""
        else:
            sqlafrica = """select distinct m.symbol,b.prio from stock_master m join stock_all a 
                on m.symbol = a.symbol join benchmark_all b on a.index_code=b.symbol
                where b.prio=6"""
            sqlafricab = """select distinct b.symbol,ba.prio from benchmark_master b join
                benchmark_all ba on b.symbol=ba.symbol where ba.prio=6"""
        conn = connect.create()
        cursor = conn.cursor()
        with conn:
            ...
            if len(results)>0:
                ...
                print("For africa run, no stocks were found with prio=6")
            ...
            if len(resultsb)>0:
                ...
                print("For africa run, no benchmarks were found with prio=6")
