        ...
        with conn6:
            ...
            if param == 'T':
                q = """select distinct a.symbol from stock_all a join benchmark_all b 
                        ...
                        where b.is_active=true and b.prio=6 fetch first 5 rows only"""
            else:
                q = """select distinct a.symbol from stock_all a join benchmark_all b 
                        on a.index_code=b.symbol
                        where b.is_active=true and b.prio=6"""
            cursor6.execute(q)
            val = cursor6.fetchall()
            val = tuple(val)
            # print(val)
            hq = """select symbol, count(*) as cnt from stock_history
                    ...
                    group by symbol"""
            if len(val)>0:
                cursor6.execute(hq, (val,))
                hval = cursor6.fetchall()
                # print(hval)
                ...
                print("No stocks in prio6 available in stock_all")
                stklst = []
            if param == 'T':
            ...
            if len(val)>0:
                ...
                print("No active benchmark in prio6 available in benchmark_all")
                bmklst = []
            ...
            for i in range(len(stklst)):
                ...
                cursor6.execute(chk,(stk,))
                chkc = cursor6.fetchone()
                chkc = chkc[0]
                if chkc != 0:
                    getlogreturn(stk, 'stock_history', cursor6)
                else:
                    getcompletelogreturn(stk, 'stock_history', cursor6,'tmp6')
                ...
                chk50 = """select count(*) from stock_history
                    ...
                    and price_date > current_date - 31"""
                cursor6.execute(chk50,(stk,))
                chk50c = cursor6.fetchone()
                chk50c = chk50c[0]
                if chk50c != 0:
                    getlastrollingmean.rollingmean50d(stk,'stock_history', cursor6)
                else:
                    getrollingmean.rollingmean50d(stk,'stock_history', cursor6,'tmp6')
                ...
                chk200 = """select count(*) from stock_history
                    ...
                    and price_date > current_date - 31"""
                cursor6.execute(chk200,(stk,))
                chk200c = cursor6.fetchone()
                chk200c = chk200c[0]
                if chk200c != 0:
                    getlastrollingmean.rollingmean200d(stk,'stock_history', cursor6)
                else:
                    getrollingmean.rollingmean200d(stk,'stock_history', cursor6,'tmp6')
            ...
            for i in range(len(bmklst)):
                ...
                chkb = """select count(*) from benchmark_history
                    where symbol=%s and price_return is not null
                    and price_date > current_date - 31"""
                cursor6.execute(chkb,(bmk,))
                chkbc = cursor6.fetchone()
                chkbc = chkbc[0]
                if chkbc != 0:
                    getlogreturn(bmk, 'benchmark_history', cursor6)
                else:
                    getcompletelogreturn(bmk, 'benchmark_history', cursor6,'tmp6')
            ...
            chkretb = """select symbol from dbo.benchmark_history where price_return is null and symbol in %s
                       group by symbol having count(*)>1"""
            if len(stklst)>0:
                cursor6.execute(chkrets, (stklst,))
                failstk = cursor6.fetchall()
                #print(failstk)
                print(failstk, "\n are the list of ", len(failstk), " records from ", len(stklst),
                      " stock history that have no calculated return")
            else:
                print("No need to check empty returns as stock list for prio-6 is empty")
            if len(bmklst)>0:
                cursor6.execute(chkretb, (bmklst,))
                failbmk = cursor6.fetchall()
                print(failbmk, "\n are the list of ",len(failbmk)," records from ",len(bmklst)," benchmark history that have no calculated return")
            else:
                print("No need to calculate empty returns as benchmark list is empty fo prio-6")
    ...
    def statistics(param):
        ...
        with conn6:
            if param == 'T':
                ...
                where rf.prio=6 fetch first 5 rows only"""
            else:
                statqry = """select mas.symbol, mas.exchange, mas.currency,
                rf.abbr as abbr,
                rf.country as country, mas.source_table,rf.symbol as bmk_ticker
                ...
                where rf.prio=6"""
            try:
                cursor6.execute(statqry)
                statres = cursor6.fetchall()
            ...
            if len(statres)>0:
                for i in range(len(statres)):
                    ...
                    if symbol is not None and bsymbol is not None and country is not None:
                        allstatistics(symbol, bsymbol, country, sourcetable, cursor6)
                        ...
                        cursor6.execute(delq, (symbol, pdate))
                        selq = """select * from stock_statistics where symbol=%s"""
                        cursor6.execute(selq, (symbol, ))
                        statdata = cursor6.fetchone()
