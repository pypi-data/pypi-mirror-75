...
def mainmasterprogfundamental(param):
    ...
    with conn6:
        ...
        jobloadlist =[]
        cursor6.execute(seljobload,(jday,))
        jobtbl = cursor6.fetchall()
        if jobtbl is None:
            jobtbl = []
        else:
            pass
        ...
        if param == 'T':
            stktblq = """select distinct sa.symbol,ba.prio from stock_all sa join benchmark_all ba
                        ...
                        where ba.prio=6 order by ba.prio fetch first 5 rows only"""
        else:
            stktblq = """select distinct sa.symbol,ba.prio from stock_all sa join benchmark_all ba
                        on sa.index_code=ba.symbol join stock_master mas
                        on sa.symbol=mas.symbol
                        where ba.prio=6 order by ba.prio"""
        cursor6.execute(stktblq)
        tbl = cursor6.fetchall()
        if tbl is None:
            tbl = []
        else:
            pass
        ...
        if lt>0:
            for i in range(lt):
                ...
                if symbol in jobloadlist:
                    ...
                    cursor6.execute(insjob,(symbol,'mfundamental',jday,'WIP','stock_all'))
                    loadstockfundamentals(symbol, stocktable, fxr, prio, jday, cursor6)
        else:
            print("no stocks in stock_All with prio=6. check if someone deleted the stock_All entries")
        if param == 'T':
            ...
            bmktblq = """select distinct ba.symbol,ba.prio from benchmark_all ba
                        ...
                        bm.symbol=ba.symbol
                        where ba.prio=6 order by ba.prio"""
        cursor6.execute(bmktblq)
        btbl = cursor6.fetchall()
        if btbl is None:
            btbl = []
        else:
            pass
        ...
        if blt>0:
            for i in range(blt):
                ...
                if bsymbol in jobloadlist:
                    ...
                    cursor6.execute(insjob,(bsymbol,'mfundamental',jday,'WIP','benchmark_all'))
                    loadstockfundamentals(bsymbol, bstocktable, fxr, bprio, jday, cursor6)
        else:
            print("no stocks in stock_All with prio=6. check if someone deleted the stock_All entries")
    ...
    with conn6:
        ...
        if param == 'T':
            stktblq = """select distinct substring(sa.symbol,position('.' in sa.symbol)+1) as exch,
                        ...
                        on sa.index_code=ba.symbol where ba.prio=6
        ...
        if lt>0:
            for i in range(lt):
                ...
                if symbol in jobloadlist:
                    ...
                    print("Loading:",symbol, stocktable, prio)
                    cursor6.execute(insjob,((symbol+'-'+str(prio)),'mprice',jday,'WIP','stock_all'))
                    loadstockprice(symbol, stocktable, jday, prio, cursor6)
        else:
            print("no stocks in stock_All with prio=6. check if someone deleted the stock_All entries")
        if param == 'T':
            bmrk = """select distinct symbol,prio from benchmark_all
            where is_active=True and prio=6 order by prio fetch first 5 rows only"""
        else:
            bmrk = """select distinct symbol,prio from benchmark_all
            where is_active=True and prio=6 order by prio"""
        cursor6.execute(bmrk)
        btbl = cursor6.fetchall()
        if btbl is None:
            btbl = []
        else:
        ...
        if blt>0:
            for i in range(blt):
                ...
                if bsymbol in jobloadlist:
                    ...
                    print("Loading:",bsymbol, bstocktable, bprio)
                    cursor6.execute(insjob,(bsymbol,'mprice',jday,'WIP','benchmark_all'))
                    loadstockprice(bsymbol, bstocktable, jday, bprio, cursor6)
        else:
            print("no benchmark in benchmark_All with prio=6. check if someone deleted the benchmark_All entries")
