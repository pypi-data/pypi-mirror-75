...
class BeerKeeper:
    ...
    def get_all_beers_from_page(
        self, page_number: int = 1, per_page: int = 25, params: typing.Dict = {}
    ) -> List[Beer]:
        """
        ...
        """
        logger.info(
            f"Getting all beers from page {page_number}, beers per page: {per_page}, params: {params}"
        )

        params["page"] = str(page_number)
        params["per_page"] = str(per_page)
        beers = self.get_request(self.ENDPOINT, params=params)
    ...
    def get_all_beers_generator(
        ...
        while True:
            beers_on_page = self.get_all_beers_from_page(
                page_number, beers_per_page, params=params
            )
            page_number += 1
    ...
    def get_beers_by_abv(self, abv_index: float, greater: bool = True) -> list:
        ...
        beers = self.get_all_beers(params=params)
        return beers
