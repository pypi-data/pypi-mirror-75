...
experiment_table = tables["puppettableTestIndexed"]
...
def test_structure_df(structure_list, assert_equal=None):
    ...
    experiment_table[0] = structure_list.iloc[0]
    experiment_table[1] = structure_list.iloc[1]

    assert_equal(experiment_table[0], structure_list.iloc[0])
    assert_equal(experiment_table[1], structure_list.iloc[1])
    assert_equal(experiment_table[-1], structure_list.iloc[-1])
    assert_equal(experiment_table[-2], structure_list.iloc[-2])
    assert_equal(experiment_table[-2:], structure_list.iloc[-2:])
    assert_equal(experiment_table[0:1], structure_list.iloc[0:1])
    assert_equal(experiment_table[0:], structure_list.iloc[0:])
...
def test_structure(structure_list, assert_equal=None):
    ...
    if assert_equal is None:
        assert_equal = _assert_equal   # TEST INDIVIDUAL ASSIGNMENT
...
test_structure_df(pd.DataFrame(np.random.randn(2, 10), index=[0, 1]), assert_equal=assert_equal)
