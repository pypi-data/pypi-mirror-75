...
class JiebaTokenizer(object):
    ...
    def tokenize(self,
        ...
        text = text.lower()
        return [w for w in self.tokenizer.cut(text) if len(w) >= 2 and w.lower() not in self.stop_words and w[0] != ' ']
    ...
    def pos_tokenize(self, text, pos_filter=('n', 'vn', 'v', 'l', 'eng'), withFlag=False):
        ...
        return [ wp.word for wp in self.pos_tokenizer.cut(text) if wp.flag in pos_filter
                and len(wp.word) >= 2 and wp.word.lower() not in self.stop_words and wp.word[0] != ' ']
