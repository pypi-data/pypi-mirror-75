...
class EnsembleRanker(object):
    ...
    def sentence_rank(self,
                      ...
                      with_weights=False):
        pass
        # self.get_matrix_by_co
        # self.get_weight_from_position
        # self.get_weight_between_sentence_and_topic
        # self.get_matrix_by_w2v
        # self.get_weight_entity_in_desc
    ...
    def sentence_vector(self, sen):
        ...
        :return:
        """
        vec = np.array([self.w2v[word] for word in sen if word in self.w2v])
        if len(vec) > 0:
    ...
    def deduplicate(self, result, s_matrix, num_keyphrase=10):
        ...
        for idx, x in enumerate(result[1:]):
            ...
            for y in data:
                ...
                if self.w2v:
                    vec = self.w2v[x[0]]
                    if self.calculate_cosine_similarity(self.w2v[y[0]], vec) > 0.8:
                        if len(x[0]) > len(y[0]):
                            data.append(x)
                            data.remove(y)
