...
def run_in_proc(working_dir: Optional[str],
    ...
    try:
        ...
        logger.warning('Creating temporary folder in path specified in settings failed ([Errno {}] {}), '
        ...
        try:
            def onerror(func: 'Callable[..., Any]', path: str, exc_info: Any) -> None:
                ...
                if isinstance(exception, OSError):
                    ...
                    logger.warning('{} failed with OSError ([Errno {}] {}) during temp file cleanup, skipping.'
                                   .format(func.__qualname__, exception.errno, exception.strerror))
                else:
                    # As far as I know, this will only get called with OSError, but have this here in case
                    logging_utilities.log_traceback(exception, logger)
                    logger.warning('{} failed with {} during temp file cleanup, skipping.'
                                   .format(func.__qualname__, exception.__class__.__name__))

            shutil.rmtree(tempdir, onerror=onerror)
        except Exception as e:
            logging_utilities.log_traceback(e, logger)
            logger.warning('shutil.rmtree failed with {} during temp file cleanup, skipping.'
