...
def _y_transform(
    ...
    if task_type == constants.Tasks.CLASSIFICATION:
        ...
        if runtime_utilities._is_y_transform_needed(y, y_type) or \
            ...
            if y_valid is not None:
                # ndarray does not have mixed type e.g. np.array([1, 2, 'three']) becomes string ['1', '2', 'three']
                # This can lead to ValueError: y contains previously unseen labels
                # when combining two different data types for y and y_valid
                if y_type == 'string' and y_valid_type != 'string':
                    # convert y_valid to string as well
                    y_valid = pd.Series(y_valid).apply(str).values
                elif y_type != 'string' and y_valid_type == 'string':
                    # convert y to string as well
