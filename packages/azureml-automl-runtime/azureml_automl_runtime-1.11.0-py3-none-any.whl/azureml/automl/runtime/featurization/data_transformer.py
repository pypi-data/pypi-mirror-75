...
class DataTransformer(AutoMLTransformer, FeaturizationInfoProvider):
    ...
    def _check_columns_names_and_convert_types(self,
        ...
        if len(curr_columns_types_mapping) != len(columns_types_mapping):
            ...
            ).with_generic_msg("The number of columns for fitted data and input data is not the same.")
            ...
            for col, col_type in type_converted_columns.items():
                try:
                    ...
                    raise DataErrorException(
                        ...
                        target="DataTransformer._check_columns_names_and_convert_types"
                    ).with_generic_msg(
    ...
    def _check_transformer_param_error(
        ...
        if featurization_utilities.is_transformer_param_overridden(self._featurization_config):
            ...
            if is_fit:
                error_msg = '{} This could be caused by transformer parameters being overridden. '\
                            'Please check logs for detailed error messages.'.format(self.FIT_FAILURE_MSG)
            else:
                error_msg = '{} This could be caused by transformer parameters being overridden. ' \
                            'Please check logs for detailed error messages.'.format(self.TRANSFORM_FAILURE_MSG)
            ...
            try:
                for transformer in transformer_mapper.transformers:
                    ...
                    if featurization_utilities.get_transformer_params_by_column_names(
                        ...
                        raise InvalidValueException.from_exception(
                            ex,
                            msg=error_msg,
                            target='DataTransformer',
                            has_pii=False,
