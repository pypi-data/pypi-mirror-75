...
class EnsembleSelector:
    ...
    def __init__(self,
        ...
        Keyword Arguments:
            ...
            :type iterations: int
        """
        Contract.assert_value(dataset, "dataset", reference_code=ReferenceCodes._ENSEMBLE_SELECTOR_NULL_DATASET)
        Contract.assert_true(iterations >= 1, "Number of iterations ({}) is less than 1".format(iterations),
                             target="iterations", log_safe=True)

        # validate the task_type and input metric to optimize on
        self.task_type = EnsembleSelector._get_validated_task_type(dataset)
    ...
    @staticmethod
    def _get_validated_task_type(dataset: datasets.ClientDatasets) -> str:
        """
        Validate and return the task type used for training.

        :param dataset: ClientDataset object which was used in training
