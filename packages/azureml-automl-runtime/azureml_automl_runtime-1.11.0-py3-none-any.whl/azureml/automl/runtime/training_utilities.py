...
def check_x_y(x: DataInputType,
    ...
    if automl_settings.task_type == constants.Tasks.CLASSIFICATION and not automl_settings.enable_streaming:
        y_ravel = y.ravel()
        unique_classes = _get_num_unique(y_ravel, ignore_na=True)
        logger.info("{} unique classes detected.".format(unique_classes))
        if unique_classes < 2:
            raise InsufficientDataException(
...
def validate_timeseries_training_data(automl_settings: AutoMLBaseSettings,
    ...
    if automl_settings.n_cross_validations is None and X_valid is None:
        ...
        # If the automl_settings.lags uses the auto lag mode, passing in the ndarray will make the returned lags
        # be [None] which will fail the get_min_points(), see details in _get_auto_parameters_maybe().
        # Restore the dataframe if the X is a ndarray.
        if isinstance(X, np.ndarray):
            df = pd.DataFrame(X, columns=x_raw_column_names)
        else:
            df = X
        lags, window_size, max_horizon = _get_auto_parameters_maybe(automl_settings, df, y)
...
def _get_auto_parameters_maybe(automl_settings: AutoMLBaseSettings,
    ...
    lags = automl_settings.lags[constants.TimeSeriesInternal.DUMMY_TARGET_COLUMN] \
        if automl_settings.lags is not None else [0]  # type: List[Union[str, int, None]]
