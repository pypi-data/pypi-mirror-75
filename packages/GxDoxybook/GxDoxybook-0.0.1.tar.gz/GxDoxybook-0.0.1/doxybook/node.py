...
class Node:
    _output_dir = ''
    def __init__(self, xml_file: str, xml: Element, cache: Cache, parser: XmlParser, parent: 'Node', refid: str = None, position: str = None ):
        self._children: ['Node'] = []
    ...
    @property
    def url(self) -> str:
        if self.is_parent or self.is_group or self.is_file or self.is_dir or self.is_page or self.is_function:
            if self._position:
    ...
    def details(self) -> str:
        ...
        if re.findall('gxdocref \[', details):
            details = re.sub('gxdocref \[','[',details)
            return details
       # name = self._name.replace('_', '\\_')
        refs = re.findall('gxdocref +[\S]+', details)
        if refs:
            for ref in refs:
                ...
                details = re.sub('gxdocref *'+refname,'[**'+refname+'**]('+
                        node.url + ')', details)
        return details
    ...
    def brief(self) -> str:
        ...
        if re.findall('gxdocref_', brief):
            brief = re.sub('gxdocref_', 'gxdocref', brief)
            brief = re.sub('(gxdocref) +\[_\*\*(.+)\*\*_\] *\(.+\) _','\g<1> \g<2> ',brief)
        refs = re.findall('gxdocref +[\S]+', brief)
        if refs:
            for ref in refs:
                refname = re.sub('gxdocref[\s]+', '', ref)
                refname = refname.replace('\\','')
                if refname[len(refname)-1] == '_':
                    refname = refname[0:-1]
                node = self._cache.get_on_name(refname)
                refname = refname.replace('_','\\\\_')
                brief = re.sub('gxdocref *'+refname,'[**'+refname+'**]('+
                        node.url + ')', brief)
        return brief
