...
def create_batch_adata(unique,sample_names):
    ...
    for index in indeces:
        ...
        temp_dir[variable_name] = anndata.read_h5ad( \
            "".join([directory_name,"/counts_unfiltered/adata.h5ad"]))  # change to directory_name
...
def populate(adata,organism):
    ...
    adata.var["gene_counts"] = adata.X.sum(axis=0)
...
#----------------------------------------------------------------------------#
def run_qc(adata,directory_name,unique,mito_criteria=None, filtered=False):
    
    plot_directory = directory_name
    os.makedirs(plot_directory, exist_ok=True)
    os.makedirs("/".join([plot_directory, unique]), exist_ok=True)
    plot_directory = "/".join([plot_directory, unique])
    # because this is before filtering
    mito_criteria=mito_criteria
    
    qc_plot_functions.knee_plot(adata,plot_directory)
    qc_plot_functions.mito_content(adata,mito_criteria,plot_directory)
    qc_plot_functions.library_saturation_1(adata,plot_directory)
    qc_plot_functions.library_saturation_2(adata,plot_directory)
    
    if filtered == True:
        sc.pl.highest_expr_genes(adata, save = "".join([plot_directory, "/highest_expr_genes.png"]))

#----------------------------------------------------------------------------#
def filtering(adata,min_cells, min_genes, mito_criteria, organism, max_genes=2500):
