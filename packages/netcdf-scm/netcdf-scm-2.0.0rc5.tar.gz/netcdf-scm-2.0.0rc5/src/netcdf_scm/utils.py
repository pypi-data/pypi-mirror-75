...
def take_lat_lon_mean(in_scmcube, in_weights):
    ...
    -------
    :obj:`ScmCube`, float
        First output is a copy of the input cube in which the data is now the
        latitude-longitude mean of the input cube's data. Second output is
        the sum of weights i.e. normalisation used in the weighted mean.
    """
    out_cube = type(in_scmcube)()
    if in_weights.shape != in_scmcube.cube.shape:
        in_weights_broad = broadcast_onto_lat_lon_grid(in_scmcube, in_weights)

    else:
        in_weights_broad = in_weights

    out_cube.cube = in_scmcube.cube.collapsed(
        [in_scmcube.lat_name, in_scmcube.lon_name],
        iris.analysis.MEAN,
        weights=in_weights_broad,
    )

    normalising_factor = _get_normalising_factor(in_scmcube, in_weights)

    return out_cube, normalising_factor


def _get_normalising_factor(in_scmcube, in_weights):
    helper = type(in_scmcube)()
    helper.cube = in_scmcube._get_lat_lon_slice()
    if len(in_weights.shape) > len(helper.cube.shape):
        weights_area_slice = [
            slice(None)
            if d in [in_scmcube.lat_dim_number, in_scmcube.lon_dim_number]
            else 0
            for d in range(len(in_weights.shape))
        ]
        in_weights = in_weights[tuple(weights_area_slice)]

    in_weights_single_timestep = broadcast_onto_lat_lon_grid(helper, in_weights)
    helper_dat = helper.cube.data  # have to realise data in order to get mask

    if hasattr(helper_dat, "mask"):
        normalising_factor = ma.masked_array(
            in_weights_single_timestep, helper_dat.mask
        ).sum()
    else:
        normalising_factor = in_weights_single_timestep.sum()
