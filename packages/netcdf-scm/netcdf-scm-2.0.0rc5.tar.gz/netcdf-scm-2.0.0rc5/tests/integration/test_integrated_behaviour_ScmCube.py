...
class _ScmCubeIntegrationTester(object):
    ...
    @patch.object(ScmCube, "_sanity_check_area")
    def test_get_scm_timeseries_cubes(
        self, mock_sanity_check_area, test_cube, memory_error, kw_args, regions_to_get,
    ):
        ...
        for label, weights in mocked_weights.items():
            ...
            ] = "{} (more info at gitlab.com/netcdf-scm/netcdf-scm)".format(
        ...
        sanity_check_area_calls = [call(r, region_areas[r]) for r in regions_to_get]
        mock_sanity_check_area.assert_has_calls(sanity_check_area_calls, any_order=True)
    ...
    def test_convert_scm_timeseries_cubes_to_openscmdata(self, test_cube, out_calendar):
        ...
        pd.testing.assert_frame_equal(
            result.timeseries(), expected.timeseries(result.meta.columns)
        )
...
class TestScmCubeIntegration(_ScmCubeIntegrationTester):
    ...
    def test_load_gregorian_calendar_with_pre_zero_years(
        ...
        # ignore tostring warning messages, fixed with netCDF v1.5.4
        # see https://github.com/Unidata/netcdf4-python/blob/master/Changelog
        messages = [
            m for m in caplog.messages if not any([v in m for v in ["tostring"]])
        ]
...
class TestCMIP6OutputCube(_CMIPCubeIntegrationTester):
    ...
    def test_ocean_data_handling(
        ...
        res = test_cube.get_scm_timeseries(regions=regions_to_get)

        assert res.get_unique_meta("variable", no_duplicates=True) == "hfds"
        assert res.get_unique_meta("unit", no_duplicates=True) == "W m^-2"
        assert res.get_unique_meta("scenario", no_duplicates=True) == "ssp119"
        assert set(res.get_unique_meta("region")) == set(regions_to_get)

    def test_different_grid_same_answer_issue_35(
        self, test_cmip6_output_different_grid_same_answer_issue_35
    ):
        regions_to_get = [
            "World",
            "World|Ocean",
        ...
        gr_dir = test_cmip6_output_different_grid_same_answer_issue_35["gr"]
        gn_dir = test_cmip6_output_different_grid_same_answer_issue_35["gn"]

        gr_handler = self.tclass()
        gr_handler.load_data_in_directory(gr_dir)
        gr_res = gr_handler.get_scm_timeseries(regions=regions_to_get)

        gn_handler = self.tclass()
        gn_handler.load_data_in_directory(gn_dir)
        gn_res = gn_handler.get_scm_timeseries(regions=regions_to_get)

        assert (
            gr_res.metadata["crunch_source_files"]
            != gn_res.metadata["crunch_source_files"]
        )
        for key in gr_res.metadata:
            if "area" in key:
                # El Nino is small area so regridding matters more
                rtol = 3 * 1e-3 if "el_nino" not in key else 3 * 1e-2
                np.testing.assert_allclose(
                    gr_res.metadata[key], gn_res.metadata[key], rtol=rtol
                )

        gr_ts = gr_res.timeseries()
        gn_ts = gn_res.timeseries()
        pd.testing.assert_frame_equal(gr_ts, gn_ts, check_like=True)

    def test_get_lat_lon_slice_plevels(self, test_cube, test_cmip6_output_co2_file):
        test_cube.load_data_from_path(test_cmip6_output_co2_file)
        res = test_cube.lat_lon_shape

        assert res == (192, 288)

    def test_get_aermon_lat_lon_slice_plevels(
        self, test_cube, test_cmip6_output_co2_aermon_file
    ):
        test_cube.load_data_from_path(test_cmip6_output_co2_aermon_file)
        res = test_cube.lat_lon_shape

        assert res == (192, 288)

    @pytest.mark.xfail(reason="nans in data cause area calculation to blow up")
    def test_nan_handling_co2_plevels(self, test_cube, test_cmip6_output_co2_file):
        test_cube.load_data_from_path(test_cmip6_output_co2_file)
        # nans mean that calculated area is much less than world area hence
        # error is raised
        test_cube.get_scm_timeseries_cubes()

    def test_aermon_co2_plevels(self, test_cube, test_cmip6_output_co2_aermon_file):
        test_cube.load_data_from_path(test_cmip6_output_co2_aermon_file)
        res = test_cube.get_scm_timeseries_cubes()

        for region, scm_cube in res.items():
            assert scm_cube.cube.shape == (3, 32)
            assert scm_cube.cube.attributes["region"] == region
            assert scm_cube.cube.attributes["variable"] == "co2"
            assert (
                scm_cube.cube.attributes["variable_standard_name"]
        ...
        np.testing.assert_allclose(res["World"].cube.data[0, 0], 0.0002876096972782)
        np.testing.assert_allclose(res["World|Ocean"].cube.data[0, -1], 0.0002864358727)
        np.testing.assert_allclose(res["World|Land"].cube.data[-1, 0], 0.0002897403184)
        np.testing.assert_allclose(
            res["World|Southern Hemisphere"].cube.data[-1, -1], 0.00028643743275
        )
        np.testing.assert_allclose(
            res["World|Northern Hemisphere|Ocean"].cube.data[-2, -2], 0.00028638078818
        )
