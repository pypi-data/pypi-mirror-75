...
TEST_STITCH_OUTPUT_DIR = join(TEST_DATA_ROOT_DIR, "expected-stitch-output")
...
TEST_DATA_CMIP6OUTPUT_CANNOT_BE_PROCESSED_DIR = join(
    TEST_DATA_ROOT_DIR, "cmip6output-cannot-be-processed"
)


@pytest.fixture(scope="session")
def test_data_cmip6output_cannot_be_processed_dir(test_data_root_dir):
    return TEST_DATA_CMIP6OUTPUT_CANNOT_BE_PROCESSED_DIR


@pytest.fixture(scope="session")
def test_cmip6_output_co2_file(test_data_cmip6output_cannot_be_processed_dir):
    return join(
        test_data_cmip6output_cannot_be_processed_dir,
        "CMIP6",
        "CMIP",
        "NCAR",
        "CESM2",
        ...
        "gn",
        "v20191105",
        "co2_Amon_CESM2_esm-hist_r1i1p1f1_gn_185001-185012.nc",
    )


@pytest.fixture(scope="session")
def test_cmip6_output_co2_aermon_file(test_data_cmip6output_dir):
    return join(
        test_data_cmip6output_dir,
        "CMIP6",
        "CMIP",
        ...
        "gn",
        "v20191105",
        "co2_AERmon_CESM2_esm-hist_r1i1p1f1_gn_185001-185003.nc",
    )
...
@pytest.fixture(scope="session")
def test_cmip6_output_conflicting_phenomenon_metadata_directory(
    test_data_cmip6output_dir,
):
    return join(
        test_data_cmip6output_dir,
...
@pytest.fixture(scope="session")
def test_cmip6_output_ocean_data_handling_directory(test_data_cmip6output_dir):
    return join(
        test_data_cmip6output_dir,
        "CMIP6",
        "ScenarioMIP",
...
@pytest.fixture(scope="session")
def test_cmip6_output_different_grid_same_answer_issue_35(test_data_cmip6output_dir):
    gr = join(
        test_data_cmip6output_dir,
        "CMIP6",
        "CMIP",
...
def test_cmip6_stitch_output(test_data_root_dir):
    return join(TEST_STITCH_OUTPUT_DIR, "cmip6output")


@pytest.fixture(
    params=[
        f
        for f in glob.glob(join(TEST_STITCH_OUTPUT_DIR, "**", "*.MAG"), recursive=True)
    ]
)
def stitched_mag_file(request):
    yield request.param
...
@pytest.fixture(scope="session")
def test_marble_cmip5_stitch_output(test_data_root_dir):
    return join(TEST_STITCH_OUTPUT_DIR, "marble-cmip5output")
                ...
                if filenames:
                    ...
                    for f in filenames:
                        ...
                        if update:
                            ...
                            try:
                                base_scmdf = load_scmrun(base_f)
                                comparison_scmdf = load_scmrun(comparison_f)
                                ...
                                for comparison_cube in comparison_cubes:
                                    ...
                                    np.testing.assert_allclose(
                                        base_cube.data, comparison_cube.data, rtol=1e-6
                                    )
...
def assert_scmdata_frames_allclose():
    def _do_assertion(res_scmdf, exp_scmdf):
        ...
        exp_df = exp_scmdf.timeseries(res_df.index.names).sort_index()
        ...
        for base, check in [(exp_scmdf, res_scmdf), (res_scmdf, exp_scmdf)]:
            for k, v in base.metadata.items():
                ...
                if k == "crunch_source_files":
                    ...
                    np.testing.assert_allclose(v, check.metadata[k], rtol=1e-6)
                else:
...
@pytest.fixture(params=NORMALISATION_METHODS)
def normalisation_method(request):
    return request.param


@pytest.fixture(scope="function")
def junk_data_normalisation():
    out = ScmDataFrame(
        np.arange(100),
...
@pytest.fixture(scope="function")
def junk_data_picontrol_normalisation(junk_data_normalisation):
    out = junk_data_normalisation.copy()
...
@pytest.fixture(scope="session")
def picontrol_data_normalisation(test_cmip6_crunch_output):
    test_file = join(
...
@pytest.fixture(scope="session")
def historical_data_normalisation(test_cmip6_crunch_output):
    test_file = join(
        test_cmip6_crunch_output,
        "Amon",
        "CMIP6",
