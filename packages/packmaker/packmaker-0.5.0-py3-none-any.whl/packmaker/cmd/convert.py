...
class ConvertCommand (Subcommand):
    ...
    def run_command(self, arguments):
        ...
        for addon in manifest['files']:
            ...
            print('DEBUG: addon = {}'.format(addon))
            slug = addon['slug']
            if slug is None:
                print('  !! Cannot find a project with id {}'.format(project_id))
                continue
            addonfile = self.api.get_addon_file(project_id, file_id)
            if addonfile is None:
                print('  || Cannot find the correct version of addon: {} (fileID = {})'.format(slug, file_id))
                continue
