...
class CIDClient:
    ...
    def __graphql_get_fulltext(self, cid):
        """
        Produces GQL query for getting CID by artifact
        """
        return f"""
    ...
    def __graphql_get_by_id(self, cid):
        """
    ...
    def get_fulltext(self, cid: str):
        """
        Returns fulltext for particular CID if it exists
        :param cid:
        """
        self.__check_auth()
        data = self.__run_gql_query(self.__graphql_get_fulltext(cid))
        return data['data']['getFulltext']['fulltext']

    def get_by_id(self, cid: str):
        """
        Returns all fields of CID.
        """
        data = self.__run_gql_query(self.__graphql_get_by_id(cid))
