...
class classifyProducts(object):
    ...
    def inference_from_category_model(self, ingredients=[''], allergics=[''], description=[''], product_name=['']): 
        """
        ...
        if 'category_model' not in os.listdir():
            self.download_model(
                'https://drive.google.com/u/1/uc?export=download&confirm=WAvq&id=1Oi3aQlZppBQT_GuiJaBCj3wZapxYgW4a', 'category_model.tar.gz')
        ...
        cnn.load_weights('./category_model/v1/model_checkpoint/cp.ckpt')
    ...
    def inference_from_food_model(self, product_name=['']):
        """
        ...
        cnn = CNN(vocab_size=self.tokenizer.vocab_size, emb_dim=300, nb_filters=100,
            ffn_units=256, nb_classes=2, dropout_rate=0.2)

        cnn.load_weights('./food_model/v1/model_checkpoint/cp.ckpt')

        product_type = cnn(np.array(X), training=False).numpy()
        product_type = 1 if product_type > 0.5 else 0

        return label_encoder.inverse_transform([product_type]) 
