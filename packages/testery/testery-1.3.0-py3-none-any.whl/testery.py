...
def upload_build_artifacts(api_url, token, project, branch, build_id, path, zip_dir):
    """
    Uploads a file or directory of build artifacts and associates them with the specified build-id
    """
    headers['Authorization'] = "Bearer " + token
    base_api_url = api_url + '/projects/' + project + '/upload-url?build=' + build_id
...
@click.option('--api-url', default='https://api.testery.io/api', help='The URL for the Testery API. Only required for development purposes.')
@click.option('--token', required=True, help='Your Testery API token.')
@click.option('--project', default=None, help='The project key of the repo being deployed.')
@click.option('--git-provider', default=None, help='The Git provider used for the reposityr that is being deployed. Should be GitHub or BitBucket')
...
@click.option('--build-id', default=None, help='The build the atifact should be associated with.')
@click.option('--environment', required=True, help='Which environment you would like to run your tests against.')
@click.option('--commit', required=True, help='Which environment you would like to run your tests against.')
@click.option('--branch', default=None, help='The Git branch the deploy came from.')
def create_deploy(api_url, token, project, git_provider, git_owner, git_repo, environment, commit, build_id, branch):
    """
    Creates a deploy for a project and environment.
    """
    deploy_request = {"environment": environment, "commit": commit}

    if project:
        deploy_request['project'] = project
...
def report_test_run_cmd(api_url, token, test_run_id, fail_on_failure, output, outfile):
    ...
    file_node.set('path', str(test_run_id))
    ...
    for test_run_result in test_run_results:
        ...
        if test_run_result.get('status') == 'FAIL':
            ...
            error_node.text = test_run_result.get('error')
