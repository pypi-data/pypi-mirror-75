...
class LDAPAuthenticator(Authenticator):
    ...
    create_user_home_dir_cmd = Unicode(
        config=True,
        help="""
    ...
    def _default_create_user_home_dir_cmd(self):
        if sys.platform == 'linux':
            home_dir_cmd = "adduser --disabled-password --gecos '' %s && adduser %s sudo && echo '%%sudo ALL=(ALL) NOPASSWD:ALL' >> /etc/sudoers;"
    ...
    def add_user(self, user):
        if self.create_user_home_dir:
            user_name = user.name
            user_exists = yield gen.maybe_future(self.user_home_dir_exists(user_name))
            if not user_exists:
                yield gen.maybe_future(self.add_user_home_dir(user_name))
        yield gen.maybe_future(super().add_user(user))

    def user_home_dir_exists(self, user_name):
        """
    ...
    def add_user_home_dir(self, user_name):
        """
        Creates user home directory
        """
        # cmd = self.create_user_home_dir_cmd + [user_name]
        self.log.info(self.create_user_home_dir_cmd)
        cmd = self.create_user_home_dir_cmd % (user_name, user_name)
