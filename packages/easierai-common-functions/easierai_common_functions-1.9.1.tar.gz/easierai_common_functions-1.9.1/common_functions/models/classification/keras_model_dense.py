...
class Predictor:
    ...
    def get_model_seriot(self, shape, num_classes, inner_activation='relu', loss='categorical_crossentropy', metrics=['acc','categorical_accuracy'], learning_rate=0.01, decay_lr=0):
        model = Sequential()
        
        model.add(Conv1D(filters=256, kernel_size=2, input_shape=shape, activation=inner_activation))
        model.add(MaxPooling1D(pool_size=4))
        # model.add(Dense(units=shape[0], input_shape=shape, activation=inner_activation))
        # model.add(Dropout(0.1))
        model.add(Dense(units=256, activation=inner_activation))
        model.add(Dropout(0.2))
        model.add(Dense(units=128, activation=inner_activation))
        model.add(Dropout(0.1))
        model.add(Dense(units=32, activation=inner_activation))
        model.add(Dropout(0.05))
        model.add(Flatten())
        model.add(Dense(units=num_classes, activation='softmax'))

        # opt = optimizers.Adagrad(lr=self.learning_rate, epsilon=None, decay=self.decay_lr)
        opt = optimizers.RMSprop(lr=learning_rate, decay=decay_lr)
...
class Predictor:
    ...
    def get_model_configurable(self, shape, num_classes, inner_activation='relu', num_dense_layers=1, num_units_per_layer=[64], loss='categorical_crossentropy', metrics=['acc','categorical_accuracy']):
        ...
        self.model = model
        return model
>>>>>>> b0f035d0d84d9ec58f96dbdc16d8ad715af5bb94
