...
def stun_test(sock, host, port, source_ip, source_port, send_data=""):
    ...
    while not recvCorr:
        ...
        log.debug("messagetype:%s", msgtype)
        isBindRespMsg = msgtype in dictValToMsgType.keys() and dictValToMsgType[msgtype] == "BindResponseMsg"
        tranid_match = tranid.upper() == binascii.b2a_hex(buf[4:20]).upper().decode()
        if isBindRespMsg and tranid_match:
            recvCorr = True
            retVal['Resp'] = True
...
def get_ip_info(source_ip="0.0.0.0", source_port=54321, stun_host=None,
                stun_port=3478):
...
def parser_argument():
    ...
    parser.add_argument('-H', '--stun-host',
        help='STUN host to use' )
    parser.add_argument('-P', '--stun-port', type=int,
    ...
    parser.add_argument(       '-p', '--source-port', type=int,
...
def main():
    try:
        ...
        options.debug = True
        options.stun_host = 'stun.ekiga.net'
