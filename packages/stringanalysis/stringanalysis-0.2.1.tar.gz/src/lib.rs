...
trait ImperfectLookup<U> {
    fn get_by_superstring(&self, key: &str) -> Vec<(&str, &U)>;
...
impl<U> ImperfectLookup<U> for BTreeMap<String, U> {
    fn get_by_superstring(&self, key: &str) -> Vec<(&str, &U)> {
        key.char_quadgrams().iter()
            ...
            .map(|(k, v)| (k.as_str(), v))
            .sorted_by_key(|(k, v)| {
                Reverse((100.0 * normalized_damerau_levenshtein(k, key)) as u8)
            })
            .collect()
    }
...
impl StringAnalysisDict {
    ...
    fn get_by_superstring(&self, k: Option<&str>, min_length: usize) -> Vec<(&str, &PyObject)> {
        ...
        // let keys = k.and_then(|k| k.char_quadgrams().iter().fold(None, |acc: Option<HashSet<String>>, i| {
        //     println!("i={:?} acc={:?}", i, acc);
        //     let out = if let Some(keys) = acc {
        //         let x: HashSet<_> = keys.intersection(self.quadgram_index.get(*i).unwrap_or_else(|| &empty_set)).map(|i| i.clone()).collect();
        //         Some(x)
        //     } else {
        //         self.quadgram_index.get(*i).map(|i| i.clone())
        //     };
        //     out
        // }));
        //
    ...
    fn get_by_any_superstring(&self, ks: Vec<Option<&str>>, min_length: usize) -> Vec<(&str, &PyObject)> {
        ks.iter().flat_map(|k| self.get_by_superstring(k.as_ref().map(|i| *i), min_length)).dedup().collect()
    }
