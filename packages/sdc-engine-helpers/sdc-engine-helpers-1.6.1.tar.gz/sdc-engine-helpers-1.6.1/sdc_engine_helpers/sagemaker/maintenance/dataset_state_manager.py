...
class DatasetStateManager:
    ...
    def download_json_blob(self, s3_url: str):
        """download json data from s3"""
        data_stream = self.download_s3_blob(s3_url=s3_url)
        return json.loads(data_stream.read().decode("UTF-8"))

    def stream_data_between_s3(self, src_url: str, dst_url: str):
        """
        ...
        """
        try:
            data_stream = self.download_s3_blob(s3_url=src_url)
            self.upload_s3_blob(
                blob=data_stream,
                s3_url=dst_url
            )
            ...
            if exception.response['Error']['Code'] == "404":
                ...
                print((
                    "Unable to find object. Skipping the task, "
                    "the following exception was found = {}".format(exception)
                ))
            else:
                raise

    def cache_lookup(self, cache_key: str, json_data: dict):
        """cache lookup in elasticcache
    ...
    def update_state(
        ...
        if dataset.get('should_store'):
            ...
            if isinstance(identifier, str):
                ...
                print("Version Control: Move prod -> train")
                self.stream_data_between_s3(
                    src_url=prod_url,
                    dst_url=previous_url
                )

                print("caching data in elastic cache")
                # get data
                json_data = self.download_json_blob(s3_url=identifier)
                ...
                print("Version Control: Move prod -> previous")
                self.stream_data_between_s3(
                    src_url=identifier,
                    dst_url=prod_url
                )
