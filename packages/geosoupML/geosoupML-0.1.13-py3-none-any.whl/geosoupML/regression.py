...
class _Regressor(object):
    ...
    def get_training_fit(self,
        ...
        if self.fit:
            ...
            if callable(sample_predictions):
                pred = sample_predictions(data.format_data(),
    ...
    def cv_result(regressor,
        ...
        if not return_summary:
            ...
            return {'rsq_mean': np.mean([result['rsq'] for result in results]),
                    ...
                    'split_rsq': [result['rsq'] for result in results],
                    'split_rmse': [result['rmse'] for result in results]}
...
class RFRegressor(_Regressor):
    ...
    def sample_predictions(self,
        ...
        :param data: Dictionary object from Samples.format_data
        :param output_type: Metric to be computed from RandomForestRegressor. List of options or a single option.
        ...
        if type(output_type) not in (tuple, list):
            output_type = [output_type]

        mean_prediction = self.predict(data['features'],
                                       output_type='mean',
                                       verbose=verbose)
        ...
        rmse = sqrt(mean_squared_error(data['labels'], mean_prediction))

        # if outfile and pickle file are not provided,
        # then only return values
        out_dict = {
            'pred_y': mean_prediction,
            'obs_y': data['labels'],
            'rmse': rmse,
            'rsq': lm['rsq'],
            'slope': lm['slope'],
        ...
        for output_type_elem in output_type:
            if output_type_elem != 'mean':
                out_dict[output_type_elem] = self.predict(data['features'],
                                                          output_type=output_type_elem,
                                                          verbose=verbose)
        return out_dict
